...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/xml_export/delete_confirm','id=' . htmlentities($templates['id']))) . ' ' . button_icon_edit(url_for('ext/xml_export/form','id=' . htmlentities($templates['id']))) ?></td>
  <td><?php echo htmlentities($templates['entities_name']) ?></td>
  <td style="white-space: normal"><?php echo htmlentities($templates['name']) . 
								 '<div><small><a href="' . url_for('ext/xml_export/filters','templates_id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)) . '">' . TEXT_FILTERS . ' (' . xml_export::count_filters(filter_var($templates['id'],FILTER_SANITIZE_STRING)). ')</a></small></div>';  					
  					echo (filter_var($templates['is_public'],FILTER_SANITIZE_STRING)==1 ? '<div><small>' . TEXT_URL . ': <a href="' . url_for('export/xml','id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)) . '" target="_blank">' . url_for('export/xml','id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)) . '</a></small></div>':'')
  ?></td>  
  <td>
<?php
