...
switch($app_module_action)
{
	case 'save':
		...
		redirect_to('ext/processes/clone_subitems','process_id=' . _get::int('process_id') . '&actions_id=' . _get::int('actions_id'));
		break;
	case 'delete':
	...
	case 'get_fields_schema':
		if(isset($_POST['id']))
		{
			$obj = db_find('app_ext_processes_clone_subitems',filter_var($_POST['id'],FILTER_SANITIZE_STRING));			
		}
		else
		{
			$obj['fields'] = '';
		...
		$html = '
				<div class="form-group">
			  	<label class="col-md-3 control-label" for="fields_id">' . 
			  		TEXT_FIELDS  . 
			  		fields::get_available_fields_helper(_post::int('from_entity_id'), 'fields',TEXT_EXT_FROM_ENTITY . ' #' . _post::int('from_entity_id'),[],true) . 
			  		fields::get_available_fields_helper(_post::int('to_entity_id'), 'fields',TEXT_EXT_TO_ENTITY . ' #' . _post::int('to_entity_id'),[],true) .  '</label>
			    <div class="col-md-9">	
			  	  ' . textarea_tag('fields',$obj['fields'],array('class'=>'form-control input-large required')) . '
