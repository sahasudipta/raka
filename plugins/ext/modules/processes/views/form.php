...
<?php echo form_tag('process_form', url_for('ext/processes/processes','action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
$check_query = db_query("select * from app_ext_processes " . (isset($_GET['id']) ? " where id!='" . filter_var($_GET['id'],FILTER_SANITIZE_STRING) . "'":''));
