...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/xml_import/delete_confirm','id=' . htmlentities($templates['id']))) . ' ' . button_icon_edit(url_for('ext/xml_import/form','id=' . htmlentities($templates['id']))) ?></td>
  <td><?php echo htmlentities($templates['id']) ?></td>
  <td><?php echo htmlentities($templates['entities_name']) ?></td>
  <td style="white-space: normal">
  	<?php echo htmlentities($templates['name']) ?>
  	<?php 
  	if(strlen($templates['filepath']))
  	{
  	    echo tooltip_text('<small>' . TEXT_EXT_IMPORT_BY_URL . ': <a href="' . url_for('ext/xml_import/import_preview','id=' . htmlentities($templates['id'])) . '">' . TEXT_EXT_XML_IMPORT_PREVIEW . '</a></small>');
  	}
  	?>
  </td>  
  <td>
