...
<?php echo form_tag('templates_form', url_for('ext/xml_import/templates','action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
<script>
  ...
  function load_xml_template_fields()
  {
    ...
    $('#template_fields').load('<?php echo url_for("ext/xml_import/templates","action=get_fields")?>',{entities_id:$('#entities_id').val(), id:'<?php echo htmleneities($obj["id"]) ?>'},function(response, status, xhr) {
      if (status == "error") {                                 
  ...
  {  	   	
    ...
    $('#parent_items_choices').load('<?php echo url_for("ext/xml_import/templates","action=get_parent_items_choices")?>',{entities_id:$('#entities_id').val(), id:'<?php echo htmlentities($obj["id"]) ?>'},function(response, status, xhr) {
      if (status == "error") {                                 
         $(this).html('<div class="alert alert-error"><b>Error:</b> ' + xhr.status + ' ' + xhr.statusText+'<div>'+response +'</div></div>')                    
      }
      else
