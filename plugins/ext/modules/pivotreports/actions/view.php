...
switch($app_module_action)
{
	case 'get_localization':
		...
		if(isset($_GET['id']))
		{
			...
			$pivotreports = db_fetch_array($pivotreports_query);
		}
		else
		{
...
$pivotreports_query = db_query("select * from app_ext_pivotreports where id='" . db_input((int)filter_var($_GET['id'],FILTER_SANITIZE_STRING)) . "'");
...
{
	$sql_data = array('name'=>'',
			'entities_id'=>filter_var($pivotreports['entities_id'],FILTER_SANITIZE_STRING),
			'reports_type'=>'pivotreports' . filter_var($pivotreports['id'],FILTER_SANITIZE_STRING),
			'in_menu'=>0,
			'in_dashboard'=>0,
...
{
	case 'set_view_mode':
		$sql_data = array('view_mode'=>filter_var(_get::int('view_mode'),FILTER_SANITIZE_STRING));
		
		if($app_user['group_id']>0 and $pivotreports['allow_edit']==1)
		{
			db_perform('app_ext_pivotreports_settings',$sql_data,'update',"reports_id='" . filter_var((int)$_GET['id'],FILTER_SANITIZE_STRING) . "' and users_id='" . filter_var($app_user['id'],FILTER_SANITIZE_STRING). "'");
		}
		else 
		{			
			db_perform('app_ext_pivotreports',$sql_data,'update',"id='" . filter_var((int)$_GET['id'],FILTER_SANITIZE_STRING) . "'");
		}
		
		redirect_to('ext/pivotreports/view','id=' . filter_var(_get::int('id'),FILTER_SANITIZE_STRING));
		break;
	case 'update_settings':
		...
		if($app_user['group_id']>0 and $pivotreports['allow_edit']==1)
		{
			db_perform('app_ext_pivotreports_settings',filter_var_array($sql_data),'update',"reports_id='" . db_input(filter_var((int)$_GET['id'],FILTER_SANITIZE_STRING)) . "' and users_id='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)). "'");			
		}
		else
		{					
			db_perform('app_ext_pivotreports',filter_var_array($sql_data),'update',"id='" . filter_var((int)$_GET['id'],FILTER_SANITIZE_STRING) . "'");
		}
	...
	case 'get_csv':
		...
		{
			$fields_query = db_query("select * from app_fields where id in (" . implode(',',filter_var_array($reports_fields)). ") order by id");
		...
		{			
			foreach($parrent_entities as $entities_id)
			{
				...
				if(count($reports_fields))
				{
					$fields_query = db_query("select f.*, e.name as entity_name from app_fields f left join app_entities e on f.entities_id=e.id where f.id in (" . implode(',',filter_var_array($reports_fields)). ") order by f.id");
					while($fields = db_fetch_array($fields_query))
					{
