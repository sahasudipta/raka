...
<?php echo ajax_modal_template_header(TEXT_HEADING_FUNCTION_IFNO) ?>

<?php echo form_tag('common_reports_form', url_for('ext/functions/functions','action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
<div class="tab-content">
  <div class="tab-pane fade active in" id="general_info">  
		  ...
		  <div class="form-group">
		  	<label class="col-md-3 control-label" for="functions_name"><?php echo TEXT_EXT_FUNCTION ?></label>
		    <div class="col-md-9">	
		  	  <?php echo select_tag('functions_name',functions::get_choices(),$obj['functions_name'],array('class'=>'form-control input-large required')) ?>
		    </div>			
		  </div>  
		  
		  <div class="form-group form-group-function-select" style="display:none">
		  	<label class="col-md-3 control-label" for="functions_name"><?php echo TEXT_INFO ?></label>
		    <div class="col-md-9">	
		  	  <p class="form-control-static"><?php echo TEXT_EXT_FUNCTION_SELECT_INFO ?></p>
		    </div>			
		  </div>
		  
		  <div class="form-group function-prop">
		  	<label class="col-md-3 control-label" for="functions_formula"><?php echo TEXT_FORMULA ?> <div id="available_fields"></div></label>
		    <div class="col-md-9">	
		  	  <?php echo textarea_tag('functions_formula',$obj['functions_formula'],array('class'=>'form-control')) ?>
		      <?php echo tooltip_text(TEXT_EXT_FUNCTIONS_FORMULA_INFO); ?>      
		    </div>			
		  </div>  
...
<script>
  ...
  {         
    ...
    $('#available_fields').load('<?php echo url_for("ext/functions/functions","action=get_available_fields") ?>',{entities_id:$('#entities_id').val()},function(){
