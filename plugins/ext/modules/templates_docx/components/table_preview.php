...
{
    $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where b.templates_id = " . filter_var($template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id = " . filter_var($rows['id'],FILTER_SANITIZE_STRING) . " order by b.sort_order, b.id");
...
while($blocks = db_fetch_array($blocks_query))
{
    ...
    $cell_name = (strlen($settings->get('heading')) ? $settings->get('heading') : fields_types::get_option(filter_var($blocks['field_type'],FILTER_SANITIZE_STRING), 'name',filter_var($blocks['name'],FILTER_SANITIZE_STRING)));
...
while($rows = db_fetch_array($rows_query))
{
    $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where b.templates_id = " . filter_var($template_info['id']) . " and b.parent_id = " . filter_var($rows['id']) . " order by b.sort_order, b.id");
    ...
    if(db_num_rows($blocks_query))
    {
        ...
        while($blocks = db_fetch_array($blocks_query))
        {
            $settings = new settings(filter_var($blocks['settings']));
            
            //get field value if field is selected
            $cell_value = (filter_var($blocks['fields_id'])>0 ? '*':$settings->get('heading'));
