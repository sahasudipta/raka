...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/templates_docx/extra_rows_delete','id=' . filter_var($blocks['id'],FILTER_SANITIZE_STRING) . '&templates_id=' . $template_info['id']. '&parent_block_id=' . $parent_block['id'] . '&row_id=' . $row_info['id'])) . ' ' . button_icon_edit(url_for('ext/templates_docx/extra_rows_form','id=' . filter_var($blocks['id'],FILTER_SANITIZE_STRING) . '&templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id'] . '&row_id=' . $row_info['id'])) ?></td>   
  <td><?php echo $settings->get('heading') ?></td>
  <td><?php 
  if(filter_var($blocks['fields_id'])>0)
  {
      echo fields::get_name_by_id(filter_var($blocks['fields_id'],FILTER_SANITIZE_STRING));
  }
  ?></td>
