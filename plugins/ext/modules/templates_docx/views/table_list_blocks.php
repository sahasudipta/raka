<ul class="page-breadcrumb breadcrumb">
  ...
  <li><?php echo TEXT_EXT_TABLE_LIST ?></li>  
</ul>

<p><?php echo TEXT_EXT_EXPORT_TEMPLATES_TABLE_LIST_BLOCK_TIP ?></p>

<h3 class="page-title"><?php echo TEXT_COLUMNS ?></h3>

<?php echo button_tag(TEXT_BUTTON_ADD,url_for('ext/templates_docx/table_list_blocks_form','templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id'])) ?>
...
$blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where b.block_type='table_list_cell' and b.templates_id = " . $template_info['id'] . " and b.parent_id = " . $parent_block['id'] . " order by b.sort_order, b.id",false);
...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/templates_docx/table_list_blocks_delete','id=' . filter_var($blocks['id'],FILTER_SANITIZE_STRING) . '&templates_id=' . $template_info['id']. '&parent_block_id=' . $parent_block['id'])) . ' ' . button_icon_edit(url_for('ext/templates_docx/table_list_blocks_form','id=' . filter_var($blocks['id'],FILTER_SANITIZE_STRING) . '&templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id'])) ?></td>  
  ...
  {
      $choices = [];
      $fields_query = db_query("select * from app_fields where id in (" . implode(',',$settings->get('fields')) . ") order by field(id," . implode(',',$settings->get('fields')) . ")");
