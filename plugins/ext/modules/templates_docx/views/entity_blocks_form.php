...
<?php echo form_tag('templates_form', url_for('ext/templates_docx/entity_blocks','templates_id=' . filter_var(_GET('templates_id'),FILTER_SANITIZE_STRING) . '&action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'')  . '&parent_block_id=' . $parent_block['id']),array('class'=>'form-horizontal')) ?>
...
$choices = [];
$fields_query = fields::get_query($field_entity_id," and f.type not in ('fieldtype_action','fieldtype_parent_item_id') and f.id not in (select fields_id from app_ext_items_export_templates_blocks where block_type = 'entity' and templates_id=" . $template_info['id'] . " " . ($obj['fields_id']>0 ? " and fields_id!=" . $obj['fields_id'] :"") . "  and parent_id=" . $parent_block['id'] . ")");
while($fields = db_fetch_array($fields_query))
{
    $choices[$app_entities_cache[$field_entity_id]['name']][filter_var($fields['id'],FILTER_SANITIZE_STRING)] = fields_types::get_option(filter_var($fields['type'],FILTER_SANITIZE_STRING), 'name',filter_var($fields['name'],FILTER_SANITIZE_STRING));
}
...
<script>
  ...
  { 
    ...
    $('#block_settings').load('<?php echo url_for("ext/templates_docx/entity_blocks_settings",'templates_id=' . _GET('templates_id') ) ?>',{fields_id:fields_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
      if (status == "error") {                                 
