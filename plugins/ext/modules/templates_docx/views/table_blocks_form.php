...
<?php echo form_tag('templates_form', url_for('ext/templates_docx/table_blocks','templates_id=' . filter_input(INPUT_GET,_GET('templates_id'),FILTER_VALIDATE_INT) . '&action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'')  . '&parent_block_id=' . filter_var($parent_block['id'],FILTER_SANITIZE_STRING)),array('class'=>'form-horizontal')) ?>
...
<script>
  ...
  {
	 $('#field_settings').load("<?php echo url_for('ext/templates_docx/table_blocks','id=' . $obj['id'] . '&action=get_field_settings&templates_id=' . $template_info['id'] . '&parent_block_id=' . $parent_block['id']) ?>",{fields_id:$('#fields_id').val()},function(){ appHandleUniform(); })  
  }
</script>