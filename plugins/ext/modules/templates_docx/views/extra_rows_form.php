...
<?php echo form_tag('templates_form', url_for('ext/templates_docx/extra_rows','templates_id=' . filter_var(_GET('templates_id'),FILTER_SANITIZE_STRING) . '&action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'')  . '&parent_block_id=' . filter_var($parent_block['id'],FILTER_SANITIZE_STRING) . '&row_id=' . filter_var($row_info['id'],FILTER_SANITIZE_STRING)),array('class'=>'form-horizontal')) ?>
