...
<?php echo form_tag('reports_filters', url_for('ext/templates/export_templates_filters','action=save&templates_id=' . filter_var($_GET['templates_id'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING) . (isset($_GET['parent_reports_id']) ? '&parent_reports_id=' . filter_var($_GET['parent_reports_id'],FILTER_SANITIZE_STRING):'').  (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
