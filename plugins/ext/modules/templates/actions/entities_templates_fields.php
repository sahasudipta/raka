...
$template_info_query = db_query("select ep.*, e.name as entities_name from app_ext_entities_templates ep, app_entities e where e.id=ep.entities_id and ep.id='" . db_input(filter_var($_GET['templates_id'],FILTER_SANITIZE_STRING)) . "' order by e.id, ep.sort_order, ep.name");
...
switch($app_module_action)
{
  case 'render_template_field':
        ...
        {        
          ...
          if(isset($_POST['id']))
          {
            $obj = db_find('app_ext_entities_templates_fields',filter_var($_POST['id'],FILTER_SANITIZE_STRING));
          ...
          $params = array('parent_entity_item_id'=>0); 
  ...
  case 'save':
      ...
      else
      {
        ...
        $check_query = db_query("select * from app_ext_entities_templates_fields where fields_id='" . db_input(filter_var($field['id'],FILTER_SANITIZE_STRING)) . "' and templates_id='" . db_input(filter_var($_GET['templates_id'],FILTER_SANITIZE_STRING)) . "'");
      ...
      {                   
        db_perform('app_ext_entities_templates_fields',$sql_data,'update',"id='" . db_input($templates_fields_id) . "'");       
      }
      else
      {                     
        db_perform('app_ext_entities_templates_fields',$sql_data);                              
      }
          
      redirect_to('ext/templates/entities_templates_fields','templates_id=' . filter_var($_GET['templates_id'],FILTER_SANITIZE_STRING)); 
    break;
  case 'delete':
      if(isset($_GET['id']))
      {                          
        db_query("delete from app_ext_entities_templates_fields where id='" . db_input(filter_var($_GET['id'],FILTER_SANITIZE_STRING)) . "'");
