...
<?php echo form_tag('configuration_form', url_for('ext/funnelchart/reports','action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
<?php 
	$choices = ['funnel'=>TEXT_EXT_FUNNEL_CHART,'bars'=>TEXT_EXT_BARS_CAHRT,'table'=>TEXT_EXT_TABLE];	
?>    
    <div class="form-group">
    	<label class="col-md-3 control-label" for="name"><?php echo TEXT_TYPE ?></label>
      <div class="col-md-9">	
    	  <?php echo select_tag('type',$choices,$obj['type'],array('class'=>'form-control input-large')) ?>        
      </div>			
    </div>
...
{ 
  ...
  $('#reports_entities_fields').load('<?php echo url_for("ext/funnelchart/reports","action=get_entities_fields")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
...
function ext_get_entities_fields_choices()
{
	...
	if(fields_id>0)
	{
	  ...
	  $('#fields_chocies_list').load('<?php echo url_for("ext/funnelchart/reports","action=get_entities_fields_choices")?>',{fields_id:fields_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
	    if (status == "error") {                                 
	       $(this).html('<div class="alert alert-error"><b>Error:</b> ' + xhr.status + ' ' + xhr.statusText+'<div>'+response +'</div></div>')                    
	    }
	    else
