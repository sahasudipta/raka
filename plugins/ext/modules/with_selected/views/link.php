...
<?php echo ajax_modal_template_header(TEXT_EXT_LINK_RECORDS_HEADING) ?>
...
<?php echo form_tag('add_related_items', url_for('ext/with_selected/link','action=add_related_items&reports_id=' . _GET('reports_id')), array('class'=>'form-horizontal')) ?>
...
<div class="modal-body ajax-modal-width-790" style="padding-bottom: 0;">
	<div class="form-body"> 
      ...
      <div class="form-group">  	
           ...
           if(count($choices)>1)
           {
               echo '<div class="col-md-4">' . select_tag('related_to_field',$choices,'',array('class'=>'form-control chosen-select', 'onChange'=>'set_related_to_field()')) . '</div>';
           }
           else
           {
               echo '<label class="col-md-4 control-label">' . current(filter_var_array($choices)) . input_hidden_tag('related_to_field',key(filter_var_array($choices))) . '</label>';
           }
           ?>                   		
        <div class="col-md-8" >
...
function set_related_to_field()
{
	...
	$('#items_select').load('<?php echo url_for('ext/with_selected/link','action=items_select&path=' . $app_path . '&entities_id=' . _GET('entities_id') . '&reports_id=' . _GET('reports_id')) ?>',{related_to_field:$("#related_to_field").val()})
}
