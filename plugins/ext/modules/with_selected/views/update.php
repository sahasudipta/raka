...
<?php echo form_tag('form-update-fields', url_for('ext/with_selected/update','action=update_selected&reports_id=' . filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING))) ?>
...
if(isset($_GET['path']))
{
  echo input_hidden_tag('path',filter_var($_GET['path'],FILTER_SANITIZE_STRING));
}
?>
...
<?php
  ...
  $allowed_types = array('fieldtype_checkboxes',
  ...
  while($v = db_fetch_array($fields_query))
  {
    ...
    $cfg = new fields_types_cfg(filter_var($v['configuration'],FILTER_SANITIZE_STRING));
    
    //handle stages
    if(filter_var($v['type'],FILTER_SANITIZE_STRING)=='fieldtype_stages' and $cfg->get('click_action')=='change_value_next_step') continue;
    
    $choices[filter_var($v['id'],FILTER_SANITIZE_STRING)] = filter_var($v['name'],FILTER_SANITIZE_STRING); 
  }                                
?>
...
<script>
  ...
  function use_field_to_update(obj)
  {
    ...
    $('#portlet-body-'+portlet_id).load('<?php echo url_for("ext/with_selected/update","action=get_field_values&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING ). (isset($_GET["path"]) ? "&path=" . filter_var($_GET["path"],FILTER_SANITIZE_STRING):"") . "&fields_id=") ?>'+$(obj).val(),function(response, status, xhr) {
      if (status == "error") {                                 
