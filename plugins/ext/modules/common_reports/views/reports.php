...
?>
  <tr>
    <td style="white-space: nowrap;"><?php 
      echo button_icon_delete(url_for('ext/common_reports/delete','id=' . htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING)))) . ' ' . 
           button_icon_edit(url_for('ext/common_reports/form','id=' . htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING)))) . ' ' . 
           button_icon(TEXT_COPY,'fa fa-files-o',url_for('ext/common_reports/reports','action=copy&reports_id=' . htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING))),false,['onClick'=>'return confirm("' . addslashes(TEXT_COPY_RECORD). '?")']) . ' ' .
           button_icon(TEXT_BUTTON_CONFIGURE_FILTERS,'fa fa-cogs',url_for('ext/common_reports/filters','reports_id=' . htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING))),false) . ' ' . 
           button_icon(TEXT_HEADING_REPORTS_SORTING,'fa fa-sort-alpha-asc',url_for('reports/sorting','reports_id=' . htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING)) . '&redirect_to=common_reports')); 
    ?></td>    
    <td><?php echo htmlentities(filter_var($v['id'],FILTER_SANITIZE_STRING)) ?></td>       
    <td>
      <?php
        $users_groups_list = array();
        foreach(explode(',',filter_var($v['users_groups'],FILTER_SANITIZE_STRING)) as $users_groups_id)
        {
    ...
    <td><?php echo htmlentities(filter_var($v['entities_name'],FILTER_SANITIZE_STRING)) ?></td>
    <td><?php echo htmlentities(filter_var($v['name'],FILTER_SANITIZE_STRING)) ?></td>
    <td><?php echo render_bool_value($v['in_menu'],FILTER_SANITIZE_STRING) ?></td>
    <td><?php echo render_bool_value($v['in_dashboard'],FILTER_SANITIZE_STRING) ?></td>
    <td><?php echo render_bool_value($v['in_header'],FILTER_SANITIZE_STRING) ?></td>    
