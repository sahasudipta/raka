...
class calendar
{
	...
	static function get_view_modes($reports)
	{
		...
		if($reports['view_modes']==$reports['default_view']) return '';
	...
	{
		if((int)$reports['use_background']==0) return '';
		...
		{
			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($reports['use_background']). "' and length(bg_color)>0");
		}
  ...
  public static function get_repeat_events($date_to,$calendar_type,$is_single_day)
  {
    ...
    while($events = db_fetch_array($events_query))
    {
      $start_date = filter_var($events['start_date']);
      
      //set repeat end      
      $repeat_end = false;
      if(filter_var($events['repeat_end'])>0)
      {
        $repeat_end = filter_var($events['repeat_end']);
      } 
      ...
      switch($events['repeat_type'])
      {
        case 'daily': 
            ...
            for($date = $start_date; $date<=$date_to_timestamp; $date+=86400)            
            {
              if($date>$start_date)
              {
                ...
                if($dif>0)
                {  
                  $event_obj = filter_var_array($events);                                                
        ...
        case 'weekly':  
            ...
            for($date = $start_date; $date<=$date_to_timestamp; $date+=86400)            
            {
              if($date>$start_date)
              {
                ...
                if($dif>0 and (in_array(date('N',$date),explode(',',filter_var($events['repeat_days'])))))
                {  
                  $event_obj = filter_var_array($events);                                                
                  $event_obj['start_date'] = strtotime('+' . $dif . ' day',$event_obj['start_date']);                                                          
                  $event_obj['end_date'] = strtotime('+' . $dif . ' day',$event_obj['end_date']);
        ...
        case 'yearly':                        
            ...
            if($dif>0)
            {             
              filter_var($events['start_date']) = strtotime('+' . $dif . ' year',$events['start_date']);                                                          
              filter_var($events['end_date']) = strtotime('+' . $dif . ' year',$events['end_date']);
                                      
              if(calendar::check_repeat_event_dif($dif,filter_var_array($events),$repeat_end))
              {
                $list[] = filter_var_array($events);
              }
            }                                                                                               
