...
class modules
{
	...
	{				
		...
		$values_schema = array();
		$values_query = db_query("select * from app_ext_modules_cfg where modules_id='" . filter_var($modules_id,FILTER_SANITIZE_STRING) . "'");
		while($values = db_fetch_array($values_query))
		{
			$values_schema[filter_var($values['cfg_key'],FILTER_SANITIZE_STRING)] = filter_var($values['cfg_value'],FILTER_SANITIZE_STRING);
		}
		...
		foreach($modules_configuration as $cfg)
		{
			...
			$configuration[$cfg['key']] = filter_var_array($value);
		}
