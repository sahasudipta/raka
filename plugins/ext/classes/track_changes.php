...
class track_changes
{
	...
	static function exclude_hidden_entities_query()
	{
		...
		if($app_user['group_id']>0)
		{
			...
			{
				$entities_list[] = filter_var($acess_info['entities_id'],FILTER_SANITIZE_STRING);
			}
	...
	static function render_header_menu()
	{
		...
		while($reports = db_fetch_array($reports_query))
		{
			...
			$listing_sql = "select tcl.*,tc.color_insert, tc.color_update, tc.color_comment, e.name as entity_name, c.description as comment from app_ext_track_changes_log tcl left join app_entities e on e.id=tcl.entities_id left join app_comments c on c.id=tcl.comments_id, app_ext_track_changes tc where tcl.reports_id='" . filter_var($reports['id'],FILTER_SANITIZE_STRING) . "' and tc.id=tcl.reports_id and FROM_UNIXTIME(tcl.date_added,'%Y-%m-%d')=date_format(now(),'%Y-%m-%d') " . self::exclude_hidden_entities_query() . " order by tcl.id desc";
			...
			while($item = db_fetch_array($items_query))
			{
				...
				if($count_items<$poup_items_limit)
				{
					...
					$items_list .= '
	          <li>
	  					 <a href="' . url_for('items/info','path=' . $item_info['path']['full_path']) . '">' . $item_info['name']  . ' ' . track_changes::get_item_label_by_type($item)   . '
        ...
        ';
			...
			$html .= '
				<li class="dropdown hot-reports" id="track_changes_' . filter_var($reports['id'],FILTER_SANITIZE_STRING) . '" >	
          <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
  				  <i class="fa ' . filter_var($reports['menu_icon'],FILTER_SANITIZE_STRING) . '"></i>
  				  <span class="badge badge-info">' . $count_items . '</span>
  				</a>
  				<ul class="dropdown-menu extended tasks">
  					<li style="cursor:pointer" onClick="location.href=\'' . url_for('ext/track_changes/view','reports_id=' . filter_var($reports['id'])) . '\'">
  						<p>' . filter_var($reports['name'],FILTER_SANITIZE_STRING)  . ' (' . format_date(time()) . ')</p>
  					</li>
  					<li>
	...
	static function delete_log($entities_id, $items_id)
	{
		...
		db_query("delete from app_ext_track_changes_log where entities_id='" . $entities_id . "' and items_id='" . $items_id . "'");
