...
class export_templates
{
    ...
    {
        $templates_query = db_query("select id from app_ext_export_templates where entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "'");
        while($templates = db_fetch_array($templates_query))
        {
            reports::delete_reports_by_type('export_templates' . filter_var($templates['id'],FILTER_SANITIZE_STRING));
            
            export_templates_blocks::delele_blocks_by_template_id(filter_var($templates['id'],FILTER_SANITIZE_STRING));
            
            db_query("delete from app_ext_export_templates where id='" . db_input(filter_var($templates['id'],FILTER_SANITIZE_STRING)) . "'");
        }
    }
	...
	static function get_available_fields_for_all_entities($template_entity_id, $css_class="insert_to_template_description")
	{
		...
		foreach(filter_var_array($entities_list) as $entity_id)
		{
			...
			$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in (" . fields_types::get_reserverd_types_list() . ") and f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
			
			while($fields = db_fetch_array($fields_query))
			{
				if($fields['type']=='fieldtype_dropdown_multilevel')
				{
					$html .= fieldtype_dropdown_multilevel::output_export_template(filter_var_array($fields));
				}
				else
				{
					$html .= '
				    <li>
				      <a href="#" class="' . $css_class . '">{#' . filter_var($fields['id'],FILTER_SANITIZE_STRING) .':'. strip_tags(fields_types::get_option(filter_var($fields['type'],FILTER_SANITIZE_STRING),'name',filter_var($fields['name'],FILTER_SANITIZE_STRING))) . '}</a>
				    </li>';
				}
			}
	...
	static function get_users_templates_by_position($entities_id,$position, $url_params='')
	{
		...
		{								
			...
			$button_icon = (strlen(filter_var($templates['button_icon'],FILTER_SANITIZE_STRING)) ? filter_var($templates['button_icon'],FILTER_SANITIZE_STRING):'fa-print');
			...
			{
				case 'default':
					$html .= '<li>' . button_tag($button_title,url_for('items/export_template','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&templates_id=' . htmlentities(filter_var($templates['id'],FILTER_SANITIZE_STRING))),true,array('class'=>'btn btn-primary btn-sm btn-template-' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),$button_icon). '</li>';
					$html .= self::prepare_button_css(filter_var_array($templates));
					break;
				...
				case 'menu_with_selected':
					...
					break;
				case 'menu_print':
					$html .=  '<li>' . link_to_modalbox('<i class="fa ' . $button_icon . '"></i> ' . $button_title,url_for('items/export_template','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&templates_id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),['style'=>$style]) . '</li>';
					break;
				case 'menu_with_selected_dashboard':
					$html .=  '<li>' . link_to_modalbox('<i class="fa ' . $button_icon . '"></i> ' . $button_title,url_for('items/print_template','templates_id=' . htmlentities(filter_var($templates['id'],FILTER_SANITIZE_STRING)) . $url_params),['style'=>$style]) . '</li>';
					break;
			}
	...
	static public function check_buttons_filters($buttons)
	{
		...
		$reports_info_query = db_query("select id from app_reports where entities_id='" . db_input(filter_var($buttons['entities_id'],FILTER_SANITIZE_STRING)). "' and reports_type='export_templates" . filter_var($buttons['id'],FILTER_SANITIZE_STRING) . "'");
  ...
  {  	  	  	  	
  	...
  	$count = 0;
  	$comments_query_sql = "select * from app_comments where entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' and items_id='" . filter_var($items_id,FILTER_SANITIZE_STRING) . "'  order by date_added desc " . ($limit ? ' limit ' . $limit :'');
  ...
  static function output_entities_items_list($entities_id,$items_id,$export_entity_id, $pattern)
  {
  	...
  	{	
	  	...
	  	if($reports_info = db_fetch_array($reports_info_query))
	  	{
	  		...
	  		{	
	  			$fields_list = array();
	  		  $fields_query = db_query("select f.*,if(length(f.short_name)>0,f.short_name,f.name) as name  from app_fields f where f.id in (" . filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING) . ") and  f.entities_id='" . db_input(filter_var($export_entity_id,FILTER_SANITIZE_STRING)) . "' order by field(f.id," . filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING) . ")");
	  		  while($v = db_fetch_array($fields_query))
	  		  {
	  		  	$fields_list[]=$v['id'];
	  		  }
  	...
  	if(count($fields_list))
  	{
  		...
  		{
  			$field_query = db_query("select * from app_fields where id='" . filter_var($id,FILTER_SANITIZE_STRING) . "'");
  		...
  		if($reports_id>0)
  		{  			
  			$listing_sql_query = reports::add_filters_query(filter_var($reports_id,FILTER_SANITIZE_STRING),$listing_sql_query);
  			
  			//prepare having query for formula fields
  			if(isset($sql_query_having[$export_entity_id]))
  			{
  				$listing_sql_query_having  = reports::prepare_filters_having_query($sql_query_having[filter_var($export_entity_id,FILTER_SANITIZE_STRING)]);
  			}
  			
  			if(strlen($reports_info['listing_order_fields'])>0)
  			{
  				$info = reports::add_order_query($reports_info['listing_order_fields'],filter_var($export_entity_id,FILTER_SANITIZE_STRING));
  				  			
  				$listing_sql_query .= $info['listing_sql_query'];
  				$listing_sql_query_join .= $info['listing_sql_query_join'];	  				
  			} 
  			
  			$listing_sql_query = items::add_access_query(filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING),$listing_sql_query, $reports_info['displays_assigned_only']);
  		}
  		else 
  		{
  			$listing_sql_query = items::add_access_query(filter_var($export_entity_id,FILTER_SANITIZE_STRING),$listing_sql_query);
  		}
  		  		  		    		
  		$listing_sql_query .= $listing_sql_query_having;
  		 
  		$items_info_sql = "select e.* " . fieldtype_formula::prepare_query_select(filter_var($export_entity_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($export_entity_id,FILTER_SANITIZE_STRING) . " e {$listing_sql_query_join} where e.id>0 {$listing_sql_query}";
  ...
  {      
    ...
    if(!strlen($html)) return false;
  	...
  	{  	
	      ...
	      db_query("update app_ext_export_templates set description='" . db_input(filter_var($html,FILTER_SANITIZE_STRING)) . "' where id='" . filter_var($templates_id,FILTER_SANITIZE_STRING) . "'");
  		}	
  	}
  ...
  static function get_html($entities_id, $items_id, $templates_id, $template_field = 'description')
  {
    ...
    $item_query = db_query("select e.*  " . $listing_sql_query_select . " from app_entity_" . $entities_id . " e where e.id='" . filter_var($items_id,FILTER_SANITIZE_STRING) . "'");
      ...
      if($templates = db_fetch_array($templates_query))
      {
        ...
        if(preg_match_all('/{#(\w+):[^}]*}/',$export_template,$matches))
        {
        	...
        	foreach(entities::get_parents($entities_id) as $entity_id)
        	{
        		...
        		$parent_item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select($entity_id, '') . " from app_entity_" . $entity_id . " e where e.id='" . filter_var($parent_item_id,FILTER_SANITIZE_STRING) . "'");
          ...
          foreach($matches[1] as $matches_key=>$fields_id)
          {
          	...
          	else 
          	{
	            ...
	            {            
	              ...
	              if(in_array($field['type'],array('fieldtype_mysql_query','fieldtype_formula','fieldtype_todo_list','fieldtype_users_approve','fieldtype_signature','fieldtype_textarea_wysiwyg', 'fieldtype_barcode','fieldtype_qrcode','fieldtype_text_pattern','fieldtype_text_pattern_static')))
	              {
  ...
  static function prepare_output_for_entities($entity_id, $value, $pattern)
  {
  	...
  	{
  		return self::prepare_output_table_for_entities(filter_var($entity_id,FILTER_SANITIZE_STRING), filter_var($value,FILTER_SANITIZE_STRING), $pattern);
  	}
  	  	   	
  	$output = array();
  	
  	$items_info_sql = "select e.* " . fieldtype_formula::prepare_query_select(db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)), '') . " from app_entity_" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . " e where e.id in (" . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . ")";
  	...
  	while($item = db_fetch_array($items_query))
  	{
  		...
  		{ 
  			...
  			$fields_query = db_query("select f.* from app_fields f where f.type not in ('fieldtype_action') and f.id in (" . filter_var($matches[1],FILTER_SANITIZE_STRING) . ") and  f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' order by field(f.id," . filter_var($matches[1],FILTER_SANITIZE_STRING) . ")");
  ...
  static function prepare_output_table_for_entities($entity_id, $value, $pattern)
  {
  	...
  	if(count($fields_list))
  	{	  	  	
	  	$html = '<table border="1" width="100%" class="export-table export-table-' . filter_var($entity_id,FILTER_SANITIZE_STRING) . '"><tr>';
	  	
	  	foreach(filter_var_array($fields_list) as $id)
	  	{
	  		$field_query = db_query("select * from app_fields where id='" . $id . "'");
	  		if($field = db_fetch_array($field_query))
	  		{
	  			$html .= '<th class="export-table-th-' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '" style="padding: 1px 5px; text-align: left;">' . fields_types::get_option(filter_var($field['type'],FILTER_SANITIZE_STRING),'name',filter_var($field['name'],FILTER_SANITIZE_STRING)) . '</th>';
	  		}
	  	}
	  	
	  	$html .= '</tr>';
	  		  		  		  				  
	  	$items_info_sql = "select e.* " . fieldtype_formula::prepare_query_select(filter_var($entity_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entity_id,FILTER_SANITIZE_STRING) . " e where e.id in (" . filter_var($value,FILTER_SANITIZE_STRING) . ")";
	  	$items_query = db_query($items_info_sql);
	  	while($item = db_fetch_array($items_query))
	  	{
	  		$html .= '<tr>';
