...
class xml_import
{
    ...
    function prepare_sql_data($item_key,$import_fields_queries,$item_node)
    {
        ...
        {               
            ...
            {                
                ...
                switch($filed_info['type'])
                {
                    ...
                    case 'fieldtype_entity_multilevel':
                        ...
                        if($heading_id = fields::get_heading_id($cfg->get('entity_id')))
                        {
                            ...
                            if(in_array($heading_field_info['type'],['fieldtype_input','fieldtype_input_masked','fieldtype_text_pattern_static','fieldtype_input_url']))
                            {
                                ...
                                {
                                    $item_query = db_query("select id from app_entity_" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " where field_" . filter_var($heading_id,FILTER_SANITIZE_STRING) . "='" . db_input($value_name). "'");
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            $check_query = db_query("select id from app_entity_" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING));
                    ...
                    case 'fieldtype_stages':
                        ...
                        if($cfg->get('use_global_list')>0)
                        {
                            ...
                            {
                                $fields_choices_info_query = db_query("select * from app_global_lists_choices where name='" . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . "' and lists_id='" . db_input(filter_var($cfg->get('use_global_list'),FILTER_SANITIZE_STRING)) . "'");
                    ...
                    case 'fieldtype_dropdown_multilevel':
                        ...
                        if(strlen($value))
                        {
                            ...
                            if($cfg->get('use_global_list')>0)
                            {
                                ...
                                {
                                    $fields_choices_info_query = db_query("select * from app_global_lists_choices where name='" . db_input(trim(filter_var($value,FILTER_SANITIZE_STRING))) . "' and lists_id='" . db_input(filter_var($cfg->get('use_global_list'),FILTER_SANITIZE_STRING)) . "'");
                                    if($fields_choices_info = db_fetch_array($fields_choices_info_query))
                                    {
                    ...
                    case 'fieldtype_tags':
                        ...
                        if($cfg->get('use_global_list')>0)
                        {
                            ...
                            {
                                $fields_choices_info_query = db_query("select * from app_global_lists_choices where name='" . db_input(trim(filter_var($value_name,FILTER_SANITIZE_STRING))) . "' and lists_id='" . db_input(filter_var($cfg->get('use_global_list'),FILTER_SANITIZE_STRING)) . "'");
                                if($fields_choices_info = db_fetch_array($fields_choices_info_query))
                                {
                    ...
                    case 'fieldtype_input_datetime':                        
                        $sql_data['field_' . $field_id] = strtotime($xml_field_value);                        
                        break;
                    default:
                ...
                {
                    $check_query = db_query("select id from app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." where field_".filter_var($field_id,FILTER_SANITIZE_STRING)."='" . db_input($sql_data['field_' . filter_var($field_id,FILTER_SANITIZE_STRING)]) . "' limit 1");
                    if($check = db_fetch_array($check_query))
                    {
        ...
        if($this->template_info['import_action']=='update' or $this->template_info['import_action']=='update_import')
        {
            ...
            {
                $where_sql = " where id='" . db_input(filter_var($xml_field_value,FILTER_SANITIZE_STRING)) . "'";
            }
            else
            {
                $where_sql = " where field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . "='" . db_input(filter_var($xml_field_value,FILTER_SANITIZE_STRING)) . "'";
            }
            
            $where_sql .= " and parent_item_id = '" . filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING) . "'";
            
            $item_query = db_query("select id from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . $where_sql);
        ...
        if(count($choices_values)>0 and $item_id)
        {
            ...
            foreach($choices_values as $field_id=>$values)
            {
                ...
                {
                    db_query("INSERT INTO app_entity_" . $entities_id . "_values (items_id, fields_id, value) VALUES ('" . filter_var($item_id,FILTER_SANITIZE_STRING) . "', '" . filter_var($field_id,FILTER_SANITIZE_STRING) . "', '" . filter_var($value,FILTER_SANITIZE_STRING) . "');");
                }
    ...
    static function get_users_templates_by_position($entities_id,$position, $url_params='')
    {
        ...
        while($templates = db_fetch_array($templates_query))
        {
            ...
            {
                case 'default':
                    $html .= '<li>' . button_tag($button_title,url_for('items/xml_import','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&templates_id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),true,array('class'=>'btn btn-primary btn-sm btn-template-' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),$button_icon). '</li>';
                    $html .= self::prepare_button_css(filter_var_array($templates));
                    break;
                case 'in_listing':
                    $html .= '&nbsp;&nbsp;' . button_tag($button_title,url_for('items/xml_import','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&templates_id=' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),true,array('class'=>'btn btn-primary btn-template-' . filter_var($templates['id'],FILTER_SANITIZE_STRING)),$button_icon);
                    $html .= self::prepare_button_css(filter_var_array($templates));
                    break;             
