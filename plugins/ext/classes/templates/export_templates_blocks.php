...
class export_templates_blocks
{
    ...
    {     
        ...
        if(!$item)
        {
            $item_query = db_query("select e.*  " .  fieldtype_formula::prepare_query_select($entities_id, '') . " from app_entity_" . $entities_id . " e where e.id='" . filter_var($items_id,FILTER_SANITIZE_STRING) . "'");
    ...
    function prepare_template_entity_table_list_blocks($parent_block_id,$entity_id, $items_id_list,$parent_block_settings)
    {
        ...
        {
            $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select(filter_var($entity_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entity_id,FILTER_SANITIZE_STRING) . " e where e.id in (" . filter_var($items_id_list,FILTER_SANITIZE_STRING) . ")");
    ...
    function prepare_template_related_records($blocks,$field, $item)
    {
        ...
        $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select(filter_var($entity_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entity_id,FILTER_SANITIZE_STRING) . " e where e.id in (" . implode(',', filter_var_array($related_items)) . ")",false);
    ...
    function prepare_template_sub_entity_blocks($block_id,$entity_id, $block_settings)
    {
        ...
        {            
            $reports_query = db_query("select id, listing_order_fields from app_reports where id='" . db_input(filter_var($block_settings->get('reports_id'),FILTER_SANITIZE_STRING)) . "' and entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "'");
            if($reports = db_fetch_array($reports_query))
            {                
                $listing_sql_query = reports::add_filters_query(filter_var($reports['id'],FILTER_SANITIZE_STRING),$listing_sql_query,'e');
                
                //prepare having query for formula fields
                if(isset($sql_query_having[$entity_id]))
                {
                    $listing_sql_query .= reports::prepare_filters_having_query($sql_query_having[filter_var($entity_id,FILTER_SANITIZE_STRING)]);
                }
        ...
        $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select(filter_var($entity_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entity_id,FILTER_SANITIZE_STRING) . " e " . $listing_sql_query_join . $listing_sql_query_from . " where e.parent_item_id='" . db_input($this->items_list[filter_var($this->template_info['entities_id'],FILTER_SANITIZE_STRING)]['id']) . "' " . $listing_sql_query,false);
    ...
    function prepare_template_entity_table_blocks($parent_block_id,$entity_id, $items_id_list,$parent_block_settings)
    {
        ...
        if(strlen($items_id_list))
        {
            $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select(db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)), '') . " from app_entity_" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . " e where e.id in (" . db_input(filter_var($items_id_list,FILTER_SANITIZE_STRING)) . ")");
            while($item = db_fetch_array($item_query))
            {
                $output[] = $item;
            }
    ...
    function prepare_extra_rows($wordTable,$parent_block_id,$block_type,$header_bg_color = '',$parent_block_settings)
    {
        ...
        $rows_query = db_query("select b.* from app_ext_items_export_templates_blocks b where b.block_type='" . filter_var($block_type,FILTER_SANITIZE_STRING) . "' and b.templates_id = " . filter_var($this->template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id = " . filter_var($parent_block_id,FILTER_SANITIZE_STRING) . " order by b.sort_order, b.id");                        
        while($rows = db_fetch_array($rows_query))
        {                                              
            $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where b.templates_id = " . filter_var($this->template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id = " . filter_var($rows['id'],FILTER_SANITIZE_STRING) . " order by b.sort_order, b.id");
    ...
    function prepare_template_table_blocks($parent_block_id,$entity_id, $output,$parent_block_settings)
    {
        ...
        $blocks_query = db_query("select b.*, f.name, f.entities_id, f.type as field_type,f.configuration as field_configuration from app_ext_items_export_templates_blocks b, app_fields f, app_entities e where  block_type='body_cell' and b.fields_id=f.id and b.templates_id = " . filter_var($this->template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id = " . filter_var($parent_block_id,FILTER_SANITIZE_STRING) . " and f.entities_id=e.id order by b.sort_order, b.id",false);
        ...
        if($has_totals)
        {
            ...
            $blocks_query = db_query("select b.*, f.name, f.entities_id, f.type as field_type,f.configuration as field_configuration from app_ext_items_export_templates_blocks b, app_fields f, app_entities e where  block_type='body_cell' and b.fields_id=f.id and b.templates_id = " . filter_var($this->template_info['id']) . " and b.parent_id = " . filter_var($parent_block_id,FILTER_SANITIZE_STRING) . " and f.entities_id=e.id order by b.sort_order, b.id",false);
            while($blocks = db_fetch_array($blocks_query))
            {      
                $settings = new settings($blocks['settings']);
    ...
    function prepare_template_table_list_blocks($parent_block_id,$entity_id, $output,$parent_block_settings)
    {
        ...
        $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where  block_type='table_list_cell' and b.templates_id = " . filter_var($this->template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id = " . filter_var($parent_block_id,FILTER_SANITIZE_STRING) . " order by b.sort_order, b.id",false);
        ...
        foreach($output as $item_count=>$item)
        {
            ...
            $blocks_query = db_query("select b.* from app_ext_items_export_templates_blocks b where  block_type='table_list_cell' and b.templates_id = " . filter_var($this->template_info['id']) . " and b.parent_id = " . filter_var($parent_block_id ,FILTER_SANITIZE_STRING). " order by b.sort_order, b.id",false);
            while($blocks = db_fetch_array($blocks_query))
            {                               
                $settings = new settings($blocks['settings']);  
    ...
    {   
        ...
        if(strlen($item_id))
        {
            $item_query = db_query("select e.*  " . fieldtype_formula::prepare_query_select($entity_id, '') . " from app_entity_" . $entity_id . " e where e.id='" . filter_var($item_id ,FILTER_SANITIZE_STRING). "'");
            $item = db_fetch_array($item_query);
        }
                
        
        $blocks_query = db_query("select b.*, f.name, f.entities_id, f.type as field_type from app_ext_items_export_templates_blocks b, app_fields f, app_entities e where b.fields_id=f.id and b.templates_id = " . filter_var($this->template_info['id'],FILTER_SANITIZE_STRING) . " and b.parent_id=" . filter_var($parent_block_id ,FILTER_SANITIZE_STRING). " and f.entities_id=e.id order by e.sort_order, e.name, f.name");
        while($blocks = db_fetch_array($blocks_query))
        {
            if($item)
            {                       
    ...
    {     
        ...
        foreach($entities_list as $entity_id)
        {
            ...
            {
                $choices[$app_entities_cache[$entity_id]['name']][filter_var($fields['id'],FILTER_SANITIZE_STRING)] = fields_types::get_option(filter_var($fields['type'],FILTER_SANITIZE_STRING), 'name',filter_var($fields['name'],FILTER_SANITIZE_STRING));
            }
        }
        ...
        {
            $field_query = db_query("select id from app_fields where type='fieldtype_id' and entities_id=" . filter_var($entities['id'],FILTER_SANITIZE_STRING));
    ...
    {        
        $block_query = db_query("select id from app_ext_items_export_templates_blocks where templates_id='" . filter_var($template_id,FILTER_SANITIZE_STRING) . "' and parent_id=0");
    ...
    {
        db_query("delete from app_ext_items_export_templates_blocks where id=" . filter_var($block_id,FILTER_SANITIZE_STRING));
        
        $block_query = db_query("select id from app_ext_items_export_templates_blocks where parent_id='" . filter_var($block_id,FILTER_SANITIZE_STRING) . "'");
        while($block = db_fetch_array($block_query))
        {
            self::delele_block($block['id']);
        }
