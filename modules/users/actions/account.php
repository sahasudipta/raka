...
switch($app_module_action)
{
  ...
  case 'update':
      ...
      if(isset($_POST['fields'][9]))
	      ...
	      {
	        $check_query = db_query("select count(*) as total from app_entity_1 where field_9='" . db_input(filter_var($_POST['fields'],FILTER_SANITIZE_STRING)[9]) . "'  and id!='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "'");
      ...
      if(CFG_ALLOW_CHANGE_USERNAME==1 and isset($_POST['fields'][12]))
      {
        if(strlen($_POST['fields'][12])>0)
        {
          $check_query = db_query("select count(*) as total from app_entity_1 where field_12='" . db_input(filter_var($_POST['fields'],FILTER_SANITIZE_STRING)[12]) . "'  and id!='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "'");
          $check = db_fetch_array($check_query);
          if($check['total']>0)
          {
