...
<?php echo form_tag('rules_form', url_for('forms_fields_rules/rules','action=save&entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING) . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
<script>
	...
	{		
		...
		$('#fields_choices').load('<?php echo url_for('forms_fields_rules/rules','action=get_fields_choices&entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING) . '&id=' . filter_var($obj['id'],FILTER_SANITIZE_STRING))?>&fields_id='+fields_id, function(response, status, xhr){
