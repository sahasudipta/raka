...
<?php echo form_tag('reports_filters', url_for('records_visibility/filters','action=save&rules_id=' . filter_var($_GET['rules_id'],FILTER_SANITIZE_STRING) . '&entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING) . (isset($_GET['parent_reports_id']) ? '&parent_reports_id=' . filter_var($_GET['parent_reports_id'],FILTER_SANITIZE_STRING):'').  (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal')) ?>
...
<div class="modal-body">
  ...
  <div class="form-group">
    ...
    <div class="col-md-9">	
  	  <?php echo select_tag('fields_id',fields::get_filters_choices($reports_info['entities_id'],false,"'fieldtype_formula','fieldtype_dynamic_date'"),$obj['fields_id'],array('class'=>'form-control required','onChange'=>'load_fitlers_options(this.value)')) ?>
    </div>			
  </div>     
