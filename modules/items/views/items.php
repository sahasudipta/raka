...
<?php 
  ...
  echo input_hidden_tag('entity_items_listing_path',filter_var($_GET['path'],FILTER_SANITIZE_STRING)); 
?>
...
<div class="row">
  <div class="col-sm-5">
    <div class="entitly-listing-buttons-left">
      <?php if(users::has_access('create') and access_rules::has_add_buttons_access(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING))) echo button_tag((strlen($entity_cfg->get('insert_button'))>0 ? $entity_cfg->get('insert_button') : TEXT_ADD), url_for('items/form','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING))) . ' '; ?>
...
{	
	$with_selected_menu .= '<li>' . link_to_modalbox('<i class="fa fa-file-excel-o"></i> ' . TEXT_EXPORT,url_for('items/export','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) )) . '</li>';
}
...
$with_selected_menu .=  plugins::render_simple_menu_items('with_selected','&reports_id=' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING));
  
 
if(users::has_access('delete') and users::has_access('delete_selected') and $current_entity_id!=1)
{
	$with_selected_menu .=  '<li>' . link_to_modalbox('<i class="fa fa-trash-o"></i> ' . TEXT_BUTTON_DELETE,url_for('items/delete_selected','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) )) . '</li>';
}
...
{
	echo  link_to_modalbox('<i class="fa fa-upload"></i>',url_for('items/import','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING)), array('class'=>'btn btn-default','title'=>TEXT_IMPORT));
}
...
?>
  ...
  <div class="col-sm-5">
    <div class="entitly-listing-buttons-right1">    
      <?php echo render_listing_search_form(filter_var($entity_info['id'],FILTER_SANITIZE_STRING),$listing_container, filter_var($reports_info['id'],FILTER_SANITIZE_STRING)) ?>
