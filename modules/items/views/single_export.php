...
<?php echo form_tag('export-form', url_for('items/single_export','action=export&path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING))) ?>
...
while($tabs = db_fetch_array($tabs_query))
{
  ...
  $fields_query = db_query("select f.* from app_fields f where  f.type not in ('fieldtype_action') and f.entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "' and forms_tabs_id='" . db_input(filter_var($tabs['id'],FILTER_SANITIZE_STRING)) . "' order by f.sort_order, f.name");
...
<?php endif ?>  
  ...
  <p><?php echo TEXT_FILENAME . '<br>' . input_tag('filename',items::get_heading_field(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING)) ,array('class'=>'form-control input-medium')) ?></p>
