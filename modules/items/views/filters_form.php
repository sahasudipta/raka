...
<?php echo form_tag('reports_filters', url_for('items/filters','action=save&reports_id=' . filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING) . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') . '&path=' .filter_var($_GET['path'],FILTER_SANITIZE_STRING) ),array('class'=>'form-horizontal')) ?>
...
function load_fitlers_options(fields_id)
{
  ...
  $('#filters_options').load('<?php echo url_for("reports/filters_options")?>',{fields_id:fields_id, id:'<?php echo htmlentities($obj["id"]) ?>',path:'<?php echo htmlentities($_GET["path"])?>'},function(response, status, xhr) {
    if (status == "error") {                                 
