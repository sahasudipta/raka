...
<!-- First Column  -->
    ...
    <div class="portlet portlet-item-description">
        ...
        <div class="tools">
        	<?php
        		$help_pages = new help_pages(filter_var($current_entity_id,FILTER_SANITIZE_STRING));
...
<?php
	$access_rules = new access_rules(filter_var($current_entity_id,FILTER_SANITIZE_STRING), $item_info);
	
	$item_actions_menu = '';

  if(is_ext_installed())
  {
    $timer = new timer(filter_var($current_entity_id,FILTER_SANITIZE_STRING), filter_var($current_item_id,FILTER_SANITIZE_STRING));
    $item_actions_menu .= $timer->render_button();
  }
                                           
  if(users::has_access('update',$access_rules->get_access_schema()))
  { 
   	$item_actions_menu .= '<li>' . button_tag(TEXT_BUTTON_EDIT,url_for('items/form','id=' . filter_var($current_item_id,FILTER_SANITIZE_STRING). '&entity_id=' . filter_var($current_entity_id,FILTER_SANITIZE_STRING) . '&path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING) . '&redirect_to=items_info'),true,array('class'=>'btn btn-primary btn-sm'),'fa-edit')  .'</li>';
  }
                         
	if(is_ext_installed())
	{
		$processes = new processes(filter_var($current_entity_id,FILTER_SANITIZE_STRING));
		$processes->items_id = filter_var($current_item_id,FILTER_SANITIZE_STRING);
		$item_actions_menu .= $processes->render_buttons('default');
		
		//print templates
		$item_actions_menu .= export_templates::get_users_templates_by_position(filter_var($current_entity_id,FILTER_SANITIZE_STRING), 'default');
		$item_actions_menu .= export_templates::get_users_templates_by_position(filter_var($current_entity_id,FILTER_SANITIZE_STRING), 'menu_print');
		
		$item_actions_menu .= xml_export::get_users_templates_by_position(filter_var($current_entity_id,FILTER_SANITIZE_STRING), 'default');
		$item_actions_menu .= xml_export::get_users_templates_by_position(filter_var($current_entity_id,FILTER_SANITIZE_STRING), 'menu_export');
		
		$item_actions_menu .= xml_import::get_users_templates_by_position(filter_var($current_entity_id,FILTER_SANITIZE_STRING), 'default');
  }
...
{	
	$more_actions_menu .= '<li>' . link_to_modalbox('<i class="fa fa-file-pdf-o"></i> ' . TEXT_BUTTON_EXPORT,url_for('items/single_export','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING))) . '</li>';
}

if(users::has_access('update',$access_rules->get_access_schema()) and $current_entity_id==1)
{	
	$more_actions_menu .=  '<li>' . link_to('<i class="fa fa-unlock-alt"></i> ' .TEXT_CHANGE_PASSWORD, url_for('items/change_user_password','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING))) . '</li>';
}    
...
{	
	...
	if($check)
	{
  	$more_actions_menu .=  '<li><a href="#" onClick="open_dialog(\'' . url_for('items/delete','id=' .filter_var($current_item_id,FILTER_SANITIZE_STRING) . '&entity_id=' . filter_var($current_entity_id,FILTER_SANITIZE_STRING) . '&path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING)) . '\'); return false;"><i class="fa fa-trash-o"></i> ' . TEXT_BUTTON_DELETE . '</a></li>';
	}
}   
...
{
	if(!users::has_access_to_assigned_item(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING)))
	{
...
?>        
        ...
        <div class="item-content-box ckeditor-images-content-prepare">
          <?php 
	         ...
	         {
	         	 echo items::render_info_box(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING),false,false);
	         }	
	         else 
	         {	
          	 echo items::render_content_box(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING)); 
	         }
          ?>
        </div>
...
//include reladed records that displays as single list    
      $reladed_records = new related_records(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING));
...
//include related emails	    
	...
	//include related records in box    
	    ...
	    ?>
			...
			<?php if($entity_cfg->get('item_page_details_columns','2')==2 and strlen($info_box = items::render_info_box(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($current_item_id,FILTER_SANITIZE_STRING)))): ?>
