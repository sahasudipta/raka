...
switch($app_module_action)
{
	case 'import':
		...
		{
			$parent_item_query = db_query("select * from app_entity_" . filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING) . " where id='" . db_input($parent_item_id) . "'");
	...
	case 'bind_field':
		$col = $_POST['col'];
		$filed_id = filter_var($_POST['filed_id'],FILTER_SANITIZE_STRING);
		...
		if($filed_id>0)
		{
			...
			echo fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING));
		}
