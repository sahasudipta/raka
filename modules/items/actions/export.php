...
{ 
	case 'delete_templates':
				db_delete_row('app_items_export_templates',filter_var($_POST['id'],FILTER_SANITIZE_STRING));
			exit();
		break;
	case 'set_default_templates':
			...
			db_query("update app_items_export_templates set is_default=1 where users_id='" . db_input($app_user['id']) . "' and id='" . filter_var($_POST['id'],FILTER_SANITIZE_STRING) . "'");
			exit();
		break;
	...
	case 'update_templates_fields':
			$sql_data_array = array(			
					'templates_fields' =>filter_var($_POST['fields_list'],FILTER_SANITIZE_STRING),			
			);
			
			db_perform('app_items_export_templates', $sql_data_array,'update',"users_id='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . "' and id='" . db_input(filter_var($_POST['id'],FILTER_SANITIZE_STRING)) . "'");
	...
	case 'update_templates_name':
			...
			db_perform('app_items_export_templates', $sql_data_array,'update',"users_id='" . db_input($app_user['id']) . "' and id='" . db_input($_POST['id'] ). "'");
				
			exit();
			break;
	case 'save_templates':
				$templates_name = db_prepare_input(filter_var($_POST['templates_name'],FILTER_SANITIZE_STRING));
	...
	case 'get_templates_button':
			...
			if($check['total']>0)
			{
				...
				{
					$html_list .= '<li><a href="javascript: use_items_export_template(\'' . filter_var($templates['templates_fields'],FILTER_SANITIZE_STRING) . '\',' . filter_var($templates['id'],FILTER_SANITIZE_STRING) . ',\'' . addslashes($templates['name']). '\')">' . filter_var($templates['name'],FILTER_SANITIZE_STRING) . '</a></li>';					
				}
			}
			else 
			{
	...
	case 'get_templates':
		...
		$html = '
			...
			if($check['total']>0)
			{
				...
				{
					$html .= '
						<tr class="templates-row-' . filter_var($templates['id'],FILTER_SANITIZE_STRING) . '">
							<td style="padding-right: 15px;">' . input_tag('export_template_name[]',filter_var($templates['name'],FILTER_SANITIZE_STRING),array('class'=>'form-control','onKeyUp'=>'update_items_export_templates_name(' . filter_var($templates['id'],FILTER_SANITIZE_STRING) . ',this.value)')) . '</td>							
							<td align="center">' . input_radiobox_tag('is_default_template',1, array('checked'=>filter_var($templates['is_default'],FILTER_SANITIZE_STRING),'data-id'=>filter_var($templates['id'],FILTER_SANITIZE_STRING))) . '</td>
							<td><button onClick="delete_items_export_templates(' . filter_var($templates['id'],FILTER_SANITIZE_STRING) . ')" class="btn btn-default" type="button"><i class="fa fa-trash-o" title="' . addslashes(TEXT_BUTTON_DELETE). '"></i></button></td>
						</tr>
					';
				}
			}
	...
	case 'print':
		...
		if(count($app_selected_items[$_POST['reports_id']])>0 and isset($_POST['fields']))
		{
			...
			$selected_items = implode(',',$app_selected_items[filter_var($_POST['reports_id'],FILTER_SANITIZE_STRING)]);
		
			//prepare forumulas query
			$listing_sql_query_select = fieldtype_formula::prepare_query_select($current_entity_id, '',false,array('fields_in_listing'=>implode(',',filter_var($_POST['fields'],FILTER_SANITIZE_STRING))));
			...
			while($item = db_fetch_array($items_query))
			{
				...
				foreach($listing_fields as $field)
				{
					...
					$output_options = array('class'=>$field['type'],
							...
							'is_export'  => true,
							'is_print'  => true,
							'reports_id'=> filter_var($_POST['reports_id'],FILTER_SANITIZE_STRING),
  ...
  case 'export':
      ...
      {    
        ...
        //adding reserved fields               
        $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in ('fieldtype_action') and f.id in (" . implode(',',filter_var_array($_POST['fields'])). ") and f.entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
        while($fields = db_fetch_array($fields_query))
        {
        	if($fields['type']=='fieldtype_dropdown_multilevel')
        	{
        ...
        //prepare forumulas query
        $listing_sql_query_select = fieldtype_formula::prepare_query_select(filter_var($current_entity_id,FILTER_SANITIZE_STRING), '',false,array('fields_in_listing'=>implode(',',filter_var_array($_POST['fields']))));
  ...
  case 'export_csv':
  	...
  	if(count($app_selected_items[$_POST['reports_id']])>0 and isset($_POST['fields']))
  	{
  		...
  		$filename = str_replace(' ','_',trim(filter_var($_POST['filename'],FILTER_SANITIZE_STRING)));
  		...
  		if($file_extension=='csv')
  		{
  			echo htmlentities(chr( 0xFF ) . chr( 0xFE ) . mb_convert_encoding( $content, 'UTF-16LE', 'UTF-8' ));
  		}
  		else
  		{
  		...
  		//prepare forumulas query
  		$listing_sql_query_select = fieldtype_formula::prepare_query_select($current_entity_id, '',false,array('fields_in_listing'=>implode(',',filter_var($_POST['fields']))));
