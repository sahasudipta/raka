  ...
  case 'export':
    ...
    if(isset($_POST['fields']))
    {
      ...
      if(users::has_comments_access('view') and $entity_cfg['use_comments']==1 and isset($_POST['export_comments']))
      {
        ...
        while($item = db_fetch_array($items_query))
        {
          ...
          while($field = db_fetch_array($comments_fields_query))
          {
            ...
            $html_fields .='                      
                <tr><th>&bull;&nbsp;' . filter_var($field['name'],FILTER_SANITIZE_STRING) . ':&nbsp;</th><td>' . trim(strip_tags(fields_types::output($output_options))). '</td></tr>           
            ';
          }
          ...
          $attachments = fields_types::output(array('class'=>'fieldtype_attachments','is_export'=>true,'value'=>filter_var($item['attachments'],FILTER_SANITIZE_STRING),'path'=>$current_path,'field'=>array('entities_id'=>filter_var($current_entity_id,FILTER_SANITIZE_STRING)),'item'=>array('id'=>filter_var($current_item_id,FILTER_SANITIZE_STRING))));
          ...
          if($entity_cfg['use_editor_in_comments']!=1)
          {
           ...
           $html_comments .= '
            <tr>                  
              ...
              <td class="nowrap" valign="top">' . date(CFG_APP_DATETIME_FORMAT,filter_var($item['date_added'],FILTER_SANITIZE_STRING)) . '<br>' . $app_users_cache[filter_var($item['created_by'],FILTER_SANITIZE_STRING)]['name']. '<br>' . '</td>
            </tr>
            <tr>
              <td colspan="2"><hr></td>
            </tr>
      ...
      */
      ...
      foreach(filter_var_array($export) as $tab=>$fields)
      {
        $html .= '
          <tr>
        ...
        foreach(filter_var_array($fields) as $v)
        $html .= '
          <tr>
