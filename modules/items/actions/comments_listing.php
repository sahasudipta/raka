...
$html = '';
$listing_sql = "select * from app_comments where entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "' and items_id='" . db_input(filter_var($current_item_id,FILTER_SANITIZE_STRING)) . "' " . $listing_sql_query . " order by id desc";
...
while($item = db_fetch_array($items_query))
{
  ...
  if($user_has_comments_access)
  {
    $html_action_column = '
      <td class="nowrap">
      ' . ((users::has_comments_access('delete', $access_rules->get_comments_access_schema()) and ($item['created_by']==filter_var($app_user['id'],FILTER_SANITIZE_STRING) or filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)==0)) ? button_icon_delete(url_for('items/comments_delete','id=' .filter_var($item['id'],FILTER_SANITIZE_STRING) . '&path=' . filter_var($_POST['path'],FILTER_SANITIZE_STRING))) . '<br>':'') . '
      ' . ((users::has_comments_access('update', $access_rules->get_comments_access_schema()) and ($item['created_by']==filter_var($app_user['id'],FILTER_SANITIZE_STRING) or $app_user['group_id']==0)) ? button_icon_edit(url_for('items/comments_form','id=' .filter_var($item['id'],FILTER_SANITIZE_STRING) . '&path=' . filter_var($_POST['path'],FILTER_SANITIZE_STRING))) . '<br>':'') . '
			' . (users::has_comments_access('create', $access_rules->get_comments_access_schema()) ?  button_icon(TEXT_REPLY,'fa fa-reply',url_for('items/comments_form','reply_to=' .filter_var($item['id'],FILTER_SANITIZE_STRING) . '&path=' . filter_var($_POST['path'],FILTER_SANITIZE_STRING))):'') . '
      </td>
    ';
  }
  ...
  if($entity_cfg->get('use_editor_in_comments')!=1)
  {
   ...
   $html .= '
    <tr>
      ' . $html_action_column . ' 
      ' . ($entity_cfg->get('display_comments_id')==1 ? '<td>' . filter_var($item['id'],FILTER_SANITIZE_STRING) . '</td>':''). '   
      <td style="white-space: normal;">
        			<div class="ckeditor-images-content-prepare"><div class="fieldtype_textarea_wysiwyg">' . auto_link_text(filter_var($item['description'],FILTER_SANITIZE_STRING))  . '</div></div>' . 
        			$attachments . 
        			$html_fields .        			
      '</td>
      <td class="nowrap">' . format_date_time($item['date_added']) . ($item['created_by']>0 ? '<br><span ' . users::render_publi_profile($app_users_cache[filter_var($item['created_by'],FILTER_SANITIZE_STRING)],true). '>' . $app_users_cache[filter_var($item['created_by'],FILTER_SANITIZE_STRING)]['name']. '</span><br>' . render_user_photo($app_users_cache[filter_var($item['created_by'],FILTER_SANITIZE_STRING)]['photo']) : '') . '</td>
    </tr>
  '; 
}
...
$html .= '
  <table width="100%">
    <tr>
      <td>' . htmlentities($listing_split->display_count()) . '</td>
