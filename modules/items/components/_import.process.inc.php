        ...
        if($filed_info = db_fetch_array($filed_info_query))
        {
            ...
            switch($filed_info['type'])
            {
                ...
                case 'fieldtype_entity_multilevel':
                    ...
                    if($heading_id = fields::get_heading_id(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING)))
                    {
                        $heading_field_info = db_find('app_fields',filter_var($heading_id,FILTER_SANITIZE_STRING));
                        if(in_array($heading_field_info['type'],['fieldtype_input','fieldtype_input_masked','fieldtype_text_pattern_static','fieldtype_input_url']))
                        {
                            ...
                            {
                                $item_query = db_query("select id from app_entity_" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " where field_" . $heading_id . "='" . db_input(filter_var($value_name,FILTER_SANITIZE_STRING)). "'");
                                if($item = db_fetch_array($item_query))
                                {
                                    $values_list[] =  filter_var($item['id'],FILTER_SANITIZE_STRING);
                                }
                                else
                                {
...
{    
    ...
    $check_query = db_query("select count(*) as total from app_entity_1 where field_12='" . db_input(filter_var($sql_data['field_12'],FILTER_SANITIZE_STRING)) . "'");
...
{
    $field_info = db_find('app_fields',filter_var($_POST['update_by_field'],FILTER_SANITIZE_STRING));
    ...
    {
        $where_sql = " where field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . "='" . db_input($use_column_value) . "'";
    }
    
    $where_sql .= " and parent_item_id = '" . filter_var($import_entity_parent_item_id,FILTER_SANITIZE_STRING) . "'";
