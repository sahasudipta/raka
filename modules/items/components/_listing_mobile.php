...
while($item = db_fetch_array($items_query))
{
	...
	foreach(filter_var_array($listing_type['sections']) as $section)
	{
		...
		foreach(filter_var_array($section['fields']) as $field)
		{
			//check field access
			...
			$output_options = array(
					...
					'reports_id'  => (filter_var($reports_entities_id,FILTER_SANITIZE_STRING)>0 ? filter_var($_POST['reports_id'],FILTER_SANITIZE_STRING):0),
			...
			if($field['is_heading']==1)
			{
				...
				{
					$value .= comments::get_last_comment_info(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($item['id'],FILTER_SANITIZE_STRING),filter_var($path,FILTER_SANITIZE_STRING), $fields_access_schema);
				}
		...
		if($section['display_as']=='list')
		{
			...
			foreach(filter_var_array($section_fields) as $field)
			{
				$html .= '
						<tr>
