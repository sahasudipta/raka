    ...
    <div class="entitly-listing-buttons-left">
      <?php 
      	if(users::has_comments_access('create', $access_rules->get_comments_access_schema()))
      	{
      		echo '
    				<div class="btn-group">' . 
      				button_tag(TEXT_BUTTON_ADD_COMMENT, url_for('items/comments_form','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING)),true,array('class'=>'btn btn-primary btn-sm')) . 
...
<script>
  ...
  {      
    ...
    $('#'+listing_container).load('<?php echo url_for("items/comments_listing")?>',{path:'<?php echo filter_var($_GET["path"],FILTER_SANITIZE_STRING)?>',page:page,search_keywords:$('#'+listing_container+'_search_keywords').val()},
