...
{
  $current_path = filter_var($_POST['path'],FILTER_SANITIZE_STRING);
}
...
if($current_item_id>0)
{
  ...
  $item_query = db_query("select e.id from app_entity_" . filter_var($current_entity_id,FILTER_SANITIZE_STRING) . " e where e.id='" . db_input(filter_var($current_item_id,FILTER_SANITIZE_STRING)) . "' " . records_visibility::add_access_query(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . " " . items::add_access_query_for_parent_entities(filter_var($current_entity_id,FILTER_SANITIZE_STRING)),false);
...
//check path to entity  
  if($current_item_id==0)
  {
    ...
    $item_query = db_query("select e.id from app_entity_" . filter_var($parent_entity_id,FILTER_SANITIZE_STRING) . " e where e.id='" . db_input(filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING)) . "' "  . items::add_access_query(filter_var($parent_entity_id,FILTER_SANITIZE_STRING),'') . ' '. items::add_access_query_for_parent_entities(filter_var($parent_entity_id,FILTER_SANITIZE_STRING)));
