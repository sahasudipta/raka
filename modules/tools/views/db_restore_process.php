...
switch($app_module_action)
{
	case 'restore_by_id':
		$html .= '
			<script>
				$(function(){
					$("#db_restore_process").load("' . url_for('tools/db_restore_process','action=restore&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING)) . '")
				})				
			</script>
		';
		break;
	...
	$is_file = false;
	if(strlen($filename = filter_var($_FILES['filename']['name'],FILTER_SANITIZE_STRING))>0)
	{
