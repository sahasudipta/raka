...
?>
  <tr>
    <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('users_alerts/delete','id=' . filter_var($alets['id'],FILTER_SANITIZE_STRING))) . ' ' . button_icon_edit(url_for('users_alerts/form','id=' . filter_var($alets['id'],FILTER_SANITIZE_STRING))); ?></td>        
    <td><?php echo '<span class="label label-' . filter_var($alets['type'],FILTER_SANITIZE_STRING) . '">' . users_alerts::get_type_by_name(filter_var($alets['type'],FILTER_SANITIZE_STRING)) . '</span>' ?></td>
    <td><?php echo htmlentities($alets['title']) ?></td>
