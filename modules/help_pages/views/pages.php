...
?>
  <tr>
    <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('help_pages/delete','id=' . filter_var($pages['id'],FILTER_SANITIZE_STRING) . '&entities_id=' .  filter_var(_get::int('entities_id'),FILTER_SANITIZE_STRING))) . ' ' . button_icon_edit(url_for('help_pages/form','id=' . filter_var($pages['id'],FILTER_SANITIZE_STRING) . '&type=page'. '&entities_id=' .  filter_var(_get::int('entities_id'),FILTER_SANITIZE_STRING))); ?></td>        
    <td><?php echo '<span class="label label-' . filter_var($pages['color'],FILTER_SANITIZE_STRING) . '">' . help_pages::get_color_by_name(filter_var($pages['color'],FILTER_SANITIZE_STRING)) . '</span>' ?></td>
    <td>
    	<?php echo htmlentities($pages['name']) ?>
    	<div><i><small><?php echo (strlen($pages['description'])>64 ? filter_var(substr(strip_tags($pages['description']),0,64),FILTER_SANITIZE_STRING) . '...' : filter_var($pages['description'],FILTER_SANITIZE_STRING) )?></small></i></div>
