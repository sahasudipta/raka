...
$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($entities_info['parent_id'],FILTER_SANITIZE_STRING)). "' and reports_type='{$reports_type}'");
if(!$reports_info = db_fetch_array($reports_info_query))
{
  $sql_data = array('name'=>'',
                    'entities_id'=>filter_var($entities_info['parent_id'],FILTER_SANITIZE_STRING),
...
switch($app_module_action)
{
  case 'save':
    ...
    redirect_to('entities/hide_subentity_filters','reports_id=' . filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING) . '&entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING));      
  break;
  case 'delete':
      if(isset($_GET['id']))
      {      
