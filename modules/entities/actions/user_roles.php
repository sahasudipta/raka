...
switch($app_module_action)
{
	case 'save':
		...
		if(isset($_GET['id']))
		{			 
			db_perform('app_user_roles',$sql_data,'update',"id='" . db_input($filter_var(_GET['id'],FILTER_SANITIZE_STRING)) . "'");			
		}
		else
		{
	...
	case 'delete':
		...
		{		
			...
			{
				db_query("delete from app_user_roles where id='" . db_input(filter_var($info['id'],FILTER_SANITIZE_STRING)) . "'");
				db_query("delete from app_user_roles_access where user_roles_id='" . db_input(filter_var($info['id'],FILTER_SANITIZE_STRING)) . "'");
			}				
