...
<?php echo form_tag('fields_form', url_for('entities/fields','action=save' . (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):'') ),array('class'=>'form-horizontal','enctype'=>'multipart/form-data')) ?>
<div class="modal-body">
  <div class="form-body ajax-modal-width-790">
     
<?php echo input_hidden_tag('entities_id',filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING)) ?>
<?php if(isset($_GET['redirect_to']))echo input_hidden_tag('redirect_to',filter_var($_GET['redirect_to'],FILTER_SANITIZE_STRING)) ?>

<?php
$forms_tabs_choices = forms_tabs::get_choices(filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING));
...
{
    $entities_access_schema = users::get_entities_access_schema(filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING),$groups['id']);
    
    if(!in_array('view',$entities_access_schema) and  !in_array('view_assigned',$entities_access_schema) and filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING)!=1) continue;
            
    $access_schema = users::get_fields_access_schema(filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING),$groups['id']);
    
    $value = (isset($access_schema[$obj['id']]) ? $access_schema[filter_var($obj['id'],FILTER_SANITIZE_STRING)] : 'yes');
    
?>  	
  	
	  <div class="form-group">
	  	<label class="col-md-3 control-label" for="name"><?php echo htmlentities($groups['name']) ?></label>
	    <div class="col-md-9">	
	  	  <?php echo select_tag('access[' . filter_var($groups['id'],FILTER_SANITIZE_STRING). ']',filter_var($access_choices,FILTER_SANITIZE_STRING), $value,array('class'=>'form-control input-medium acess-group-cfg access_group_' . filter_var($groups['id'],FILTER_SANITIZE_STRING))) ?>
	    </div>			
	  </div> 
<?php 
  } 
...
{ 
  ...
  $('#fields_types_configuration').load('<?php echo url_for("entities/fields_configuration")?>',{field_type:field_type, id:'<?php echo htmlentities($obj["id"]) ?>',entities_id:'<?php echo htmlentities($_GET["entities_id"])?>'},function(response, status, xhr) {
    if (status == "error") {                                 
...
{   		
  ...
  $('#fields_types_ajax_configuration_'+name).load('<?php echo url_for("entities/fields_ajax_configuration")?>',{name: name, value: value, field_type:field_type, id:'<?php echo htmlentitites($obj["id"] )?>',entities_id:'<?php echo htmlentities($_GET["entities_id"])?>'},function(response, status, xhr) {
    if (status == "error") {                                 
       $(this).html('<div class="alert alert-error"><b>Error:</b> ' + xhr.status + ' ' + xhr.statusText+'<div>'+response +'</div></div>')                    
    }
    else
...
function check_is_heading_option()
{
   ...
   if($.inArray(selected_type,["fieldtype_ajax_request","fieldtype_digital_signature","fieldtype_time","fieldtype_iframe","fieldtype_input_protected","fieldtype_google_map","fieldtype_mapbbcode","fieldtype_todo_list","fieldtype_mysql_query","fieldtype_image_map","fieldtype_mind_map","fieldtype_section","fieldtype_input_numeric_comments","fieldtype_input_url","fieldtype_attachments","fieldtype_input_file","fieldtype_image","fieldtype_textarea_wysiwyg","fieldtype_formula","fieldtype_related_records","fieldtype_boolean","fieldtype_boolean_checkbox"])==-1)
   {
