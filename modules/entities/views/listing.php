...
<h3 class="page-title"><?php echo link_to(TEXT_NAV_LISTING_CONFIG, url_for('entities/listing_types','entities_id=' . _get::int('entities_id'))) ?> <i class="fa fa-angle-right"></i> <?php echo TEXT_TABLE ?></h3>
...
<div class="row">
  <div class="col-md-8">
    ...
    <div class="cfg_listing">        
      ...
      <?php
      $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.listing_status = 1 " . $fields_sql_query . " and  f.entities_id='" . db_input(filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by f.listing_sort_order");
      while($v = db_fetch_array($fields_query))
      {
        echo '<li id="form_fields_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING)) . '</div></li>';
      }
      ?> 
      </ul>         
    </div>
    ...
    <?php
    $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.listing_status = 0 " . $fields_sql_query . " and  f.entities_id='" . db_input(filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id and f.type not in ('fieldtype_mapbbcode') order by t.sort_order, t.name, f.sort_order, f.name");
    while($v = db_fetch_array($fields_query))
    {
      echo '<li id="form_fields_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING)). '</div></li>';
    }
...
<legend><?php echo tooltip_icon(TEXT_IS_HEADING_INFO) . TEXT_SELECT_HEADING_FIELD ?></legend>
<div class="row">
  ...
  <div class="col-md-9">
  	<form class="form-inline" role="form">
			...
			<form class="form-inline" role="form" style="padding-top: 5px;">
				<div class="form-group">
					<label for="number_fixed_field_in_listing"><?php echo TEXT_CHANGE_COL_WIDTH_IN_LISTIN ?></label>
					<?php echo select_tag('change_col_width_in_listing',array('1'=>TEXT_YES,'0'=>TEXT_NO),(int)$cfg->get('change_col_width_in_listing'),array('class'=>'form-control input-small')) ?>
				</div>
			</form>   	
  </div>
</div>
...
<script>
  $(function() {         
    	$( "ul.sortable" ).sortable({
          ...
          data = data.slice(1)                      
          $.ajax({type: "POST",url: '<?php echo url_for("entities/fields","action=sort_fields&entities_id=" . filter_var($_GET["entities_id"],FILTER_SANITIZE_STRING))?>',data: data});
        }
    	});
        ...
        $.ajax({type: "POST",
                url: '<?php echo url_for("entities/fields","action=set_heading_field_width&entities_id=" . filter_var($_GET["entities_id"],FILTER_SANITIZE_STRING))?>',
     ...
     $("#change_col_width_in_listing").change(function(){
        $.ajax({type: "POST",
                url: '<?php echo url_for("entities/fields","action=set_change_col_width_in_listing&entities_id=" . filter_var($_GET["entities_id"],FILTER_SANITIZE_STRING))?>',
      ...
      $("#number_fixed_field_in_listing").keyup(function(){
         $.ajax({type: "POST",
                 url: '<?php echo url_for("entities/fields","action=set_number_fixed_field_in_listing&entities_id=" . filter_var($_GET["entities_id"],FILTER_SANITIZE_STRING))?>',
      ...
      $("#heading_field_id").change(function(){
         $.ajax({type: "POST",
                 url: '<?php echo url_for("entities/fields","action=set_heading_field_id&entities_id=" . filter_var($_GET["entities_id"],FILTER_SANITIZE_STRING))?>',
