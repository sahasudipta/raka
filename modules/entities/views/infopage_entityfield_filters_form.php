...
<?php echo form_tag('reports_filters', url_for('entities/infopage_entityfield_filters','action=save&reports_id=' . filter_var($_GET['reports_id'] ,FILTER_SANITIZE_STRING). (isset($_GET['id']) ? '&id=' . filter_var($_GET['id'],FILTER_SANITIZE_STRING):''). '&entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING). '&related_entities_id=' . filter_var($_GET['related_entities_id'],FILTER_SANITIZE_STRING). '&fields_id=' . filter_var($_GET['fields_id'],FILTER_SANITIZE_STRING)),array('class'=>'form-horizontal')) ?>
