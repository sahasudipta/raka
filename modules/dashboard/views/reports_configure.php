...
<div class="cfg_listing">        
  ...
  <?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.id in (" . $reports_list . ") order by field(r.id,{$reports_list})");
...
<div class="cfg_listing">        
  ...
  <?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.id in (" . $counters_list . ") order by field(r.id,{$counters_list})");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
  }
...
<?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.id not in (" . $counters_list . ") order by e.name, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
  }
