...
<div class="cfg_listing">        
  ...
  <?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.in_dashboard=1 order by r.dashboard_sort_order, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
  }
  ?> 
  </ul>         
</div>
...
<?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.in_dashboard!=1 order by e.name, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
  }
...
<?php
  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.in_dashboard_counter!=1 order by e.name, r.name");
  while($v = db_fetch_array($reports_query))
  {
    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
  }
...
<div class="tab-pane" id="form_tab_reports_header">
	...
	<div class="cfg_listing">        
	  ...
	  <?php
	  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.in_header=1 order by r.header_sort_order, r.name");
	  while($v = db_fetch_array($reports_query))
	  {
	    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
	  }
	...
	<?php
	  $reports_query = db_query("select r.*,e.name as entities_name from app_reports r, app_entities e where e.id=r.entities_id and r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and r.in_header!=1 order by e.name, r.name");
	  while($v = db_fetch_array($reports_query))
	  {
	    echo '<li id="report_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"><div>' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</div></li>';
	  }
