...
switch($app_module_action)
{
  ...
  case 'sort_reports':
        ...
        {
          $sort_order = 0;
          foreach(explode(',',filter_var($_POST['reports_on_dashboard'],FILTER_SANITIZE_STRING)) as $v)
          {
            $sql_data = array('in_dashboard'=>1,'dashboard_sort_order'=>$sort_order);
            db_perform('app_reports',$sql_data,'update',"id='" . db_input(str_replace('report_','',$v)) . "' and created_by='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)). "'");
            $sort_order++;
          }
        }
        ...
        {          
          foreach(explode(',',filter_var($_POST['reports_excluded_from_dashboard'],FILTER_SANITIZE_STRING)) as $v)
          {
  ...
  case 'sort_reports_countr':
    	...
    	{
    		$sort_order = 0;
    		foreach(explode(',',filter_var($_POST['reports_counter_on_dashboard'],FILTER_SANITIZE_STRING)) as $v)
    		{
    	...
    	{
    		foreach(explode(',',filter_var($_POST['reports_counter_excluded_from_dashboard'],FILTER_SANITIZE_STRING)) as $v)
    		{
   ...
   case 'sort_reports_header':
    		...
    		{
    			$sort_order = 0;
    			foreach(explode(',',filter_var($_POST['reports_in_header'],FILTER_SANITIZE_STRING)) as $v)
    			{
    		...
    		{
    			foreach(explode(',',filter_var($_POST['reports_excluded_in_header'],FILTER_SANITIZE_STRING)) as $v)
    			{
   ...
   case 'update_hot_reports':
          ...
          echo $hot_reports->render_dropdown(filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING)); 
