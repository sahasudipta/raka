...
foreach($app_fields_cache[$field['entities_id']] as $fiels_id=>$fields_data)
{        
    if(!in_array($fields_data['type'],fields_types::get_reserved_types()))
    {
        $fields_values[$fiels_id] = (isset($_POST['fields'][$fiels_id]) ? filter_var($_POST['fields'][$fiels_id],FILTER_SANITIZE_STRING) : 0);
        
        //prepare date value
        if(in_array($fields_data['type'],['fieldtype_input_date','fieldtype_input_datetime']))
        {
            $fields_values[$fiels_id] = (int)get_date_timestamp($fields_values[$fiels_id]);
        }
    }
}
...
if($app_entities_cache[$field['entities_id']]['parent_id']>0 and isset($_POST['parent_item_id']) and $_POST['parent_item_id']>0)
{
    ...
    {        
        foreach(filter_var_array($parent_item) as $fields_id=>$fields_value)
        {
