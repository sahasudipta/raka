...
switch($app_module_action)
{
	case 'select_items':
		...
		{
			$listing_sql_query = $listing_sql_query . fieldtype_entity::prepare_parents_sql(filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING),filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING),filter_var($field_entity_info['parent_id'],FILTER_SANITIZE_STRING));
		}
		...
		else
		{
			...
			$listing_sql_query .= records_visibility::add_access_query(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING));
		}
		
		$listing_sql_query .= fieldtype_entity_ajax::mysql_query_where($cfg,filter_var_array($field),filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING));
		...
		{							
			...
			$info = reports::add_order_query($default_reports['listing_order_fields'],filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING));
		...
		{	
			...
			{					
				$listing_sql_select = fieldtype_formula::prepare_query_select(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING), '',false,array('fields_in_listing'=>filter_var(implode(',',$matches[1])),FILTER_SANITIZE_STRING));
			}
		}
		
		$results = [];
		$listing_sql = "select  e.* " . $listing_sql_select . " from app_entity_" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " e "  . $listing_sql_query_join . " where " . $listing_sql_query . $listing_sql_query_having . $listing_sql_query_order;
