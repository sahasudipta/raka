...
<h3 class="page-title"><?php echo TEXT_HEADING_FILTERS_FOR_REPORT  . ' ' . link_to($page_title,url_for('reports/view','reports_id=' . filter_var($current_reports_info['id'],FILTER_SANITIZE_STRING))) ?></h3>
...
<div class="panel panel-default">
  <div class="panel-heading"><?php echo TEXT_FILTERS_FOR_ENTITY . ': <b>' . filter_var($entity_info['name'],FILTER_SANITIZE_STRING) . '</b>' ?></div>
    ...
    ?>
      <tr>
        <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('reports/filters_delete','id=' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($current_reports_info['id'],FILTER_SANITIZE_STRING) . $parent_reports_param)) . ' ' . button_icon_edit(url_for('reports/filters_form','id=' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($current_reports_info['id'],FILTER_SANITIZE_STRING) . $parent_reports_param))  ?></td>    
