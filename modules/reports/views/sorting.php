...
<?php
  ...
  elseif($app_redirect_to=='entityfield_filters')
  {
  	...
  	echo form_tag('sorting_form', url_for('entities/entityfield_filters','entities_id=' . filter_var($fields_info['entities_id'],FILTER_SANITIZE_STRING) . '&fields_id=' . filter_var($fields_id,FILTER_SANITIZE_STRING)));
  }
  elseif($app_redirect_to == 'related_records_field_settings')
  {
  	echo form_tag('sorting_form', url_for('entities/fields_settings','entities_id=' . filter_var($_GET['entities_id'],FILTER_SANITIZE_STRING) . '&fields_id=' . filter_var($_GET['fields_id'],FILTER_SANITIZE_STRING)));
  }
  elseif($app_redirect_to == 'parent_infopage_filters')
  {
  	echo form_tag('sorting_form', url_for('entities/parent_infopage_filters','entities_id=' . filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING)));
  }
  elseif($app_redirect_to == 'infopage_entityfield_filters')
  {
  	echo form_tag('sorting_form', url_for('entities/infopage_entityfield_filters','entities_id=' . filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING) . '&related_entities_id=' . filter_var($_GET['related_entities_id'],FILTER_SANITIZE_STRING) . '&fields_id=' . filter_var($_GET['fields_id'],FILTER_SANITIZE_STRING)));
  }
  ...
  {
    echo form_tag('sorting_form', url_for('items/','path=' . filter_var($_GET['path'],FILTER_SANITIZE_STRING)));
  }
  else
  { 
  ...
  if(count($sorting_fields)>0)
  {
    ...
    {    
      ...
      while($v = db_fetch_array($fields_query))
      {
        ...
        echo '<li id="form_fields_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '_' . $sorting_fields_info[filter_var($v['id'],FILTER_SANITIZE_STRING)] . '"><div><img rel="' . $sorting_fields_info[filter_var($v['id'],FILTER_SANITIZE_STRING)]. '" src="images/' . ($sorting_fields_info[filter_var($v['id'],FILTER_SANITIZE_STRING)]=='asc' ? 'arrow_down.png':'arrow_up.png') . '" class="condition_icon" id="condition_icon_' . $v['id'] . '"> ' . fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING)) . '</div></li>';
      }
    }
  }
  ?> 
...
while($v = db_fetch_array($fields_query))
{
  ...
  echo '<li id="form_fields_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '_asc"><div><img rel="asc" src="images/arrow_down.png" class="condition_icon" id="condition_icon_' . $v['id'] . '" > ' . fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING)). '</div></li>';
}
?> 
</ul>
</div>                     
    ...
    $('#fields_for_sorting .condition_icon').each(function(){
      ...
      {    
        ...
        $(this).click(function(){
          ...
          {        
            $.ajax({type: "POST",url: '<?php echo url_for("reports/sorting","action=set_sorting_condition&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING))?>',data: {field_id:id,condition:'desc'} });
          ...
          {
            $.ajax({type: "POST",url: '<?php echo url_for("reports/sorting","action=set_sorting_condition&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING))?>',data: {field_id:id,condition:'asc'} });
            $(this).attr('rel','asc')
  ...
  $(function() {
      prepare_condition_icons();
    		...
    		update: function(event,ui)
        {
          ...
          data = data.slice(1)                      
          $.ajax({type: "POST",url: '<?php echo url_for("reports/sorting","action=set_sorting&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING))?>',data: data});
        }
    	});
