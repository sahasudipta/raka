...
<script>
  ...
  $(function() {                     
        ...
        {               
          ...
          data = data.slice(1)                      
          $.ajax({type: "POST",url: '<?php echo url_for("reports/configure","action=set_listing_fields&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING))?>',data: data});
        }
    	});

    	$('#rows_per_page').keyup(function(){
    		$.ajax({type: "POST",url: '<?php echo url_for("reports/configure","action=set_rows_per_page&reports_id=" . filter_var($_GET["reports_id"],FILTER_SANITIZE_STRING))?>',data: {rows_per_page: $(this).val()}});
      })
