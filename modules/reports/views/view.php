<?php
  ...
  if($reports_info['reports_type']=='entity_menu')
  {
  	...
  	{  	
  		echo '<h3 class="page-title">' . filter_var($page_title,FILTER_SANITIZE_STRING) . $help_pages->render_icon('listing') . '</h3>';
  	}
...
if($reports_info['reports_type']!='common')
{
	if($reports_info['reports_type']=='entity_menu')
	{
		...
		echo htmlentities($filters_panels->render_horizontal());
	}
	else
	{
  	$filters_preivew = new filters_preivew($reports_info['id']);
  ...
  <div class="col-sm-5">
    <div class="entitly-listing-buttons-left">
      ...
      <?php
      	...
      	$reports_hide_insert_button = ($reports_info['reports_type']=='entity_menu' ? false : $entity_cfg->get('reports_hide_insert_button'));
        ...
        {         	
          ...
          {          	
          	$url = url_for('items/form','path=1-' . $app_user['id'] . '/' . $reports_info['entities_id'] . '&redirect_to=report_' . $reports_info['id']);
          }
          else
          {
...
<?php 
	...
	{
		$processes = new processes(filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING));
		$processes->rdirect_to = 'reports';
		$with_selected_menu .=  $processes->render_buttons('menu_with_selected',filter_var($reports_info['id'],FILTER_SANITIZE_STRING));
	}
  ...
  if( users::has_access('delete',$access_schema) and users::has_access('delete_selected',$access_schema) and $reports_info['entities_id']!=1)
  {
  	$with_selected_menu .=  '<li>' . link_to_modalbox('<i class="fa fa-trash-o"></i> ' . TEXT_BUTTON_DELETE,url_for('items/delete_selected', 'redirect_to=report_' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) . '&path=' . filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING) . '&reports_id=' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) )) . '</li>';
  }
  
  $listing = new items_listing(filter_var($reports_info['id'],FILTER_SANITIZE_STRING));
...
?>
  ...
  <div class="col-sm-2">
  	<?php echo (($reports_info['reports_type']!='common' and !isset($force_filters_reports_id)) ? listing_types::render_switches(filter_var_array($reports_info),$curren_listing_type):'') ?>
  </div>
  <div class="col-sm-5">
    <div class="entitly-listing-buttons-right">    
      <?php echo render_listing_search_form(filter_var($reports_info["entities_id"],FILTER_SANITIZE_STRING),$listing_container,filter_var($reports_info['id'],FILTER_SANITIZE_STRING)) ?>
      
      <?php 
      	if(!filters_preivew::has_default_panel_access($entity_cfg) and $reports_info['reports_type']=='entity_menu' and (in_array($app_user['group_id'],explode(',',$entity_cfg->get('listing_config_access'))) or !strlen($entity_cfg->get('listing_config_access'))))
      	{
      		$html = '
...
<?php if(isset($filters_panels)) { ?>

<div class="row">
  <div class="col-xs-<?php echo (12-$filters_panels->vertical_width)?>">
    <div id="<?php echo $listing_container ?>" class="entity_items_listing"></div>
  </div>
  
  <?php echo htmlentities($filters_panels->render_vertical()); ?>
</div>
