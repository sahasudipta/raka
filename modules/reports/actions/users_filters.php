...
switch($app_module_action)
{
  case 'use':
      ...
      $users_filters->use_filters(filter_var($_GET['id'],FILTER_SANITIZE_STRING));
      $users_filters->set_reports_settings(filter_var($_GET['id'],FILTER_SANITIZE_STRING));      
    break;
  case 'save':                  
  ...
  case 'delete':      
      ...
      {        
        $filters_query = db_query("select * from app_users_filters where id in (" . implode(',',filter_var_array($_POST['filters'])) . ") and users_id='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . "'");
