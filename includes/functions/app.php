...
function ajax_modal_template_header($title)
{
  $html = '  		
			<div class="modal-header">  			
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
  			<button type="button" class="close modal-collapse" aria-hidden="true"></button>
				<h4 class="modal-title">' . filter_var($title,FILTER_SANITIZE_STRING) . '</h4>
			</div>							
  ';
...
function app_powered_by_text()
{
    ...
    $text = '<small>' . TEXT_POWERED_BY .'&nbsp;<a target="_blank" href="https://www.rukovoditel.net' . (APP_LANGUAGE_SHORT_CODE=='ru' ? '.ru':''). '" title="' . TEXT_POWERED_BY_TITLE . '">Rukovoditel</a></small>';
