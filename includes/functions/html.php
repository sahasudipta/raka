<?php
  ...
  function input_tag($name='',$value='',$attributes=array())
  {
    ...
    return '<input ' . tag_attributes_to_html(filter_var_array($default),filter_var_array($attributes)) . '>';
  }
  ...
  {         
    ...
    foreach(filter_var_array($choices) as $k=>$v)
    { 
      if(is_array($v))
      {
    ...
    return '<div class="select_checkboxes_tag">' . $html . '</div> <label for="' . filter_var($name,FILTER_SANITIZE_STRING) . '[]" class="error"></label>';
  }
