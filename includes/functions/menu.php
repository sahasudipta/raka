<?php
  ...
  function build_entities_menu($menu)
  {
    ...
    {
    	$custom_entities_menu = array_merge($custom_entities_menu,explode(',',filter_var($v['entities_list'],FILTER_SANITIZE_STRING)));
    }
    ...
    {
      if(filter_var($entities['parent_id'],FILTER_SANITIZE_STRING)==0)
      {
        $s = array();
              
        $entity_cfg = entities::get_cfg(filter_var($entities['id'],FILTER_SANITIZE_STRING));
        $menu_title = (strlen($entity_cfg['menu_title'])>0 ? $entity_cfg['menu_title'] : filter_var($entities['name'],FILTER_SANITIZE_STRING));                
        $menu_icon = (strlen($entity_cfg['menu_icon'])>0 ? $entity_cfg['menu_icon'] : (filter_var($entities['id'],FILTER_SANITIZE_STRING)==1 ? 'fa-user':'fa-reorder'));        
               
        $menu[] = array('title'=>$menu_title,'url'=>url_for('items/items','path=' . filter_var($entities['id'],FILTER_SANITIZE_STRING)), 'class'=>$menu_icon);
      }
      else
      {
        $reports_info = reports::create_default_entity_report(filter_var($entities['id'],FILTER_SANITIZE_STRING), 'entity_menu');
  ...
  function build_custom_entities_menu($menu,$parent_id = 0, $level=0)
  {
  	...
  	{  		
  		...
  		if(strlen(filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING)))
  		{	
	  		$where_sql = " e.id in (" . filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING). ")";
	  		  		
	  		if($app_user['group_id']==0)
	  		{
	  			$entities_query = db_query("select * from app_entities e where e.id in (" . filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING). ") order by field(e.id," . filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING) . ")");
	  		}
	  		else
	  		{
	  			$entities_query = db_query("select e.* from app_entities e, app_entities_access ea where e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input($app_user['group_id']) . "' and e.id in (" . filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING). ") order by field(e.id," . filter_var($entities_menu['entities_list'],FILTER_SANITIZE_STRING) . ")");
	  		}
	  		...
	  		while($entities = db_fetch_array($entities_query))
	  		{
	  			if($entities['parent_id']==0)
	  			{
	  				...
	  				$sub_menu[] = array('title'=>$menu_title,'url'=>url_for('items/items','path=' . filter_var($entities['id'],FILTER_SANITIZE_STRING)),'class'=>$menu_icon);
	  			}
	  			else
	  			{
	  				$reports_info = reports::create_default_entity_report(filter_var($entities['id'],FILTER_SANITIZE_STRING), 'entity_menu');
  ...
  function build_reports_menu($menu)
  {
    ...
    {                  
      ...
      while($v = db_fetch_array($reports_query))
      {
        $menu[] = array('title'=>filter_var($v['name'],FILTER_SANITIZE_STRING),
                        'url'=>url_for('reports/view','reports_id=' . filter_var($v['id'],FILTER_SANITIZE_STRING)),
                        'class'=>(strlen(filter_var($v['menu_icon'],FILTER_SANITIZE_STRING))>0 ? filter_var($v['menu_icon'],FILTER_SANITIZE_STRING) : 'fa-list-alt'));
      }
  ...
  function build_reports_groups_menu($menu)
  {
  	...
  	$reports_query = db_query("select * from app_reports_groups where created_by = '" . filter_var($app_user['id'],FILTER_SANITIZE_STRING) . "' and is_common=0 and in_menu=1 order by sort_order, name");
  	while($v = db_fetch_array($reports_query))
  	{
  		$check_query = db_query("select id from app_entities_menu where find_in_set('dashboard" . filter_var($v['id'],FILTER_SANITIZE_STRING). "',reports_list)");
  		if(!$check = db_fetch_array($check_query))
  		{
  			$menu[] = array('title'=>filter_var($v['name'],FILTER_SANITIZE_STRING),'url'=>url_for('dashboard/reports','id=' . filter_var($v['id'],FILTER_SANITIZE_STRING)),'class'=>(strlen(filter_var($v['menu_icon'],FILTER_SANITIZE_STRING))>0 ? filter_var($v['menu_icon'],FILTER_SANITIZE_STRING) : 'fa-cubes'));
  		}
  	}
  	
  	$reports_query = db_query("select * from app_reports_groups where find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ",users_groups) and is_common=1 and in_menu=1 order by sort_order, name");
  	while($v = db_fetch_array($reports_query))
  	{
  		$check_query = db_query("select id from app_entities_menu where find_in_set('dashboard" . filter_var($v['id'],FILTER_SANITIZE_STRING). "',reports_list)");
  		if(!$check = db_fetch_array($check_query))
  		{
  			$menu[] = array('title'=>filter_var($v['name'],FILTER_SANITIZE_STRING),'url'=>url_for('dashboard/reports_groups','id=' . filter_var($v['id'],FILTER_SANITIZE_STRING)),'class'=>(strlen(filter_var($v['menu_icon'],FILTER_SANITIZE_STRING))>0 ? filter_var($v['menu_icon'],FILTER_SANITIZE_STRING) : 'fa-cubes'));
  		}
  	}
  ...
  function build_main_menu()
  {
    ...
    if($app_user['group_id']==0)
    {
      ...
      $menu[] = array('title'=>TEXT_MENU_SUPPORT,'url'=>'https://docs.rukovoditel.net' . $store_language,'submenu'=>$s,'class'=>'fa-envelope-o');
  ...
  { 
    ...
    foreach(filter_var_array($menu) as $v)
    {
