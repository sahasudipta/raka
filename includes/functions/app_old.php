...
function app_send_new_comment_notification($comments_id,$item_id,$entity_id)
{
  ...
  $item = db_find('app_entity_' . filter_var($entity_id,FILTER_SANITIZE_STRING),fiter_var($item_id,FILTER_SANITIZE_STRING));
  
  $send_to = items::get_send_to(filter_var($entity_id,FILTER_SANITIZE_STRING),filter_var($item_id,FILTER_SANITIZE_STRING),filter_var_array($item));
  ...
  if($cfg->get('send_notification_to_assigned',0)==0)
  {
	  ...
	  $comments_query = db_query("select * from app_comments where entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and items_id='" . db_input(filter_var($item_id,FILTER_SANITIZE_STRING)) . "'" . (count(filter_var_array($send_to)) ? " and created_by not in (" . implode(',',filter_var_array($send_to)). ")":''));
...
function app_powered_by_text()
{
    ...
    $text = '<small>' . TEXT_POWERED_BY .'&nbsp;<a target="_blank" href="https://myhcl.com' . (APP_LANGUAGE_SHORT_CODE=='ru' ? '.ru':''). '" title="' . TEXT_POWERED_BY_TITLE . '">HCL Technologies</a></small>';
...
function app_author_text()
{
    ...
    return '<meta content="https://myhcl.com' . (APP_LANGUAGE_SHORT_CODE=='ru' ? '.ru':'') . '" name="author"/>' . "\n";
}
