<?php
  ...
  function db_perform($table, $data, $action = 'insert', $parameters = '') 
  {
    ...
    if ($action == 'insert') 
    {
      $query = 'insert into ' . filter_var($table,FILTER_SANITIZE_STRING) . ' (';
            
      foreach(filter_var_array($data) as $columns=>$value)
      {
        $query .= $columns . ', ';
      }
      ...
      {      
        switch ((string)$value) {
          ...
          case 'null':
            ...
            break;
          default:
            $query .= '\'' . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . '\', ';
            break;
        }
      }
      $query = substr($query, 0, -2) . ')';
    ...
    {
      $query = 'update ' . filter_var($table,FILTER_SANITIZE_STRING) . ' set ';
      ...
      foreach(filter_var_array($data) as $columns=>$value)
      {
        switch ((string)$value) {
          ...
          case 'null':
            ...
            break;
          default:
            $query .= $columns . ' = \'' . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . '\', ';
            break;
        }
      }
  ...
  {
    return db_query("select * from " . filter_var($table,FILTER_SANITIZE_STRING) .  (strlen($where)>0 ? ' where ' . $where : '') .  (strlen($order_by)>0 ? ' order by ' . $order_by : ''));
  }
  ...
  {
    $info_query = db_query("select count(*) as total from " . filter_var($table,FILTER_SANITIZE_STRING) . (strlen($value)>0 ? " where " . $column . "='" . db_input(filter_var($value,FILTER_SANITIZE_STRING))  . "'":""));
  ...
  {
    db_query("delete from " . filter_var($table,FILTER_SANITIZE_STRING) . " where " . $column . "='" . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . "'");
  }
