...
class items_copy
{
	...
	function copy_item($item_info=null)
	{
		...
		if($item_info===null)
		{
			$item_info_query = db_query("select * from app_entity_" . filter_var($this->entities_id,FILTER_SANITIZE_STRING) . " where id='" . db_input(filter_var($this->items_id,FILTER_SANITIZE_STRING)) . "'");
		...
		if($item_info)
		{
			...
			//copy choices values
			$sql_data = array();
			$choices_values_query = db_query("select * from app_entity_" . filter_var($this->entities_id,FILTER_SANITIZE_STRING) . "_values where items_id = " . db_input(filter_var($this->items_id,FILTER_SANITIZE_STRING)));
	...
	function copy_sub_entities($copy_sub_entities, $entities_id=false, $parent_item_id = false, $new_item_id = false)
	{
		...
		//get entities 
		$entities_query = db_query("select * from app_entities where id in (" . implode(',', filter_var_array($copy_sub_entities)) . ") and parent_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "'");
		while($entities = db_fetch_array($entities_query))
		{
			//check sub entities
			$sub_entiteis_list = [];
			$sub_entiteis_query = db_query("select id from app_entities where parent_id='" . filter_var($entities['id'],FILTER_SANITIZE_STRING) . "'");
			...
			//copy items
			$items_query = db_query("select * from app_entity_" . filter_var($entities['id'],FILTER_SANITIZE_STRING) . " where parent_item_id='" . filter_var($parent_item_id,FILTER_SANITIZE_STRING) . "'");
