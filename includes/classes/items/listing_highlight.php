...
{     
    ...
    function apply($item)
    {
        ...
        {
            if(!isset($item['field_' . $rule['fields_id']])) continue;
    ...
    function match_rule($rule,$item_field_value)
    {
        switch(listing_highlight::get_field_type_key($rule['type']))
        {
            ...
            case 'entities':
                ...
                {
                    foreach(explode(',',filter_var($item_field_value,FILTER_SANITIZE_STRING)) as $item_value)
                    {
                        if(in_array($item_value,$fields_values)) return true;
                    }
                }
            ...
            case 'dates':
                ...
                if(!strlen($rule['fields_values']) or (int)$item_field_value==0) return false;
                ...
                if(!in_array($days[0],['+','-'])) $days = '+' . $days;
                                                
                if($rule['type'] == 'fieldtype_input_datetime')
                {
                    $eval_str = '$check = ((' . filter_var($item_field_value,FILTER_SANITIZE_STRING)  . $operator . (strtotime($days . ' day')) . ') ? true:false);';
                }
                else
                {
                    $eval_str = '$check = ((' . filter_var($item_field_value,FILTER_SANITIZE_STRING)  . $operator . get_date_timestamp(date('Y-m-d',strtotime($days . ' day'))) . ') ? true:false);';
                }
                ...
                {
                    echo alert_error(TEXT_ERROR . ' listing highlight #' . filter_var($rule['id'],FILTER_SANITIZE_STRING) . ' <br>' . $eval_str . '<br>' . $e->getMessage());
                }
            ...
            case 'numeric':
                ...
                if(!strlen($item_field_value)) return false;
                ...
                for($i=1;$i<count($values);$i+=2)
                {
                    ...
                    $eval_str = '$check = ((' . filter_var($item_field_value,FILTER_SANITIZE_STRING)  . $operator . $value . ') ? true:false);';
    ...
    static function get_field_value_by_type($field,$value)
    {
        ...
        switch(self::get_field_type_key($field['type']))
        {
           case 'choices':
               ...
               break;
           case 'entities':
               $items_info_sql = "select e.* from app_entity_" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " e where e.id in (" . filter_var($value,FILTER_SANITIZE_STRING) . ")";
