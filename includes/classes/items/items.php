...
class items
{
	...
	{
		$item_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . " e where id='" . filter_var($items_id,FILTER_SANITIZE_STRING) . "'");
	...
	{
		$entities_query = db_query("select id from app_entities where parent_id = '" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "'");
		while($entities = db_fetch_array($entities_query))
		{
			$items_query = db_query("select id from app_entity_" . filter_var($entities['id'],FILTER_SANITIZE_STRING) . " where parent_item_id in (" . implode(',',filter_var_array($itesm_list[filter_var($entities_id,FILTER_SANITIZE_STRING)])). ") ");
			while($items = db_fetch_array($items_query))
			{
				$itesm_list[filter_var($entities['id'],FILTER_SANITIZE_STRING)][] = filter_var($items['id'],FILTER_SANITIZE_STRING);
			}
	
			if(isset($itesm_list[$entities['id']]))
			{
				$itesm_list = self::get_items_to_delete(filter_var($entities['id'],FILTER_SANITIZE_STRING), filter_var_array($itesm_list));
			}
		}
	...
	public static function delete($entities_id,$items_id)
	{
	    ...
	    if($entities_id==1 and $items_id==$app_user['id']) return false;
	    	    
		//reset parent items
		$item_info = db_find("app_entity_" . $entities_id,$items_id);		
		db_query("update app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "  set parent_id='" . filter_var($item_info['parent_id'],FILTER_SANITIZE_STRING). "' where parent_id='" . filter_var($items_id,FILTER_SANITIZE_STRING) . "'");
	...
	public static function get_choices($entity_id, $add_empty = false, $empty_text='')
	{
		...
		$listing_sql_query = items::add_access_query(filter_var($entity_id,FILTER_SANITIZE_STRING),$listing_sql_query);
	
		//include access to parent records
		$listing_sql_query .= items::add_access_query_for_parent_entities(filter_var($entity_id,FILTER_SANITIZE_STRING));
	
		$listing_sql_query .= items::add_listing_order_query_by_entity_id(filter_var($entity_id,FILTER_SANITIZE_STRING));
	
		//build query
		$listing_sql = "select e.* from app_entity_" . filter_var($entity_id,FILTER_SANITIZE_STRING) . " e "  . $listing_sql_query_join . "where e.id>0 " . $listing_sql_query;
		...
		{
			$path_info = items::get_path_info($entity_id,filter_var($item['id'],FILTER_SANITIZE_STRING));
			...
			$choices[$item['id']] = $parent_name . self::get_heading_field($entity_id,filter_var($item['id'],FILTER_SANITIZE_STRING));
		}
  ...
  {            
    ...
    $default_reports_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($parent_entity_id,FILTER_SANITIZE_STRING)). "' and reports_type='default'");
    ...
    //check view assigned only access
    $listing_sql_query = items::add_access_query(filter_var($parent_entity_id,FILTER_SANITIZE_STRING),$listing_sql_query);
  
    //include access to parent records
    $listing_sql_query .= items::add_access_query_for_parent_entities(filter_var($parent_entity_id,FILTER_SANITIZE_STRING));
    
    $listing_sql_query .= items::add_listing_order_query_by_entity_id(filter_var($parent_entity_id,FILTER_SANITIZE_STRING));
        
    //build query
    $listing_sql = "select e.* from app_entity_" . filter_var($parent_entity_id,FILTER_SANITIZE_STRING) . " e "  . $listing_sql_query_join . "where e.id>0 " . $listing_sql_query;        
    ...
    {     
      ...
      {      	
        ...
        case 'fieldtype_entity':
            ...
            {
              $entity_heading_field_id = filter_var($fields['id'],FILTER_SANITIZE_STRING);
            }
    
            $output = array();
            foreach(explode(',',filter_var($heading_field_value,FILTER_SANITIZE_STRING)) as $item_id)
            {
              ...
              { 
                ...
                {               
                  ...
                  {
                    $output[] = self::get_heading_field_value($entity_heading_field_id,filter_var_array($item));               
                  }
                  else
                  {
  ...
  public static function get_breadcrumb($path_array)
  {
    ...
    foreach($path_array as $v)
    {
      ...
      if($item_id>0)
      {
          ...
          $item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entity_id),'') . " from app_entity_" . filter_var($entity_id) . " e where e.id='" . filter_var($item_id) . "'",false);
          if($item_info = db_fetch_array($item_info_query))
          {        
              $item_name = ($heading_field_id>0 ? self::get_heading_field_value($heading_field_id,filter_var_array($item_info)) : filter_var($item_info['id'],FILTER_SANITIZE_STRING));
          }
  ...
  public static function build_menu()
  {
    ...
    {        
      ...
      while($entities = db_fetch_array($entities_query))
      {    
      	if(!isset($app_users_access[$entities['id']]) and $app_user['group_id']>0) continue;
      	
      	//check if subentity hidden by filter that set on item page configuration
      	if(entities::is_hidden_by_condition($entities['id'], $item_id)) continue;
  ...
  public static function render_info_box($entity_id,$item_id,$users_id=false,$exclude_fields_types=true)
  {
    ...
    $entity_cfg = new entities_cfg(filter_var($entity_id,FILTER_SANITIZE_STRING));
    
    if($users_id>0)
    {
    	$is_email = true;
      $user_info = db_find('app_entity_1',$users_id);
      $fields_access_schema = users::get_fields_access_schema(filter_var($entity_id,FILTER_SANITIZE_STRING),$user_info['field_6']);      
    }
    else
    { 
    	$is_email = false;
      $fields_access_schema = users::get_fields_access_schema(filter_var($entity_id,FILTER_SANITIZE_STRING),filter_var($app_user['group_id'],FILTER_SANITIZE_STRING));
    }
    ...
    $listing_sql_query_select = fieldtype_formula::prepare_query_select(filter_var($entity_id,FILTER_SANITIZE_STRING), $listing_sql_query_select);
        
    $item_query = db_query("select e.* " . $listing_sql_query_select . " from app_entity_" . filter_var($entity_id) . " e where id='" . filter_var($item_id) . "'",false);
    $item = db_fetch_array($item_query);
    ...
    $count = 0;
    $tabs_query = db_fetch_all('app_forms_tabs',"entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' order by  sort_order, name");
    while($tabs = db_fetch_array($tabs_query))
    {                    
    	$html_fields = '';
    	
      $fields_query = db_query("select f.*, t.name as tab_name, if(f.type in ('fieldtype_id','fieldtype_date_added','fieldtype_date_updated','fieldtype_created_by'),-1,t.sort_order) as tab_sort_order from app_fields f, app_forms_tabs t where f.type not in ('fieldtype_action' {$exclude_fields_types} )  and f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id and f.forms_tabs_id='" . db_input(filter_var($tabs['id'],FILTER_SANITIZE_STRING)) . "' order by tab_sort_order, t.name, f.sort_order, f.name",false);
      while($field = db_fetch_array($fields_query))
      {               	      	
      	//exclude fields in email
      	if($is_email and in_array($field['type'],fields_types::get_types_excluded_in_email())) continue;
        ...
        //prepare field value
        $value = self::prepare_field_value_by_type(filter_var_array($field), filter_var_array($item));                        
        ...
        $check_query = db_query("select * from app_forms_fields_rules where fields_id='" . filter_var($field['id'],FILTER_SANITIZE_STRING) . "'");
        if($check = db_fetch_array($check_query))
        {
        	...
        	$fields_display_rules[] = 'app_handle_forms_fields_display_rules(\'\',' . filter_var($field['id'],FILTER_SANITIZE_STRING) . ',"","' . (strlen($value) ? $value : '') . '",' . (int)$is_multiple . '); ';
        }
        ...
        {
        	if(in_array($field['id'],explode(',',$entity_cfg->get('item_page_hidden_fields','')))) continue;
        }
				                
        $field_name = fields_types::get_option(filter_var($field['type'],FILTER_SANITIZE_STRING),'name',filter_var($field['name'],FILTER_SANITIZE_STRING));
        
        $field_name .= fields::get_item_info_tooltip(filter_var_array($field));
                       
        if($field['type']=='fieldtype_section')
        {
        	$html_fields .='
            <tr class="form-group-' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '">
        ...
        {         
        	...
        	{
        		$field_name_html = '<br><span class="download-all-attachments"><a style="margin-left: 0; font-weight: normal" href="' . url_for('items/info','action=download_all_attachments&id=' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '&path=' . $current_path). '"><i class="fa fa-download"></i> ' . TEXT_DOWNLOAD_ALL_ATTACHMENTS . '</a></span>';
        	}
      ...
      if(strlen($html_fields))
      {
	      ...
	      <table class="table table-bordered table-hover table-item-details table-item-details-' . filter_var($entity_id,FILTER_SANITIZE_STRING) . '">
	      		' . $html_fields . '
	      </table></div>';
      }
  ...
  public static function render_content_box($entity_id,$item_id,$users_id = false)
  {
    ...
    {
    	$is_email = false;
      $fields_access_schema = users::get_fields_access_schema(filter_var($entity_id,FILTER_SANITIZE_STRING),$app_user['group_id']);
    }
    
    $entity_cfg = new entities_cfg(filter_var($entity_id,FILTER_SANITIZE_STRING));
            
    $item = db_find('app_entity_' . filter_var($entity_id,FILTER_SANITIZE_STRING),$item_id);
    
    $html = '';
    $count = 0;

    $html = '';  
    $fields_query = db_query("select f.* from app_fields f, app_forms_tabs t where f.type in ('fieldtype_iframe','fieldtype_google_map_directions','fieldtype_google_map','fieldtype_mind_map','fieldtype_image_map','fieldtype_todo_list','fieldtype_textarea','fieldtype_textarea_wysiwyg','fieldtype_attachments','fieldtype_image','fieldtype_mapbbcode') and  f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
    while($field = db_fetch_array($fields_query))
    {   
    	//exclude fields in email
    	if($is_email and in_array($field['type'],fields_types::get_types_excluded_in_email())) continue;
      ...
      $value = $value_original = $item['field_' . filter_var($field['id'],FILTER_SANITIZE_STRING)];
      ...
      $value = fields_types::output(filter_var_array($output_options));
    
      if(strlen($value)>0)
      {
        $field_name_html = '';
        
        $field_name_html .= fields::get_item_info_tooltip(filter_var_array($field));
        
        //add dwonload All Attachments link if more then 1 files
        if($field['type']=='fieldtype_attachments' and count(explode(',',$value_original))>1)
        {
          $field_name_html .= '<span class="download-all-attachments"><a href="' . url_for('items/info','action=download_all_attachments&id=' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '&path=' . $current_path). '"><i class="fa fa-download"></i> ' . TEXT_DOWNLOAD_ALL_ATTACHMENTS . '</a></span>';
        }
        
        $html .='
        	<div  class="form-group-' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '">	
          	<div class="content_box_heading"><h4 class="media-heading">' . filter_var($field['name'],FILTER_SANITIZE_STRING)  . $field_name_html .  '</h4></div>
          	<div class="content_box_content ' . filter_var($field['type'],FILTER_SANITIZE_STRING) . '">' . $value . '</div>
          </div>
        ';
      }
    }
  ...
  public static function get_fields_values_cache($fields_cache,$path_array,$current_entity_id)
  {
    foreach($path_array as $v)
    {
      ...
      $fields_query = db_query("select f.* from app_fields f where f.type not in (" . fields_types::get_reserverd_types_list(). ") and  f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' order by f.sort_order, f.name");
      while($field = db_fetch_array($fields_query))
      {
        $fields_cache[filter_var($field['id'],FILTER_SANITIZE_STRING)] = $item_info['field_' . filter_var($field['id'],FILTER_SANITIZE_STRING)];
      }
  ...
  {    
    ...
    foreach(filter_var_array($path_array )as $v)
    {
      $path_list[] = $v['path'];
  ...
  public static function get_path_array($entities_id, $items_id,$path_array = array(),$current_item_info = false)
  {
    ...
    {	
    	...
    	{	
    		$items_query = db_query("select * from app_entity_" . filter_var($entities_id) . " where id='" . filter_var($items_id) . "'");
    ...
    {      
      $name = self::get_heading_field_value($heading_field_id,filter_var_array($items));      
    }
    else
    {
      $name = filter_var($items['id'],FILTER_SANITIZE_STRING);
    }   
    ...
    {        
      ...
      $path_array = items::get_path_array($entities['parent_id'],filter_var($items['parent_item_id']),$path_array);
    }
  ...
  {
    $path_array = explode('/',filter_var($path,FILTER_SANITIZE_STRING));
    ...
    if(count($path_array)>1)
    {
      ...
      $parent_entity_id = filter_var($v[0],FILTER_SANITIZE_STRING);
      $parent_entity_item_id = filter_var($v[1],FILTER_SANITIZE_STRING);
    }
    else
    {
      $parent_entity_id = 0;
  ...
  public static function get_paretn_entity_id_by_path($path)
  {
    ...
    foreach(filter_var_array($path_array) as $v)
    {
      ...
      $count = db_count('app_entities',filter_var($vv[0],FILTER_SANITIZE_STRING), 'parent_id');
  ...
  {   
    ...
    if($parent_id>0)
    {
      ...
      {
        $entities_query = db_query("select e.* from app_entities e where parent_id='" . db_input(filter_var($parent_id,FILTER_SANITIZE_STRING)) . "' order by e.sort_order, e.name");
      }
      else
      {
        $entities_query = db_query("select e.* from app_entities e, app_entities_access ea where e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)) . "' and e.parent_id = '" . db_input(filter_var($parent_id,FILTER_SANITIZE_STRING)) . "' order by e.sort_order, e.name");
      }
      
      
      while($entities = db_fetch_array($entities_query))
      {
        $list[] = filter_var($entities['id'],FILTER_SANITIZE_STRING);      
      }
    }
  ...
  public static function add_access_query_for_parent_entities($entities_id,$listing_sql_query='')
  {
    ...
    $entity_info  = $app_entities_cache[filter_var($entities_id,FILTER_SANITIZE_STRING)];
            
    if($entity_info['parent_id']>0)
    {      
      $listing_sql_query = ' and e.parent_item_id in (select e.id from app_entity_' . filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING). ' e where e.id>0 '  . items::add_access_query(filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING),'') . ' ' . items::add_access_query_for_parent_entities(filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING)) . ')'; 	
    } 
  ...
  public static function add_access_query_for_user_parent_entities($entities_id,$listing_sql_query='')
  {
  	...
  	if($entity_info['parent_id']>0)
  	{
  		$listing_sql_query = ' and e.parent_item_id in (select e.id from app_entity_' . filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING). ' e where '  . ($entity_info['parent_id']==1 ? 'e.id=' . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) :'e.id>0') . ' ' . items::add_access_query_for_user_parent_entities(filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING)) . ')';
  	}
  ...
  public static function add_access_query($current_entity_id,$listing_sql_query, $force_access_query = false)
  {
    ...
    $access_schema = users::get_entities_access_schema(filter_var($current_entity_id,FILTER_SANITIZE_STRING),filter_var($app_user['group_id'],FILTER_SANITIZE_STRING));
    ...
    foreach(filter_var_array($users_entities_tree) as $v)
    {
    	$users_entities[] = $v['id'];
    }
        
    //force check users entities tree access
    if(in_array($current_entity_id,$users_entities) and users::has_access('view_assigned',$access_schema) and filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)>0)
    {
    	$listing_sql_query .= self::add_access_query_for_user_parent_entities(filter_var($current_entity_id,FILTER_SANITIZE_STRING));
    ...
    elseif((users::has_access('view_assigned',$access_schema) and filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)>0) or $force_access_query)
    {                    
      $users_fields = array(); 
      $fields_query = db_query("select f.id from app_fields f where f.type in ('fieldtype_users','fieldtype_users_ajax','fieldtype_user_roles','fieldtype_users_approve') and  f.entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "'");
      while($fields = db_fetch_array($fields_query))
      {    
        $users_fields[] = filter_var($fields['id'],FILTER_SANITIZE_STRING);
      }
      
      $grouped_users_fields = array(); 
      $grouped_global_users_fields = array();
      $fields_query = db_query("select f.id, f.configuration from app_fields f where f.type in ('fieldtype_grouped_users') and  f.entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "'");
      while($fields = db_fetch_array($fields_query))
      {    
      	...
      	if($cfg->get('use_global_list')>0)
      	{
      		$grouped_global_users_fields[$cfg->get('use_global_list')] = filter_var($fields['id'],FILTER_SANITIZE_STRING);
      	}
      	else
      	{	      	
        	$grouped_users_fields[] = filter_var($fields['id'],FILTER_SANITIZE_STRING);
      	}
      }
      
      $access_group_fields = array();
      $fields_query = db_query("select f.id from app_fields f where f.type in ('fieldtype_access_group') and  f.entities_id='" . db_input(filter_var($current_entity_id,FILTER_SANITIZE_STRING)) . "'");
      while($fields = db_fetch_array($fields_query))
      {
      	$access_group_fields[] = filter_var($fields['id'],FILTER_SANITIZE_STRING);
      }
      ...
      //check users fields
        ...
        {                    
          $sql_query_array[] = "(select count(*) as total from app_entity_" .filter_var($current_entity_id,FILTER_SANITIZE_STRING) . "_values cv where  cv.items_id=e.id and cv.fields_id='" .db_input( filter_var($id,FILTER_SANITIZE_STRING)) . "' and cv.value='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . "')>0";        	        	        	
        }
        
        //check gouped users
        foreach(filter_var_array($grouped_users_fields) as $id)
        {        
          $sql_query_array[] = "(select count(*) as total from app_entity_" .filter_var($current_entity_id,FILTER_SANITIZE_STRING) . "_values cv where cv.items_id=e.id and cv.fields_id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)) . "' and cv.value in (select id from app_fields_choices fc where fc.fields_id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)) . "' and find_in_set(" . filter_var($app_user['id'],FILTER_SANITIZE_STRING)  . ",fc.users)))>0";                  	        	        	        
        }
        
        //check gouped users with globallist
        foreach(filter_var_array($grouped_global_users_fields) as $list_id=>$id)
        {
        	$sql_query_array[] = "(select count(*) as total from app_entity_" .filter_var($current_entity_id,FILTER_SANITIZE_STRING) . "_values cv where cv.items_id=e.id and cv.fields_id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)) . "' and cv.value in (select id from app_global_lists_choices fc where fc.lists_id='" . db_input(filter_var($list_id,FILTER_SANITIZE_STRING)). "' and find_in_set(" . filter_var($app_user['id'],FILTER_SANITIZE_STRING)  . ",fc.users)))>0";
        }
        
        //check access group fields
        foreach(filter_var_array($access_group_fields) as $id)
        {
        	$sql_query_array[] = "(select count(*) as total from app_entity_" .filter_var($current_entity_id,FILTER_SANITIZE_STRING) . "_values cv where cv.items_id=e.id and cv.fields_id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)) . "' and cv.value='" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . "')>0";
        }
    ...
    $listing_sql_query .= records_visibility::add_access_query(filter_var($current_entity_id,FILTER_SANITIZE_STRING));
  ...
  public static function add_listing_order_query_by_entity_id($entities_id,$order_cause='asc',$alias='e')
  {
    ...
    { 
    	$field_info = db_find('app_fields',filter_var($heading_id,FILTER_SANITIZE_STRING));
  ...
  {  	
  	...
  	$check_query = db_query("select count(*) as total from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) .  " where field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . "='" . db_input($fields_value)  . "'" . ($items_id ? " and id!='" . db_input($items_id) . "'":""));
  ...
  public static function get_send_to($entity_id, $item_id, $item=false)
  {
  	...
  	if(!$item)
  	{
  		$item = db_find('app_entity_' . filter_var($entity_id,FILTER_SANITIZE_STRING),filter_var($item_id,FILTER_SANITIZE_STRING));
  	}
  	...
  	$fields_query = db_query("select f.* from app_fields f where f.type in ('fieldtype_users_approve','fieldtype_user_roles','fieldtype_grouped_users','fieldtype_users','fieldtype_users_ajax') and  f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' ");
  	while($field = db_fetch_array($fields_query))
  	{
  		$cfg = new fields_types_cfg(filter_var($field['configuration'],FILTER_SANITIZE_STRING));
  		...
  		$field_value = $item['field_' . filter_var($field['id'],FILTER_SANITIZE_STRING)];
  ...
  static function get_paretn_users_list($entities_id,$parent_entity_item_id)
  {
      ...
      if($parent_entity_id==0 or $parent_entity_item_id==0) return false;
      
      $path_array = items::get_path_array(filter_var($parent_entity_id,FILTER_SANITIZE_STRING), filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING));
      
      foreach($path_array as $path_info)
      {
          $parent_users_fields = array();
          $parent_fields_query = db_query("select f.* from app_fields f where f.type in ('fieldtype_users','fieldtype_users_ajax','fieldtype_user_roles','fieldtype_users_approve','fieldtype_access_group','fieldtype_grouped_users') and  f.entities_id='" . db_input(filter_var($path_info['entities_id'],FILTER_SANITIZE_STRING)) . "'");
