...
{	
    ...
    {
        $menu_query = db_query("select * from app_entities_menu where parent_id=" . filter_var($parent_id,FILTER_SANITIZE_STRING) . " order by sort_order, name");
	...
	static function get_reports_list($reports_list)
	{
		...
		foreach(explode(',',$reports_list) as $reports_type)
		{
			...
			{
				$reports_info_query = db_query("select name from {$reports_table} where id='" . db_input(filter_var($reports_id,FILTER_SANITIZE_STRING)). "'");
				if($reports_info = db_fetch_array($reports_info_query))
				{					
					$choices[] = filter_var($reports_info['name'],FILTER_SANITIZE_STRING);
				}
			}
		}
