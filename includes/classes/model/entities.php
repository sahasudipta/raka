...
class entities
{
  ...
  {  	  	
  	$entities_query = db_query("select * from app_entities where parent_id='" . filter_var($parent_id,FILTER_SANITIZE_STRING) . "' order by sort_order, name");  	
  	...
  	while($entities = db_fetch_array($entities_query))
  	{
  		...
  		$tree = entities::get_flowchart_shcema(filter_var($entities['id'],FILTER_SANITIZE_STRING),$tree,$level+1,$x+130,$y);
  		
  		$y = $tree['y'];
  		
  		$count_fields = 0;
  		$check_fields_query = db_query("select * from app_fields where entities_id = '" . filter_var($entities['id'],FILTER_SANITIZE_STRING) . "' and type in ('fieldtype_users','fieldtype_users_ajax','fieldtype_grouped_users','fieldtype_entity','fieldtype_related_records','fieldtype_formula')");
  ...
  {
    $cfg = array();
    $info_query = db_fetch_all('app_entities_configuration',"entities_id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)). "'");
    while($info = db_fetch_array($info_query))
    {
      $cfg[filter_var($info['configuration_name'],FILTER_SANITIZE_STRING)] = filter_var($info['configuration_value'],FILTER_SANITIZE_STRING);
    }
  ...
  public static function get_tree($parent_id=0,$tree=array(),$level=0,$path = array(), $skip = array(), $add_id_to_name = false)
  {
    ...
    {      
      $entities_query = db_query("select e.* from app_entities e, app_entities_access ea where e.parent_id='" . filter_var($parent_id,FILTER_SANITIZE_STRING) . "' and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)) . "' order by e.sort_order, e.name");
    }
    ...
    while($entities = db_fetch_array($entities_query))
    {
      ...
      $tree[] = array('id'=>filter_var($entities['id'],FILTER_SANITIZE_STRING),
                      'name'=>filter_var($entities['name'],FILTER_SANITIZE_STRING) .  ($add_id_to_name ? ' [#' . filter_var($entities['id'],FILTER_SANITIZE_STRING) . ']':''),
                      'notes'=>filter_var($entities['notes'],FILTER_SANITIZE_STRING),	
                      'sort_order'=>filter_var($entities['sort_order'],FILTER_SANITIZE_STRING),  
                      'level'=>$level,
                      'path'=>$path,
                      );
                      
      $tree = entities::get_tree(filter_var($entities['id'],FILTER_SANITIZE_STRING),filter_var_array($tree),$level+1,array_merge($path,array(filter_var($entities['id'],FILTER_SANITIZE_STRING))),$skip,$add_id_to_name);
    }
  ...
  public static function is_hidden_by_condition($entities_id,$parent_item_id)
  {
      ...
      $reports_info_query = db_query("select id from app_reports where entities_id='" . filter_var($parent_entity_id,FILTER_SANITIZE_STRING) . "' and reports_type='hide_subentity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "'", false);
      if($reports_info = db_fetch_array($reports_info_query))
      {
          $listing_sql_query = reports::add_filters_query(filter_var($reports_info['id'],FILTER_SANITIZE_STRING),'','e');
          
          //prepare having query for formula fields
          if(isset($sql_query_having[$parent_entity_id]))
          {
              $listing_sql_query  .= reports::prepare_filters_having_query($sql_query_having[filter_var($parent_entity_id,FILTER_SANITIZE_STRING)]);
          }
          
          //if not filters set
          if(!strlen($listing_sql_query)) return false;
          
          //print_rr($reports_info);
          $item_query = db_query("select e.id from app_entity_".filter_var($parent_entity_id,FILTER_SANITIZE_STRING)." e where e.id='". db_input(filter_var($parent_item_id,FILTER_SANITIZE_STRING)). "'" . $listing_sql_query, false);
