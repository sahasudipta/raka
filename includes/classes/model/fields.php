...
class fields
{
	...
	{
		$choices = array();
		$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in ('fieldtype_action','fieldtype_parent_item_id') and  f.entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' and  f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
  ...
  {
      $field_query = db_query("select id, type, name from app_fields where id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)). "'");
      if($field = db_fetch_array($field_query))
      {
          return fields_types::get_option(filter_var($field['type'],FILTER_SANITIZE_STRING), 'name',filter_var($field['name'],FILTER_SANITIZE_STRING));
      }
      else
      {
          return '';
  ...
  public static function render_required_messages($entities_id)
  {
    ...
    $fields_query = db_query("select f.id, f.type, f.required_message, f.configuration from app_fields f where f.type not in (" . fields_types::get_reserverd_types_list(). ") and  f.entities_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "' order by f.sort_order, f.name");
    while($v = db_fetch_array($fields_query))
    {
      ...
      if(count($attributes))
      {
      	switch($v['type'])
      	{
      		...
      		case 'fieldtype_checkboxes':
      			$name = 'fields[' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '][]';
      			break;
      		default:
      			$name = 'fields[' . filter_var($v['id'],FILTER_SANITIZE_STRING) . ']';
      			break;
  ...
  public static function render_required_ckeditor_ruels($entities_id)
  {
    ...
    {
        $html .='
          "fields[' . filter_var($v['id'],FILTER_SANITIZE_STRING) . ']": { 
            required: function(element){
              CKEDITOR_holders["fields_' . filter_var($v['id'],FILTER_SANITIZE_STRING) . '"].updateElement();              
              return true;             
            }
          },' . "\n";
    }
  ...
  public static function get_search_feidls($entity_id)
  {
    ...
    $fields_query = db_query("select f.id, f.type, f.configuration, f.name, f.is_heading, t.name as tab_name from app_fields f, app_forms_tabs t where f.entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
    while($v = db_fetch_array($fields_query))
    {
      //check field access
  ...
  public static function check_if_type_changed($field_id, $new_type)
  {
    ...
    if($field_info = db_fetch_array($field_info_query))
    {
      ...
      {        
      	//delete index
      	$check_query = db_query("SHOW INDEX FROM app_entity_" . filter_var($field_info['entities_id'],FILTER_SANITIZE_STRING) . " WHERE KEY_NAME = 'idx_field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . "'");
      	if($check = db_fetch_array($check_query))
      	{     
      		db_query("ALTER TABLE app_entity_" . filter_var($field_info['entities_id'],FILTER_SANITIZE_STRING) . " DROP INDEX idx_field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING));
      	}
      	
 				//prepare db field type
 				db_query("ALTER TABLE app_entity_" . filter_var($field_info['entities_id'],FILTER_SANITIZE_STRING). " CHANGE field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . " field_" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . " " . entities::prepare_field_type($new_type) . " NOT NULL;");
  ...
  public static function get_items_fields_data_by_id($item, $fields_list = '',$entities_id, $fields_access_schema)
  {
    ...
    {
      $fields_query = db_query("select f.* from app_fields f, app_forms_tabs t where  f.id in (" . filter_var($fields_list,FILTER_SANITIZE_STRING) . ") and  f.entities_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by field(f.id," . filter_var($fields_list,FILTER_SANITIZE_STRING) . ")");
  ...
  public static function get_field_choices_background_data($field_id)
  {
  	...
  	$field_info_query = db_query("select * from app_fields where id='" . filter_var($field_id ,FILTER_SANITIZE_STRING). "'");
  	if($field_info = db_fetch_array($field_info_query))
  	{
  		$cfg = new fields_types_cfg($field_info['configuration']);
  		if($cfg->get('use_global_list')>0)
  		{
  			$choices_query = db_query("select * from app_global_lists_choices where lists_id = '" . db_input(filter_var($cfg->get('use_global_list'),FILTER_SANITIZE_STRING)). "' and length(bg_color)>0");
  		}
  		else
  		{
  			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input(filter_var($field_id,FILTER_SANITIZE_STRING)). "' and length(bg_color)>0");
  		}
  ...
  public static function get_item_info_tooltip($field)
  {
  	...
  	{
  		$text = filter_var($field['tooltip'],FILTER_SANITIZE_STRING);
  	}
  	elseif(strlen($field['tooltip_item_page']))
  	{
  		$text = filter_var($field['tooltip_item_page'],FILTER_SANITIZE_STRING);
  	}
  ...
  {  	
  	...
  	$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in (" . fields_types::get_reserverd_types_list() . ") and f.entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' and f.forms_tabs_id=t.id {$where_sql} order by t.sort_order, t.name, f.sort_order, f.name");
  	...
  	if(!count($use_fieldtypes) and !$skip_reserved)
  	{
	  	$html .= '  			  			
	  	    <li>
	  				<a href="#" class="insert_to_template_' . htmlentities($unique_id) . '" data-field="[id]">' . htmlentities(TEXT_FIELDTYPE_ID_TITLE) . ' [id]</a>
	  	    </li>
	  	    <li>
	  	      <a href="#" class="insert_to_template_' . htmlentities($unique_id) . '" data-field="[date_added]">' . htmlentities(TEXT_FIELDTYPE_DATEADDED_TITLE) . ' [date_added]</a>
	  	    </li>
	  	    <li>
	  	      <a href="#" class="insert_to_template_' . htmlentities($unique_id) . '" data-field="[created_by]">' . htmlentities(TEXT_FIELDTYPE_CREATEDBY_TITLE) . ' [created_by]</a>
	  	    </li>';
	  	...
	  	if($entities_info['parent_id']>0)
	  	{
	  		$html .= '
	  				<li>
		  	      <a href="#" class="insert_to_template_' . filter_var($unique_id ,FILTER_SANITIZE_STRING). '" data-field="[parent_item_id]">' . htmlentities(TEXT_FIELDTYPE_PARENT_ITEM_ID_TITLE) . ' [parent_item_id]</a>
		  	    </li>';
	  	}
  	}
  	...
  	while($v = db_fetch_array($fields_query))
  	{
  		...
  		else
  		{
  			$html .= '
  		    <li>
  		  		<a href="#"  class="insert_to_template_' . filter_var($unique_id,FILTER_SANITIZE_STRING) . '" data-field="[' . filter_var($v['id'],FILTER_SANITIZE_STRING) . ']">' . fields_types::get_option(filter_var($v['type'],FILTER_SANITIZE_STRING),'name',filter_var($v['name'],FILTER_SANITIZE_STRING)) . ' [' . filter_var($v['id'],FILTER_SANITIZE_STRING) . ']</a>
  		    </li>';
  		}
  	}
  	...
  	$html .= '
  			<script>
  			$(".insert_to_template_' . filter_var($unique_id ,FILTER_SANITIZE_STRING). '").click(function(){
			    html = $(this).attr("data-field").trim();
  				
  				textarea_insert_at_caret("' . filter_var($template_field_id,FILTER_SANITIZE_STRING) .'",html)  					
  ...
  static function get_query($entities_id, $where_sql='')
  {
      ...
      $fields_query = db_query("select f.*, t.name as tab_name, if(f.type in (" . $reserverd_fields_types_list . "),-1,t.sort_order) as tab_sort_order from app_fields f, app_forms_tabs t where f.entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' {$where_sql} and f.forms_tabs_id=t.id order by tab_sort_order, t.name, f.sort_order, f.name",false);
