...
class access_rules
{
	...
	function __construct($entities_id, $item_info)
	{
		...
		{			
			...
			{								
				...
				{
					$access_rules_query = db_query("select * from app_access_rules where find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ", users_groups) and find_in_set(" . filter_var($value,FILTER_SANITIZE_STRING) . ",choices) and  entities_id='" . db_input($entities_id) . "' and fields_id='" . db_input($access_rules_fields['fields_id']) . "'");
	...
	static function has_add_buttons_access($entities_id, $parent_item_id)
	{
		...
		{
			$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input($app_entities_cache[$entities_id]['parent_id']). "' and reports_type='hide_add_button_rules" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "'");
			if($reports_info = db_fetch_array($reports_info_query))
			{				
				//prepare forumulas query
				$listing_sql_query_select = fieldtype_formula::prepare_query_select(filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING),'');
				
				$listing_sql_query = reports::add_filters_query(filter_var($reports_info['id'],FILTER_SANITIZE_STRING),'');
				...
				$listing_sql = "select e.* " . $listing_sql_query_select . " from app_entity_" . filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING) . " e where e.id='" . filter_var($parent_item_id,FILTER_SANITIZE_STRING) . "' " . $listing_sql_query;
