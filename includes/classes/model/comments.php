...
class comments
{
  ...
  public static function delete_item_comments($entity_id,$item_id)
  {
    ...
    {
      db_query("delete from app_comments_history where comments_id = '" . db_input(filter_var($comments['id'],FILTER_SANITIZE_STRING)) . "'");
  ...
  public static function render_content_box($entity_id,$item_id,$user_id)
  {
    ...
    $listing_sql = "select * from app_comments where entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and items_id='" . db_input(filter_var($item_id,FILTER_SANITIZE_STRING)) . "' order by id desc limit " . (filter_var($limit,FILTER_SANITIZE_STRING)+1);
