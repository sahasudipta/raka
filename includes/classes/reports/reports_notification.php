...
class reports_notification
{
	...
	function send()
	{
		...
		{
			$app_user = array('id'=>filter_var($user['id'],FILTER_SANITIZE_STRING),
                        'group_id'=>filter_var((int)$user['field_6'],FILTER_SANITIZE_STRING),
                        'name'=> users::output_heading_from_item($user),                        
                        'email'=>filter_var($user['field_9'],FILTER_SANITIZE_STRING),                        
                        'language'=>filter_var($user['field_13'],FILTER_SANITIZE_STRING),                        
                        );
			...
			{
				$where_sql = " and r.id not in (" . users_cfg::get_value_by_users_id(filter_var($app_user['id'],FILTER_SANITIZE_STRING),'hidden_common_reports') . ")";
			}
			...
			$reports_query = db_query("select * from app_reports where ((created_by='" . filter_var($app_user['id'],FILTER_SANITIZE_STRING) . "' and reports_type='standard') " . (count(filter_var_array($common_reports_list))>0 ? " or id in(" . implode(',',filter_var_array($common_reports_list)). ")" : "") . ") and find_in_set('" . date('N'). "',notification_days) and find_in_set('" . date('H') . "',notification_time)");
			while($reports = db_fetch_array($reports_query))
			{															
