...
class users_filters
{
  ...
  {
    $this->reports_id = filter_var($reports_id,FILTER_SANITIZE_STRING);
  }
  ...
  function get_choices($add_mepty = false)
  {
    ...
    $filters_query = db_query("select * from app_users_filters where reports_id='" . db_input(filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . "' and users_id='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . "' order by name");
    while($filters = db_fetch_array($filters_query))
    {
      $choices[filter_var($filters['id'],FILTER_SANITIZE_STRING)] = filter_var($filters['name'],FILTER_SANITIZE_STRING);
    }
  ...
  function use_default_filters()
  {
    ...
    $reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING)). "' and reports_type='default'");
    if($reports_info = db_fetch_array($reports_info_query))
    {      
      $filters_query = db_query("select * from app_reports_filters where reports_id='" . db_input(filter_var($reports_info['id'],FILTER_SANITIZE_STRING)) . "'");
      while($filters = db_fetch_array($filters_query))
      {
  ...
  {
  	$reports_info_query = db_query("select * from app_reports where id='" . db_input(filter_var($this->reports_id,FILTER_SANITIZE_STRING)). "'");
  	$reports_info = db_fetch_array($reports_info_query);
  	...
  	if($filters_id=='default')
  	{
        ...
        if($reports_info = db_fetch_array($reports_info_query))
        {            
          db_query("update app_reports set fields_in_listing='" . db_input(filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING)) ."', listing_order_fields='" . db_input(filter_var($reports_info['listing_order_fields'],FILTER_SANITIZE_STRING)) ."'  where id='" . db_input(filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . "'");
        }
  	}
  	else 
  	{
  		$filters_info = db_find('app_users_filters',filter_var($filters_id,FILTER_SANITIZE_STRING));
  		
  		if(strlen($filters_info['fields_in_listing']))
  		{
  			db_query("update app_reports set fields_in_listing='" . db_input(filter_var($filters_info['fields_in_listing'],FILTER_SANITIZE_STRING)) ."' where id='" . db_input(filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . "'");
  		}
  		 
  		if(strlen($filters_info['listing_order_fields']))
  		{
  			db_query("update app_reports set listing_order_fields='" . db_input(filter_var($filters_info['listing_order_fields'],FILTER_SANITIZE_STRING)) ."' where id='" . db_input(filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . "'");
  		}
  	}
