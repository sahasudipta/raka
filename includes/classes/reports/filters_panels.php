...
class filters_panels
{
	...
	static function get_fields_list($entities_id)
	{
		...
		$panels_query = db_query("select * from app_filters_panels where (length(users_groups)=0 or find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ",users_groups)) and is_active=1 and entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' order by sort_order");
		$count_panels = db_num_rows($panels_query);
		while($panels = db_fetch_array($panels_query))
		{
						
			$fields_query = db_query("select * from app_filters_panels_fields where panels_id='" . filter_var($panels['id'],FILTER_SANITIZE_STRING) . "' order by sort_order");
	...
	function render_horizontal()
	{
		...
		$panels_query = db_query("select f.* from app_filters_panels f where (select count(*) from app_filters_panels_fields fp where fp.panels_id=f.id)>0 and f.position='horizontal' and f.type='" . db_input(filter_var($this->type,FILTER_SANITIZE_STRING)) . "' and (length(f.users_groups)=0 or find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ",f.users_groups)) and f.is_active=1 and f.entities_id='" . db_input(filter_var($this->entities_id,FILTER_SANITIZE_STRING)) . "' order by f.sort_order");
		$count_panels = db_num_rows($panels_query);
		while($panels = db_fetch_array($panels_query))
		{			
			$html .= '<ul class="list-inline filters-panels-' . filter_var($panels['id'],FILTER_SANITIZE_STRING) . '">';
			
			$fields_query = db_query("select *, f.type from app_filters_panels_fields fp, app_fields f where f.id=fp.fields_id  and fp.panels_id='" . db_input(filter_var($panels['id'],FILTER_SANITIZE_STRING)) . "' order by fp.sort_order");								
			while($fields = db_fetch_array($fields_query))
			{		
				//check field access
				...
				$html .= '<li>' . $this->render_fields(filter_var_array($fields),filter_var_array($panels)) . '</li>';
			}
			
			if($panels['is_active_filters']==0)
			{
				$html .= '<li><br><a href="javascript: apply_panel_filters(' . filter_var($panels['id'],FILTER_SANITIZE_STRING). ')" class="btn btn-info" title="' . TEXT_SEARCH . '"><i class="fa fa-search" aria-hidden="true"></i> ' . TEXT_SEARCH . '</a></li>';
			}
						
			$html .= '<li><br><a href="javascript: reset_panel_filters' . $this->custom_panel_id . '(' . filter_var($panels['id'],FILTER_SANITIZE_STRING). ')" class="btn btn-default" title="' . TEXT_RESET_FILTERS . '"><i class="fa fa-refresh" aria-hidden="true"></i></a></li>';
	...
	function render_vertical()
	{
		...
		$panels_query = db_query("select * from app_filters_panels where position='vertical' and (length(users_groups)=0 or find_in_set(" .filter_var( $app_user['group_id']) . ",users_groups)) and is_active=1 and entities_id='" . db_input(filter_var($this->entities_id,FILTER_SANITIZE_STRING)) . "' order by sort_order");
		while($panels = db_fetch_array($panels_query))
		{					
			$html .= '
					<div class="filters-panels-' . filter_var($panels['id'] ,FILTER_SANITIZE_STRING). '">
					';
			...
			while($fields = db_fetch_array($fields_query))
			{
				...
				$html .= '<div class="fields-container">' . $this->render_fields(filter_var_array($fields),filter_var_array($panels)) . '</div>';
			}
				
			$html .= '
						<div class="buttons">
							' . ($panels['is_active_filters']==0 ? '<a href="javascript: apply_panel_filters(' . filter_var($panels['id'],FILTER_SANITIZE_STRING). ')" class="btn btn-info" title="' . TEXT_SEARCH . '"><i class="fa fa-search" aria-hidden="true"></i> ' . TEXT_SEARCH . '</a>':'') . '
							<a href="javascript: reset_panel_filters(' . filter_var($panels['id'],FILTER_SANITIZE_STRING). ')" class="btn btn-default" title="' . TEXT_RESET_FILTERS . '"><i class="fa fa-refresh" aria-hidden="true"></i> ' . TEXT_RESET . '</a>
						</div>
					</div>';
	...
	{	
		...
		$field_info_query = db_query("select * from app_fields where id='" . filter_var($panel_field['fields_id'],FILTER_SANITIZE_STRING) . "'");
		...
		$reports_filters_query = db_query("select * from app_reports_filters where fields_id='" . filter_var($field_info['id'],FILTER_SANITIZE_STRING) . "' and reports_id='" . filter_var($reports_id,FILTER_SANITIZE_STRING) . "' and filters_condition!='exclude'");
				...
				<script>			
					$(function(){
						...
						$("' . $this->custom_panel_css . ' .filters-panels-fields").change(function(){
							...
							$.ajax({
								method: "POST",
								url: "' . url_for('reports/filters','action=set_field_fielter_value&reports_id=' . filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . '",
					...
					{						
						...
						$.ajax({
								method: "POST",
								url: "' . url_for('reports/filters','action=set_multiple_fields_fielter_values&reports_id=' . filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . '",
					...
					function delete_field_fielter_value' . $this->custom_panel_id . '(field_id)
					{
						...
						$.ajax({
								method: "POST",
								url: "' . url_for('reports/filters','action=delete_field_fielter_value&reports_id=' . filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . '",
					...
					function reset_panel_filters' . $this->custom_panel_id . '(panel_id)
					{
						...
						$.ajax({
								method: "POST",
								url: "' . url_for('reports/filters','action=reset_panel_filters&reports_id=' . filter_var($this->reports_id,FILTER_SANITIZE_STRING)) . '",
	...
	static function get_report_id_by_field_id($reports_id,$fields_id)
	{
	    ...
	    if($field_info = db_fetch_array($field_info_query))
	    {
    	    ...
    	    {
    	        $report_query = db_query("select id, entities_id from app_reports where id='" . filter_var($report_id,FILTER_SANITIZE_STRING) . "'");
