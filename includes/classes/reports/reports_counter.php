...
class reports_counter
{
	...
	function get_reports_details($report_info)
	{
		...
		if(strlen(filter_var($report_info['in_dashboard_counter_fields'],FILTER_SANITIZE_STRING)))
		{
			$sum_by_fields = explode(',',filter_var($report_info['in_dashboard_counter_fields'],FILTER_SANITIZE_STRING));
		}
			
		if(filter_var($report_info['dashboard_counter_sum_by_field'],FILTER_SANITIZE_STRING)>0)
		{
			$sum_by_fields[] = filter_var($report_info['dashboard_counter_sum_by_field'],FILTER_SANITIZE_STRING);
		}
		...
		$listing_sql_query_select = fieldtype_formula::prepare_query_select(filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING), $listing_sql_query_select,false,array('fields_in_listing'=>implode(',',filter_var_array($sum_by_fields)),'reports_id'=>filter_var($report_info['id'],FILTER_SANITIZE_STRING)));
		...
		{
			$listing_sql_query .= " and e.parent_item_id='" . filter_var($this->parent_item_id,FILTER_SANITIZE_STRING) . "'";
		}
	
		//check view assigned only access
		$listing_sql_query = items::add_access_query(filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING),$listing_sql_query, filter_var($report_info['displays_assigned_only'],FILTER_SANITIZE_STRING));
		...
		{			
			...
			$fields_query = db_query("select f.* from app_fields f, app_forms_tabs t  where f.id in (" . implode(',',filter_var_array($sum_by_fields)). ") and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
	...
	function reports_query()
	{
		...
		{
			$where_sql = " and r.id not in (" . filter_var($app_users_cfg->get('hidden_common_reports'),FILTER_SANITIZE_STRING) . ")";
		}
		
		//get common reports list
		$common_reports_list = array();
		$reports_query = db_query("select r.* from app_reports r, app_entities e, app_entities_access ea  where r.entities_id = e.id and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input(filter_var($app_user['group_id'],FILTER_SANITIZE_STRING)) . "' and (find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ",r.users_groups) or find_in_set(" . (filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . ",r.assigned_to)) and r.in_dashboard_counter=1 and r.reports_type = 'common' " . $where_sql . " order by r.dashboard_sort_order, r.name");
		while($reports = db_fetch_array($reports_query))
		{
		...
		$reports_query = "select r.*,e.name as entities_name,e.parent_id as entities_parent_id from app_reports r, app_entities e where e.id=r.entities_id and ((r.created_by='" . db_input(filter_var($app_logged_users_id,FILTER_SANITIZE_STRING)) . "' and r.reports_type='standard' and  r.in_dashboard_counter=1)  " . (count(filter_var_array($common_reports_list))>0 ? " or r.id in(" . implode(',',filter_var_array($common_reports_list)). ")" : "") . ") order by r.dashboard_counter_sort_order, r.dashboard_sort_order, r.name";
