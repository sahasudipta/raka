...
class hot_reports
{
  ...
  function get_items($report_info,$options = array())
  {
  	...
  	//get heading field
  	$field_heading_id = fields::get_heading_id(filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING));
    ...
    $listing_sql_query_select = fieldtype_formula::prepare_query_select(filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING), $listing_sql_query_select,false,array('reports_id'=>filter_var($report_info['id'],FILTER_SANITIZE_STRING),'fields_in_listing'=>$field_heading_id));
    ...
    if(isset($sql_query_having[filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING)]))
    {
    ...
    $listing_sql_query = items::add_access_query(filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING),$listing_sql_query, $report_info['displays_assigned_only']);
    
    //add having query
    $listing_sql_query .= $listing_sql_query_having;
    
    //prepare order query
    $info = reports::add_order_query(filter_var($report_info['listing_order_fields'],FILTER_SANITIZE_STRING),filter_var($report_info['entities_id'],FILTER_SANITIZE_STRING));
    ...
    {    	
      $path_info = items::get_path_info($report_info['entities_id'],filter_var($item['id'],FILTER_SANITIZE_STRING));
      ...
      $items_array[] = array('id'   => filter_var($item['id'],FILTER_SANITIZE_STRING),
  ...
  function render_dropdown($id)
  {
    ...
    $html = '';
    $report_info_query = db_query("select * from app_reports where id='" . db_input(filter_var($id,FILTER_SANITIZE_STRING)) . "'");
    if($report_info = db_fetch_array($report_info_query))
    {
    	$cahce_filename = 'user-' . filter_var($app_user['id'],FILTER_SANITIZE_STRING) . '-report-' . filter_var($report_info['id'],FILTER_SANITIZE_STRING);
    	$cache_lifetime = ((filter_var($report_info['in_header'],FILTER_SANITIZE_STRING) and filter_var($report_info['in_header_autoupdate'],FILTER_SANITIZE_STRING)) ? 60 : CFG_CACHE_REPORTS_IN_HEADER_LIFETIME);
    	...
    	if(!app_read_cache($html, $cahce_filename, $cache_lifetime,CFG_USE_CACHE_REPORTS_IN_HEADER))
    	{
	      ...
	      $items_info = $this->get_items(filter_var_array($report_info));
	      ...
	      foreach($items_info['items_array'] as $v)
	      {
	        $items_html .= '
	          <li>
	  					<a href="' . url_for('items/info','path=' . filter_var($v['path'],FILTER_SANITIZE_STRING)) . '">' . filter_var($v['name'],FILTER_SANITIZE_STRING) . '</a>
	  				</li>
	        ';
	      }
	      ...
	      if($items_info['items_count']>0)
	      {
	        $external_html = '
	          <li class="external">
							<a href="' . url_for('reports/view','reports_id=' . filter_var($report_info['id'],FILTER_SANITIZE_STRING)) . '">' . sprintf(TEXT_DISPLAY_NUMBER_OF_ITEMS_OPEN_REPORT,count($items_info['items_array'])) . '</a>
						</li>
	        ';
	      }
	      ...
	      $badge_html = ($items_info['items_count']>0 ? '<span class="badge badge-warning">' . filter_var($items_info['items_count'],FILTER_SANITIZE_STRING) . '</span>' : '');
	      
	      $menu_icon = (strlen(filter_var($report_info['menu_icon'],FILTER_SANITIZE_STRING)) ? filter_var($report_info['menu_icon'],FILTER_SANITIZE_STRING) : (strlen($entity_cfg['menu_icon'])>0 ? $entity_cfg['menu_icon'] : 'fa-reorder') );
	      
	      $html = '
	        <a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">					  
					  ' . app_render_icon($menu_icon) . '
					  ' . $badge_html . '
					</a>
					<ul class="dropdown-menu extended tasks">
						<li style="cursor:pointer" onClick="location.href=\'' . url_for('reports/view','reports_id=' . htmlentities(filter_var($report_info['id'],FILTER_SANITIZE_STRING))) . '\'">
							<p>' . filter_var($report_info['name'],FILTER_SANITIZE_STRING) . '</p>
						</li>
						<li>
  ...
  function reports_query()
  {
    ...
    $reports_query = db_query("select r.* from app_reports r, app_entities e, app_entities_access ea  where r.entities_id = e.id and e.id=ea.entities_id and length(ea.access_schema)>0 and ea.access_groups_id='" . db_input($app_user['group_id']) . "' and (find_in_set(" . filter_var($app_user['group_id'],FILTER_SANITIZE_STRING) . ",r.users_groups) or find_in_set(" . filter_var($app_user['id'],FILTER_SANITIZE_STRING) . ",r.assigned_to)) and r.in_header=1 and r.reports_type = 'common' " . $where_sql . " order by r.dashboard_sort_order, r.name");
    while($reports = db_fetch_array($reports_query))
    {
      $common_reports_list[] = filter_var($reports['id'],FILTER_SANITIZE_STRING);
    }
