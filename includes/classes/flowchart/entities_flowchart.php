...
class entities_flowchart
{
  ...
  {  	  	
  	...
  	while($entities = db_fetch_array($entities_query))
  	{
  		...
  		$count_fields = 0;
  		$check_fields_query = db_query("select * from app_fields where entities_id = '" . filter_var($entities['id'],FILTER_SANITIZE_STRING) . "' and type in ('fieldtype_users','fieldtype_users_ajax','fieldtype_grouped_users','fieldtype_entity','fieldtype_entity_ajax','fieldtype_entity_multilevel','fieldtype_related_records','fieldtype_formula')");
  ...
  function build_related_records_nodes()
  {
  	...
  	{  	  		  
  		...
  		$check_query = db_query("select * from app_fields where type in ('fieldtype_related_records') and entities_id='" . filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING) . "'");
