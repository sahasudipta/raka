...
class processes_flowchart
{
	...
	{
		$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($process['entities_id'],FILTER_SANITIZE_STRING)). "' and reports_type='process" . filter_var($process['id'],FILTER_SANITIZE_STRING) . "'");
		if($reports_info = db_fetch_array($reports_info_query))
		{
			$filters_title = '';
			$filters_query = db_query("select rf.*, f.name, f.type from app_reports_filters rf left join app_fields f on rf.fields_id=f.id where rf.reports_id='" . db_input(filter_var($reports_info['id'],FILTER_SANITIZE_STRING)) . "' order by rf.id");
	...
	function prepare_process_actions_filter_node($actions)
	{
		...
		$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($action_entity_id,FILTER_SANITIZE_STRING)). "' and reports_type='process_action" . filter_var($actions['id'],FILTER_SANITIZE_STRING) . "'");
		if($reports_info = db_fetch_array($reports_info_query))
		{
			$filters_title = '';
			$filters_query = db_query("select rf.*, f.name, f.type from app_reports_filters rf left join app_fields f on rf.fields_id=f.id where rf.reports_id='" . db_input(filter_var($reports_info['id'],FILTER_SANITIZE_STRING)) . "' order by rf.id");
