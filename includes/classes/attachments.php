...
class attachments
{
	public static function delete_in_queue($field_id, $filename)
	{
      ...
      //delete from queue 
      db_query("delete from app_attachments where filename='" . db_input(filter_var($filename,FILTER_SANITIZE_STRING)) . "'");
  ...
  {    
    ...
    $field_query = db_query("select id, name, configuration from app_fields where id='" . db_input(filter_var($field_id,FILTER_SANITIZE_STRING)) . "'");
    ...
    if(count($attachments_list)>0)
    {
      ...
      <table class="table table-striped table-hover">
        <tbody>'; 
      foreach(filter_var_array($attachments_list) as $k=>$v)
      {
        $file = attachments::parse_filename($v);
        
        $row_id = 'attachments_row_' . filter_var($field_id,FILTER_SANITIZE_STRING). '_' . $k;
        
        if($has_delte_access)
        {	
	        $html .= '
	          <tr class="' . $row_id . '">
	            <td width="100%">' . app_crop_str(filter_var($file['name'],FILTER_SANITIZE_STRING)) . '<small>' . fieldtype_attachments::add_file_date_added($file,$cfg). '</small></td>
	            <td align="right"><label class="checkbox delete_attachments">
	            		<i class="fa fa-trash-o pointer delete_attachments_checkbox" data-confirm-delation="' . $cfg->get('confirm_delation') . '" data-filename="' . filter_var($file['file'],FILTER_SANITIZE_STRING) . '" data-row-id="' . $row_id . '" title="' . TEXT_DELETE . '"></i></label>
	            </td>
	          </tr>
	        ';
        }
        else
        {
        	$html .= '
	          <tr class="' . $row_id . '">
	            <td width="100%" style="padding-bottom: 5px;">' . filter_var($file['name'],FILTER_SANITIZE_STRING) . '<small>' . fieldtype_attachments::add_file_date_added(filter_var_array($file),$cfg). '</small></td>	           
	          </tr>
	        ';
        }
      }
      ...
      <script>
      		appHandleAttachmentsDelete(\'' . filter_var($field_id,FILTER_SANITIZE_STRING) . '\',\'' . $delete_file_url . '\',\'' . $app_session_token . '\');
      		appHandleUniformCheckbox();
      </script>
      ';
      
      $html .= input_hidden_tag('fields[' . filter_var($field_id,FILTER_SANITIZE_STRING) . ']',implode(',',$attachments_list));
            
    }
    else
    {
    	$field = db_find('app_fields',filter_var($field_id,FILTER_SANITIZE_STRING));
    	
    	if($field['is_required'])
    	{
    		$html .= input_hidden_tag('fields[' . filter_var($field_id,FILTER_SANITIZE_STRING) . ']','',array('class'=>'form-control field_' . filter_var($field['id'],FILTER_SANITIZE_STRING) . ' required'));
    	}
    }
  ...
  {
    $comments_query = db_query("select * from app_comments where id='" . db_input(filter_var($comments_id,FILTER_SANITIZE_STRING)) . "' and length(attachments)>0");
