...
class users_cfg
{
  ...
  {
  	$cfg_query = db_query("select * from app_users_configuration where users_id='" . db_input(filter_var($users_id,FILTER_SANITIZE_STRING)) . "' and configuration_name='" . db_input(filter_var($key,FILTER_SANITIZE_STRING)) . "'");
  	if($cfg = db_fetch_array($cfg_query))
  	{
  		return filter_var($cfg['configuration_value'],FILTER_SANITIZE_STRING);
  	}
  	else 
  	{
  		return $default;
