...
class users_notifications
{
	...
	static function render_dropdown()
	{
		...
		$itmes_query = db_query("select * from app_users_notifications where users_id='" . db_input(filter_var($app_user['id'],FILTER_SANITIZE_STRING)) . "' order by id desc limit " . $poup_items_limit);
		while($itmes = db_fetch_array($itmes_query))
		{
			$path_info = items::get_path_info(filter_var($itmes['entities_id'],FILTER_SANITIZE_STRING),filter_var($itmes['items_id'],FILTER_SANITIZE_STRING));
			
			$items_html .= '
          <li>
  					<a href="' . url_for('items/info','path=' . $path_info['full_path']) . '">' . self::render_icon_by_type(filter_var($itmes['type'],FILTER_SANITIZE_STRING)) . ' ' . filter_var($itmes['name'],FILTER_SANITIZE_STRING) . ' <span class="parent-name"><i class="fa fa-angle-left"></i>' . (isset($app_users_cache[filter_var($itmes['created_by'],FILTER_SANITIZE_STRING)]) ? $app_users_cache[filter_var($itmes['created_by'],FILTER_SANITIZE_STRING)]['name'] : '') . '</span></a>
  				</li>
        ';
		...
		$itmes_count = db_count('app_users_notifications',filter_var($app_user['id'],FILTER_SANITIZE_STRING),'users_id');
		...
		$badge_html = ($itmes_count>0 ? '<span class="badge badge-warning">' . filter_var($itmes_count,FILTER_SANITIZE_STRING) . '</span>' : '');
