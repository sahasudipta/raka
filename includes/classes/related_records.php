...
class related_records
{
  ...
  function render_as_single_list($as_single_list = true)
  {
    ...
    $fields_access_schema = users::get_fields_access_schema(filter_var($this->entities_id,FILTER_SANITIZE_STRING),filter_var($app_user['group_id'],FILTER_SANITIZE_STRING));
    
    $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in ('fieldtype_related_records') and f.entities_id='" . db_input(filter_var($this->entities_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id  order by t.sort_order, t.name, f.sort_order, f.name");
    while($field = db_fetch_array($fields_query))
    { 
    
      $this->cfg = new fields_types_cfg(filter_var($field['configuration'],FILTER_SANITIZE_STRING));
      ...
      {
      	$current_field_access = $fields_access_schema[filter_var($field['id'],FILTER_SANITIZE_STRING)];
      	
        if($fields_access_schema[filter_var($field['id'],FILTER_SANITIZE_STRING)]=='hide') continue;
      }
                                    
      $this->entities_access_schema = users::get_entities_access_schema(filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING),filter_var($app_user['group_id'],FILTER_SANITIZE_STRING));
  ...
  static function get_report_info($field_info)
  {
  	...
  	$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)). "' and reports_type='" . filter_var($reports_type,FILTER_SANITIZE_STRING) . "'");
  ...
  function render_single_list($current_field_access)
  {
    ...
    if($this->cfg->get('hide_field_without_records')==1 and $count_related_items==0) return '';
    	    
    $reports_info = self::get_report_info($this->field);
    
    $listing_container = 'entity_items_listing' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) . '_' .  filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING);
    
    $entity_cfg = new entities_cfg(filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING));
    
    $with_selected_menu = '';
    	
    if(users::has_access('export_selected',$this->entities_access_schema) and users::has_access('export',$this->entities_access_schema))
    {
    	$with_selected_menu .= '<li>' . link_to_modalbox('<i class="fa fa-file-excel-o"></i> ' . TEXT_EXPORT,url_for('items/export','path=' . filter_var($reports_info["entities_id"],FILTER_SANITIZE_STRING)  . '&reports_id=' . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) )) . '</li>';
    }
    
    $with_selected_menu .=  plugins::include_dashboard_with_selected_menu_items(filter_var($reports_info['id'],FILTER_SANITIZE_STRING),'&path=' . $app_path . '/' .  filter_var($reports_info['entities_id'],FILTER_SANITIZE_STRING) . '&redirect_to=parent_item_info_page');
    ...
    if(users::has_access('update',$this->current_entities_access_schema) and $current_field_access!='view')
    {
    	...
    	{
	    	//link button    	
	    	$html_btn .= button_tag('<i class="fa fa-link"></i>', url_for('items/link_related_item','path=' . $current_path . '&related_entities=' . filter_var($this->cfg->get('entity_id') ,FILTER_SANITIZE_STRING). '&field_id=' . filter_var($this->field['id'],FILTER_SANITIZE_STRING)),true,array('class'=>'btn btn-primary btn-sm', 'title'=>TEXT_BUTTON_LINK)) . ' ';
	    	
	    	//unlink button
	    	if($count_related_items>0)
	    	{
	    		$html_btn .= button_tag('<i class="fa fa-unlink"></i>',url_for('items/unlink_related_item','path=' . $current_path . '&fields_id=' . filter_var($this->field['id'] ,FILTER_SANITIZE_STRING). '&related_entities_id=' . filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING)), true, array('class'=>'btn btn-primary btn-sm', 'title'=>TEXT_UNLINK));
	    	}
    	}
    ...
    $html = '
    <div class="portlet portlet-related-items form-group-' . filter_var($this->field['id'],FILTER_SANITIZE_STRING) . '">
			<div class="portlet-title">
				<div class="caption">        
          ' . fields_types::get_option(filter_var($this->field['type'],FILTER_SANITIZE_STRING),'name',filter_var($this->field['name'],FILTER_SANITIZE_STRING)) . '              
        </div>
        <div class="tools">
...
//if parent items are different    
  ...
  {        
    ...
    $related_items_query = db_query("select * from " . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . " where entity_" . filter_var($this->entities_id,FILTER_SANITIZE_STRING) . "_items_id='" . db_input(filter_var($this->items_id,FILTER_SANITIZE_STRING)) . "'");
    ...
    {
    	$related_items_query = db_query("select * from " . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . " where entity_" . filter_var($this->entities_id,FILTER_SANITIZE_STRING) . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . "_items_id='" . db_input(filter_var($this->items_id,FILTER_SANITIZE_STRING)) . "'");
  ...
  {                
    ...
    {  
    	...
    	$listing_sql_query = " and e.id in (" . implode(',',filter_var_array($related_items)) . ")";
          	    	
      //check view assigned only access
      $listing_sql_query = items::add_access_query(filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING),$listing_sql_query);
      
      //include access to parent records
      $listing_sql_query .= items::add_access_query_for_parent_entities(filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING));
        
      $listing_sql = "select count(e.id) as total from app_entity_" . filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " e where e.id>0 " . $listing_sql_query;
  ...
  function render_list_in_listing($options)
  {
  	...
  	if(count($related_items)>0)
  	{
  		...
  		$listing_sql = "select e.* " . fieldtype_formula::prepare_query_select(filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($this->cfg->get('entity_id'),FILTER_SANITIZE_STRING) . " e where e.id>0 " . $listing_sql_query;
  ...
  public static function delete_related_by_item_id($entities_id,$items_id)
  {
  	...
  	{  	
  		...
  		if($cfg->get('entity_id')>0)
  		{
		  	...
		  	db_query("delete from " . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . " where entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "_items_id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");
				
		  	if(strlen($table_info['sufix'])>0)
		  	{
		  		db_query("delete from " . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . " where entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . "_items_id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");  		
		  	}
  		}
  	}
  ...
  public static function get_fields_choices_available_to_relate_to_entity($entities_id)
  {
    ...
    while($fields = db_fetch_array($fields_query))
    {
      ...
      {  
        ...
        {
            $check = false;
            $field_check_query = db_query("select id,configuration from app_fields where type='fieldtype_related_records' and entities_id = '" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "'");
  ...
  public static function prepare_entities_related_items_table($entities_id, $fields_id)
  {
  	...
  	if($field['type']=='fieldtype_related_records')
  	{
  		...
  		{  			
  			...
  			{
  				$sql = '
		          CREATE TABLE IF NOT EXISTS `' . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . '` (
		            `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT,
		            `entity_' .filter_var($entities_id,FILTER_SANITIZE_STRING) .  '_items_id` int(11) UNSIGNED NOT NULL,
		            `entity_' . filter_var($related_entities_id,FILTER_SANITIZE_STRING) . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . '_items_id` int(11) UNSIGNED NOT NULL,
		            PRIMARY KEY (`id`),
		            KEY `idx_' . filter_var($entities_id,FILTER_SANITIZE_STRING) . '_items_id` (`entity_' . filter_var($entities_id,FILTER_SANITIZE_STRING) . '_items_id`),
		            KEY `idx_' . filter_var($related_entities_id,FILTER_SANITIZE_STRING)  . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . '_items_id` (`entity_' . filter_var($related_entities_id,FILTER_SANITIZE_STRING)  . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . '_items_id`)
  ...
  public static function delete_entities_related_items_table($entities_id)
  {
  	...
  	while($tables = db_fetch_array($tables_query))
  	{
  		$tables_array[] =  current(filter_var_array($tables));
  	} 
  	
  	foreach(filter_var_array($tables_array) as $table)
  	{
