...
class fieldtype_entity
{
  ...
  {  	
  	...
  	{
  		$listing_sql_query .= " and {$previous_prefix}.parent_item_id in (select {$prefix}.id from app_entity_" . $entity_id . " {$prefix} where {$prefix}.id>0 " . self::prepare_parents_sql($parent_entity_item_id, filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING), $field_entity_id, $listing_sql_query,$prefix) . ")";
  	}
  ...
  static function get_choices($field,$params = array(),$value = '',&$parent_entity_item_is_the_same)
  {
  	...
  	{
  		$listing_sql_query = $listing_sql_query . self::prepare_parents_sql($parent_entity_item_id,filter_var($entity_info['parent_id'],FILTER_SANITIZE_STRING),filter_var($field_entity_info['parent_id'],FILTER_SANITIZE_STRING));
  	}
  	
  	if($cfg->get('display_assigned_records_only')==1)
  	{
  		$listing_sql_query = items::add_access_query(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING),$listing_sql_query);
  	}
  	else 
  	{
  		//add visibility access query
  		$listing_sql_query .= records_visibility::add_access_query(db_input($cfg->get('entity_id')));
  	}
  	
  	$default_reports_query = db_query("select * from app_reports where entities_id='" . db_input(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING)). "' and reports_type='entityfield" . filter_var($field['id'],FILTER_SANITIZE_STRING) . "'");
  	if($default_reports = db_fetch_array($default_reports_query))
  	{
  		$listing_sql_select = fieldtype_formula::prepare_query_select(db_input($cfg->get('entity_id')),'');
  		...
  		{
  			$listing_sql_query_having  = reports::prepare_filters_having_query($sql_query_having[filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING)]);
  		}
  	...
  	if(strlen($value))
  	{
  		$listing_sql_query = "(" . $listing_sql_query . ") or e.id in (" . filter_var($value,FILTER_SANITIZE_STRING) . ") ";
  	}
  	
  	$field_heading_id = 0;
  	$fields_query = db_query("select f.* from app_fields f where f.is_heading=1 and  f.entities_id='" . db_input(filter_var($cfg->get('entity_id'),FILTER_SANITIZE_STRING)) . "'");
  	if($fields = db_fetch_array($fields_query))
  	{
  		$field_heading_id = $fields['id'];
  	}
  	
  	$listing_sql = "select  e.* " . $listing_sql_select . " from app_entity_" . db_input($cfg->get('entity_id')) . " e "  . $listing_sql_query_join . " where " . $listing_sql_query . $listing_sql_query_having . $listing_sql_query_order;
  	$items_query = db_query($listing_sql, false);
  	while($item = db_fetch_array($items_query))
  	{
  		if($cfg->get('entity_id')==1)
  		{
  			$choices[filer_var($item['id'],FILTER_SANITIZE_STRING)] = $app_users_cache[filter_var($item['id'],FILTER_SANITIZE_STRING)]['name'];
  		}
  ...
  function render($field,$obj,$params = array())
  {
    ...
    {    	    	
      ...
      return '<table><tr><td>' . select_tag('fields[' . $field['id'] . ']',$choices,$value,$attributes)  . '</td><td valign="bottom">' . $button_add_html . '</td></tr></table>' . fields_types::custom_error_handler($field['id']);
    }
    ...
    elseif($cfg->get('display_as')=='dropdown_muliple')
    {
      ...
      return '
          <table>
              <tr>
                <td>' .select_tag('fields[' . $field['id'] . '][]',$choices,explode(',',$value),$attributes) . '</td><td valign="top">' . $button_add_html . '</td>
              </tr>
          </table>' . fields_types::custom_error_handler($field['id']);
    }
  }
