  	...
  	switch($name)
  	{
  		case 'fields_for_search_box':
  			  $entities_id = $value;
  				...
  				$choices = [];
  				$choices[''] = '';
  				$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in ('fieldtype_entity','fieldtype_entity_ajax','fieldtype_entity_multilevel') and  f.entities_id='" . db_input(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
  				while($fields = db_fetch_array($fields_query))
  				{  					
    ...
    for($i=0;$i<count($entities_levels);$i++)
    {
    	...
    	{    		
    		...
    		$html .= '
    				<table>
    					<tr>
    						<td><p class="slect2_tag" style="margin: 0">' . select_tag($field_name,$choices,$value,$attributes) . '</p></td>
    						<td>' . $button_add_html . '</td>
    					</tr>
    				</table>';
    	}
    	else
