...
class fieldtype_entity_ajax
{
  	...
  	switch($name)
  	{
  		case 'fields_for_search_box':
  			  $entities_id = $value;
  				...
  				$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where is_heading = 0 and f.type not in ('fieldtype_action','fieldtype_parent_item_id') and  f.entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
  				...
  				$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in (" . fields_types::get_types_for_search_list(). ") and  f.entities_id='" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
  				while($fields = db_fetch_array($fields_query))
  				{
  				...
  				$cfg[] = array(
  						...
  						'<div style="padding-top: 2px;">' . fields::get_available_fields_helper(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING), 'fields_configuration_copy_values',entities::get_name_by_id(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING))) . '</div>', 
  ...
  {      
      ...
      $mysql_query_where = ' and (' . filter_var($cfg->get('mysql_query_where')) . ')';
      
      if($parent_entity_item_id>0 and $app_entities_cache[filter_var($field['entities_id'],FILTER_SANITIZE_STRING)]['parent_id']>0)
      {
          $item_info_query = db_query("select * from app_entity_" . $app_entities_cache[filter_var($field['entities_id'],FILTER_SANITIZE_STRING)]['parent_id'] . " where id=" . db_input(filter_var($parent_entity_item_id,FILTER_SANITIZE_STRING)));
          if($item_info = db_fetch_array($item_info_query))
          {
              ...
              {
                  $item_info_query = db_query("select * from app_entity_" . $app_entities_cache[$parent_entity_id]['parent_id'] . " where id=" . filter_var($item_info['parent_item_id'],FILTER_SANITIZE_STRING));
                  if($item_info = db_fetch_array($item_info_query))
                  {
                      foreach(filter_var_array($item_info) as $k=>$v)
                      {
      ...
      if(isset($_POST['form_data']))
      {
          ...
          foreach(filter_var_array($_POST['form_data']) as $k=>$v)
          {
              $key = str_replace(['fields[',']'],'',filter_var($v['name'],FILTER_SANITIZE_STRING));
              $mysql_query_where = str_replace('[' . $key . ']',filter_var($v['value'],FILTER_SANITIZE_STRING),$mysql_query_where);
          }
      }
