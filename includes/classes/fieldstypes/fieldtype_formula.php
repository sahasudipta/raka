...
class fieldtype_formula
{
  ...
  function get_configuration()
  {
    ...
    $cfg[] = array('title'=>TEXT_FORMULA . fields::get_available_fields_helper(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING), 'fields_configuration_formula'), 'name'=>'formula','type'=>'textarea','tooltip_icon'=>TEXT_FORMULA_TIP_USAGE,'tooltip'=>TEXT_FORMULA_TIP,'params'=>array('class'=>'form-control code'));
  ...
  {  	 
  	...
  	if(isset($check_needed['reports_id']))
  	{
  		...
  		{	
  			...
  			{	
	  			$reports_info_query = db_query("select id, entities_id, fields_in_listing, listing_type from app_reports where id='" . db_input(filter_var($check_needed['reports_id'],FILTER_SANITIZE_STRING)). "'");
	  		...
	  		{
	  			$fields_in_listing = [];
	  			$listing_type_query = db_query("select id from app_listing_types where entities_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "' and type='" . filter_var($reports_info['listing_type'],FILTER_SANITIZE_STRING) . "'");
	  			if($listing_type = db_fetch_array($listing_type_query))
	  			{
	  				
	  				$listing_sections_query = db_query("select fields from app_listing_sections where listing_types_id={".filter_var($listing_type['id'],FILTER_SANITIZE_STRING)."} order by sort_order, name");
	  				while($listing_sections = db_fetch_array($listing_sections_query))
	  				{  					
	  					if(strlen(filter_var($listing_sections['fields'],FILTER_SANITIZE_STRING))) $fields_in_listing = array_merge($fields_in_listing,explode(',',filter_var($listing_sections['fields'],FILTER_SANITIZE_STRING))); 
	  				}
	  			}
	  		...
	  		$reports_info_holder[$check_needed['reports_id']] = filter_var_array($reports_info);
  		}
  		else
  		{
  	...
  	elseif(isset($check_needed['fields_in_listing']))
  	{
  		...
  		{
  			$text_pattern_where_sql = " and id in (" . filter_var($check_needed['fields_in_listing'],FILTER_SANITIZE_STRING) . ")";
  		}
  	}
  	//check reports settings
  	elseif(strlen($reports_info['fields_in_listing']))
  	{
  		if(in_array($formula_fields_id,explode(',',filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING))))
  		{
  			$check_formula_needed = true;
  		}
  		
  		$text_pattern_where_sql = " and id in (" . filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING) . ")";
  	}
  	//check default listig settings
  	else  
  	{  		
  		$check_query = db_query("select id from app_fields where id='" . db_input($formula_fields_id) . "' and listing_status=1");
  	...
  	{
  		$check_query = db_query("select count(*) as total from app_reports_filters where reports_id='" . db_input(filter_var($check_needed['reports_id'],FILTER_SANITIZE_STRING)) . "' and fields_id='" . db_input($formula_fields_id) . "'");
  	...
  	{	  		
	  	$fields_query = db_query("select configuration from app_fields where entities_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "' {$text_pattern_where_sql} and type='fieldtype_text_pattern'");
  ...
  {  
    ...
    {
    	$available_fields = $app_not_formula_fields_cache[filter_var($entities_id,FILTER_SANITIZE_STRING)];
    }
    ...
    if(isset($app_formula_fields_cache[$entities_id]))
    {
    	...
    	foreach($app_formula_fields_cache[filter_var($entities_id,FILTER_SANITIZE_STRING)] as $fields)
    	{
	    ...
	    {	    		    	
	      ...
	      if($check_needed)
	      {
	      	if(!self::check_formula_query_needed(filter_var($fields['id'],FILTER_SANITIZE_STRING),filter_var($entities_id,FILTER_SANITIZE_STRING), $check_needed))
	      	{
	      		continue;
	      	}
	      }
	      ...
	      if(strlen($formula)>0)
	      {
	      	...
	      	$formula = self::prepare_formula_fields(filter_var_array($formulas_fields), $formula);
	        ...
	        //handle get_vallue()
	        $formula = self::perpare_choices_get_value_function(filter_var($entities_id,FILTER_SANITIZE_STRING), $formula);
	        ...
	        {	        		        	
	        	foreach(filter_var_array($app_currencies_cache) as $currecny)
	        	{
	        ...
	        {	        	
	          $formula = functions::prepare_formula_query(filter_var($entities_id,FILTER_SANITIZE_STRING), $formula);
	        }
	        ...
	        {    
	          ...
	          {
	            $listing_sql_query_select .= ", sum(" . $formula . ") as sum_field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING) . " ";          	
	          }
	          elseif(is_array($listing_sql_query_select))
	          {
	            $listing_sql_query_select[] = "(" . $formula . ") as field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING); 
	          }
	          else
	          {    
	            $listing_sql_query_select .= ", (" . $formula . ") as field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING);
	        ...
	        {        	
	          echo '<div class="alert alert-danger">' . sprintf(TEXT_ERROR_FORMULA_CALCULATION,$app_entities_cache[$entities_id]['name'],filter_var($fields['name'],FILTER_SANITIZE_STRING), filter_var($fields['id'],FILTER_SANITIZE_STRING), $cfg->get('formula')) .  '</div>';
	        }
	      }
	    }	    
    }  
    ...
    $listing_sql_query_select = fieldtype_mysql_query::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),$listing_sql_query_select);
    
    //prepare days diff query field type in main query
    $listing_sql_query_select = fieldtype_days_difference::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),$listing_sql_query_select);
    
    //prepare hours diff query field type in main query
    $listing_sql_query_select = fieldtype_hours_difference::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),$listing_sql_query_select);
    
    //prepare years diff query field type in main query
    $listing_sql_query_select = fieldtype_years_difference::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),$listing_sql_query_select);
    
    //prepare months diff query field type in main query
    $listing_sql_query_select = fieldtype_months_difference::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),$listing_sql_query_select);
  ...
  public static function prepare_formula_fields($formulas_fields, $formula)
  {
  	...
  	{  	
	  	...
	  	foreach(filter_var_array($formulas_fields) as $fields_id=>$formula_text)
	  	{
  ...
  public static function perpare_choices_get_value_function($entities_id, $formula,$prefix =  'e')
  {
  	...
  	{  		  		
  		...
  		{  		
  			...
  			$field_query = db_query("select type from app_fields where id='"  . db_input(filter_var($field_id,FILTER_SANITIZE_STRING)) . "'");
  			if($field = db_fetch_array($field_query))
  			{
  				...
  				switch($field['type'])
  				{
  					...
  					case 'fieldtype_radioboxes':  						
  						$formula = str_replace("get_value({$prefix}.field_" . filter_var($field_id,FILTER_SANITIZE_STRING),"(select fcv.value from app_fields_choices fcv where fcv.id = {$prefix}.field_" . filter_var($field_id,FILTER_SANITIZE_STRING), $formula);  						
  						break;
  					default:
