...
class fieldtype_dynamic_date
{
  ...
  function get_configuration()
  {
    ...
    $cfg[TEXT_SETTINGS][] = array('title'=>TEXT_FORMULA . fields::get_available_fields_helper(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING), 'fields_configuration_formula'), 'name'=>'formula','type'=>'textarea','tooltip_icon'=>TEXT_FORMULA_TIP_USAGE,'tooltip'=>TEXT_FORMULA_TIP,'params'=>array('class'=>'form-control'));
    ...
    $fields_query = db_query("select * from app_fields where type in ('fieldtype_stages','fieldtype_dropdown','fieldtype_radioboxes','fieldtype_dropdown_multiple','fieldtype_tags','fieldtype_checkboxes') and entities_id='" . db_input(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING)) . "'");
  	...
  	switch($name)
  	{
  		case 'disable_color_by_field_values':
  			if(strlen($value))
  			{
  				...
  				{
  					$field_cfg = new fields_types_cfg(filter_var($field['configuration'],FILTER_SANITIZE_STRING));
  					...
  					{
  						$choices = fields_choices::get_choices(filter_var($field['id'],FILTER_SANITIZE_STRING),false);
  					}
  						
  					$cfg[] = array(
  							'title'=>filter_var($field['name'],FILTER_SANITIZE_STRING),
  ...
  public static function update_items_fields($entities_id, $items_id)
  {
  	...
  	{
  		$item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING),'') . " from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . " e where e.id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");
  		$item_info = db_fetch_array($item_info_query);
  		//print_r($item_info);
  		//exit();
  
  		foreach(filter_var_array($update_fields) as $fields_id)
  		{
