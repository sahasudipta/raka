...
class fieldtype_parent_value
{
	...
	function get_configuration()
	{
		...
		if($entities_info['parent_id']>0)
		{
			...
			$fields_query = db_query("select f.*, t.name as tab_name, if(f.type in (" . $reserverd_fields_types_list . "),-1,t.sort_order) as tab_sort_order from app_fields f, app_forms_tabs t where f.type not in ('fieldtype_action','fieldtype_parent_item_id') and f.entities_id='" . filter_var($entities_info['parent_id'],FILTER_SANITIZE_STRING) . "' and f.forms_tabs_id=t.id order by tab_sort_order, t.name, f.sort_order, f.name");
			while($fields = db_fetch_array($fields_query))
			{
				$choices[filter_var($fields['tab_name'],FILTER_SANITIZE_STRING)][filter_var($fields['id'],FILTER_SANITIZE_STRING)] = (strlen(filter_var($fields['name'],FILTER_SANITIZE_STRING)) ? filter_var($fields['name'],FILTER_SANITIZE_STRING):fields_types::get_title(filter_var($fields['type'],FILTER_SANITIZE_STRING)));
			}				
		}
	...
	function output($options)
	{
		...
		if($parent_item_id>0 and $entities_info['parent_id']>0)
		{
			...
			if(!isset($parent_items_values_cache[$parent_item_id]))
			{
				...
				{
					$paretn_item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entities_info['parent_id'],FILTER_SANITIZE_STRING), '',false,array('fields_in_listing'=>implode(',',filter_var_array($select_fields)))) . " from app_entity_" . filter_var($entities_info['parent_id'],FILTER_SANITIZE_STRING) . " e  where e.id='" . db_input($parent_item_id) . "'");
