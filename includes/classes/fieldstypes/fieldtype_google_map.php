  	...
  	if(isset($app_fields_cache[$entities_id]))
  	{
  		foreach($app_fields_cache[$entities_id] as $fields)
  		{
  			if($fields['type']=='fieldtype_google_map')
  			{
  				...
  				//get item info
  				$item_info_query = db_query("select * from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING)." where id='" . filter_var($items_id,FILTER_SANITIZE_STRING)."'");
  				...
  				{
  					db_query("update app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." set field_".filter_var($fields_id,FILTER_SANITIZE_STRING)."='' where id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");
  				...
  				if(!strlen($lat) or $use_address!=$current_address)
  				{
  					...
  					else
  					{
  						...
  						db_query("update app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." set field_".filter_var($fields_id,FILTER_SANITIZE_STRING)."='" . db_input(filter_var($value,FILTER_SANITIZE_STRING)) . "' where id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");  				  						
  					}
  				}
