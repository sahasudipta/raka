...
class fieldtype_hours_difference
{
	...
	function get_configuration()
	{
		...
		$fields_query = db_query("select * from app_fields where type in ('fieldtype_input_date','fieldtype_input_datetime','fieldtype_dynamic_date') and entities_id='" . db_input(filter_var($_POST['entities_id'],FILTER_SANITIZE_STRING)) . "'");
...
END;";
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
  	...
  	{
  		$mysql_query =  "rukovoditel_hours_diff(" . $start_date_field . "," . $end_date_field . ",'" . $include_hours . "','" . $exclude_days . "','" . $cfg->get('exclude_holidays') . "') as field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING);
  	}
		...
		if(isset($app_fields_cache[$entities_id]))
		{
			foreach($app_fields_cache[$entities_id] as $fields)
			{
				if($fields['type']=='fieldtype_hours_difference')
				{
					...
					{
						$item_info_query = db_query("select " . self::prepare_query(filter_var($fields,FILTER_SANITIZE_STRING),'e',false,true) . " from app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." e where e.id='". filter_var($items_id,FILTER_SANITIZE_STRING)."'");
