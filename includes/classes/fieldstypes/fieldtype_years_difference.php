...
END;";
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
		...
		{
			$mysql_query =  "rukovoditel_years_diff(" . $start_date_field . "," . $end_date_field . "," . $cfg->get('calclulate_diff_days',0). ") as field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING);
		}
		...
		if(isset($app_fields_cache[$entities_id]))
		{
			foreach($app_fields_cache[$entities_id] as $fields)
			{
				if($fields['type']=='fieldtype_years_difference')
				{
					...
					{
						$item_info_query = db_query("select " . self::prepare_query(filter_var_array($fields),'e',false,true) . " from app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." e where e.id='". filter_var($items_id,FILTER_SANITIZE_STRING)."'");
