...
class fieldtype_related_records
{
  ...
  function get_ajax_configuration($name, $value)
  {
  	...
  	switch($name)
  	{
  		case 'fields_for_search_box':
  			...
  			$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in (" . fields_types::get_types_for_search_list(). ") and  f.entities_id='" . filter_var($entities_id) . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
  ...
  static function prepare_query_select($entities_id,$listing_sql_query_select, $reports_info=array())
  {
    ...
    $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in ('fieldtype_related_records') and " . (strlen(filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING)) ? "find_in_set(f.id,'" . filter_var($reports_info['fields_in_listing'],FILTER_SANITIZE_STRING) . "')":"f.listing_status=1"). " and f.entities_id='" . db_input(filter_var($entities_id,FILTER_SANITIZE_STRING)) . "' and f.forms_tabs_id=t.id  order by t.sort_order, t.name, f.sort_order, f.name");
    ...
    {    	
      ...
      if(strlen($table_info['sufix'])>0)
      {
      	$where_sql = " or ri.entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . filter_var($table_info['sufix'],FILTER_SANITIZE_STRING) . "_items_id=e.id ";
      }
      
      $listing_sql_query_select .= ", (select count(*) as total from " . filter_var($table_info['table_name'],FILTER_SANITIZE_STRING) . " ri where ri.entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . "_items_id=e.id {$where_sql}) as field_" .filter_var($field['id'],FILTER_SANITIZE_STRING);                                            
    }
