...
class fieldtype_access_group
{
  ...
  static function get_choices($field,$value='')
  {
  	...
  	$where_sql = "where (select count(*) from app_entities_access ea where ea.access_groups_id=ag.id and entities_id='" . db_input(filter_var($field['entities_id'],FILTER_SANITIZE_STRING)) . "' and length(access_schema))>0";
  	...
  	{
  		$choices[filter_var($groups['id'],FILTER_SANITIZE_STRING)] = filter_var($groups['name'],FILTER_SANITIZE_STRING);
  	}
	...
	function render($field,$obj,$params = array())
  {
    ...
    if($cfg->get('display_as')=='dropdown')
    {
      ...
      return select_tag('fields[' . $field['id'] . ']',array(''=>(strlen($cfg->get('default_text')) ? $cfg->get('default_text'): TEXT_NONE))+$choices,$value,$attributes). fields_types::custom_error_handler($field['id']);
    }
  ...
  {  	  	  	
  	...
  	$users_query = db_query("select id from app_entity_1 where field_6 in (" . filter_var($value) . ") and field_5=1");
  	while($users = db_fetch_array($users_query))
  	{
  		$send_to[] = $users['id'];
  	}
