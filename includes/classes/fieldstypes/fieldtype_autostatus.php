...
class fieldtype_autostatus
{
  ...
  static function set($entities_id, $items_id)
  {
  	...
  	{  		
  		...
  		{
	  		$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input($entities_id). "' and reports_type='fields_choices" . filter_var($choices['id'],FILTER_SANITIZE_STRING) . "'");
	  		...
	  		{	  				  				  				  				  			
	  			...
	  			$item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING), '') . " from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . " e where e.id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "' " . $listing_sql_query);
