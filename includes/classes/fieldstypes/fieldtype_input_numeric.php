...
class fieldtype_input_numeric
{
  ...
  function render($field,$obj,$params = array())
  {
  	...
  	$value = $obj['field_' . filter_var($field['id'],FILTER_SANITIZE_STRING)];
    ...
    $attributes = array('class'=>'number form-control ' . $cfg->get('width') .
                        ' fieldtype_input_numeric field_' . filter_var($field['id'],FILTER_SANITIZE_STRING) . 
    ...
    if(count($currencies)>1)
    {
    	...
    	{ 
    		...
    		{
    			$attributes = currencies::prepare_input_attributes($attributes,$currency['code'],'-' . filter_var($field['id'],FILTER_SANITIZE_STRING) . ' currency-field-grouped');
    		...
    		else 
    		{
    			$html .= '
    						<div class="input-group input-small" style="margin-top: 3px;">
									...
									' . input_tag('currency_' . $currency['code'],'',array('class'=>'form-control currency-field-'. filter_var($field['id'],FILTER_SANITIZE_STRING) . ' currency-field-grouped','data-field-id'=>filter_var($field['id'],FILTER_SANITIZE_STRING),'data-currency-value'=>$currency['value'],'data-currency-default'=>0)) . '																				
								</div>
    					';
    		}
    	}
    ...
    if(strlen($cfg->get('min_value')) and strstr($cfg->get('min_value'),'['))
    {
    	...
    	$html .= '
    		<script>
    			$(function(){
    			  $("#fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) .'").keyup(function(){ $(this).attr("min",number_format($("#fields_' . $field_val_id . '").val(),"' . $decimals . '",".","")) })
    			  $("#fields_' . $field_val_id .'").change(function(){ $("#fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) .'").attr("min",number_format($(this).val(),"' . $decimals . '",".","")) })
    			});
    		</script>
    			';
    }
    ...
    if(strlen($cfg->get('max_value')) and strstr($cfg->get('max_value'),'['))
    {
    	...
    	$html .= '
    		<script>
    			$(function(){
    			  $("#fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) .'").keyup(function(){ $(this).attr("max",number_format($("#fields_' . $field_val_id . '").val(),"' . $decimals . '",".","")) })    			
    			  $("#fields_' . $field_val_id .'").change(function(){ $("#fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) .'").attr("max",number_format($(this).val(),"' . $decimals . '",".","")) })
    			});
    		</script>
    			';
    }
    ...
    if($cfg->get('display_prefix_suffix_in_form')==1 and (strlen($cfg->get('prefix')) or strlen($cfg->get('suffix'))))
    {
    	return '
    			<div class="input-group ' . $cfg->get('width') . '">
						...
						. input_tag('fields[' . filter_var($field['id'],FILTER_SANITIZE_STRING) . ']',$value,$attributes) 
						. (strlen($cfg->get('suffix')) ? '<span class="input-group-addon">' . $cfg->get('suffix') . '</span>':'') .
				'</div>
				<label id="fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '-error" class="error" for="fields_' . filter_var($field['id'],FILTER_SANITIZE_STRING) . '" style="none"></label>		    
    			' . $html;
    }
    else
    {    	        
    	return input_tag('fields[' . filter_var($field['id'],FILTER_SANITIZE_STRING) . ']',$value,$attributes) . $html;
    }
  }
