  	...
  	if(isset($app_fields_cache[$entities_id]))
  	{
  		foreach($app_fields_cache[$entities_id] as $fields)
  		{
  			if($fields['type']=='fieldtype_google_map_directions')
  			{
  				...
  				{  					  					  					
  					db_query("update app_entity_". filter_var($entities_id,FILTER_SANITIZE_STRING)." set field_".filter_var($fields_id,FILTER_SANITIZE_STRING)."='" . db_input(implode("\n",filter_var($address_values,FILTER_SANITIZE_STRING))) . "' where id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");  					
  				}
