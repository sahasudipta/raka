...
END;";
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
  	...
  	{
  		$mysql_query =  "rukovoditel_days_diff(" . $start_date_field . "," . $end_date_field . ",'" . $exclude_days . "','" . $cfg->get('exclude_last_day') . "','" . $cfg->get('exclude_holidays') . "') as field_" . filter_var($fields['id'],FILTER_SANITIZE_STRING);
  	}
	...
	public static function update_items_fields($entities_id, $items_id)
	{
		global $app_fields_cache;
                ...
                $items_id = filter_var($items_id,FILTER_SANITIZE_STRING);
		
		if(isset($app_fields_cache[$entities_id]))
		{
			foreach(filter_var_array($app_fields_cache[$entities_id]) as $fields)
			{
				if($fields['type']=='fieldtype_days_difference')
				{
					$cfg = new fields_types_cfg($fields['configuration']);
					...
					if(isset($cfg->cfg['dinamic_query']) and $cfg->get('dinamic_query')!=1)
					{
						...
						db_query("update app_entity_".filter_var($entities_id,FILTER_SANITIZE_STRING)." set field_".filter_var($fields_id,FILTER_SANITIZE_STRING)."='" . $item_info['field_' . filter_var($fields_id,FILTER_SANITIZE_STRING)] . "' where id='" . db_input(filter_var($items_id,FILTER_SANITIZE_STRING)) . "'");												
					}
