...
class image_map
{
	...
	function __construct($map_id)
	{
		...
		$this->scale = 'default';
		$this->fields_in_popup = array();
		$this->background = false;
		$this->field_info = array();
		$this->field_cfg = array();
	...
	function get_regions_labels($map_info)
	{
		...
		$choices_query = db_query("select * from app_fields_choices where parent_id = '" . db_input(filter_var($map_info['id'],FILTER_SANITIZE_STRING)). "' order by sort_order, name");				
		while($choices = db_fetch_array($choices_query))
		{
			$x = $y = 0;
				
			//get possition
			$label_info_query = db_query("select * from app_image_map_labels where choices_id = '" . filter_var($choices['id'],FILTER_SANITIZE_STRING). "' and map_id='" . filter_var($map_info['id'],FILTER_SANITIZE_STRING) . "'");
	...
	function get_markers()
	{
	    ...
	    $items_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($entity_id, '') . " from app_entity_" . $entity_id . " e where id='" . $item_id . "'");	     
		}
		...
		{	
			...
			$listing_sql = "select e.* " . $sql_query_select . " from app_entity_" . filter_var($reports['entities_id'],FILTER_SANITIZE_STRING) . " e  where e.id>0 " . $listing_sql_query;
		...
		while($items = db_fetch_array($items_query))
		{
			...
			//get possition 
			$marker_info_query = db_query("select * from app_image_map_markers where entities_id='" . $entity_id . "' and items_id = '" . $items['id']. "' and map_id='" . $this->map_id . "'");
			...
			if($this->background)
			{
				if(isset($items['field_' . $this->background]))
				{
					...
					//prepare icon 
					if(strlen($icon_filename = $this->field_cfg->get('icon_' . $items['field_' . $this->background])))
					{
					...
					'html' => '
						<div class="cfm-inner">
							<div class="cfm-title-params">
								...
								<span class="cfm-title"><a href="' . url_for('items/info','path=' . $entity_id . '-' . $items['id']) . '" target="_new">' . items::get_heading_field($entity_id, $items['id'], $items) . '</a></span>
							</div>
	...
	function get_html($map_info)
	{
		...
		if($this->background)
		{
			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($this->background). "' order by sort_order, name");
			while($choices = db_fetch_array($choices_query))
			{				
		...
		if($this->reports_id)
		{						
			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($this->field_info['id']). "' and parent_id=0 order by sort_order, name");
			if(db_num_rows($choices_query)>1)
			{
		        ...
		        <ul class="dropdown-menu">';
					...
					while($choices = db_fetch_array($choices_query))
					{
						$maps .= '<li><a target="_parent" href="' . url_for('ext/image_map/view','id=' . $this->reports_id . '&map_id=' . $choices['id']) . '">' . $choices['name'] . '</a></li>';
					}
	...
	{
		$map_id = filter_var(_get::int('map_id'),FILTER_SANITIZE_STRING);
		...
		{
			$reports_query = db_query("select * from app_ext_image_map where id='" . db_input(filter_var($_GET['reports_id'],FILTER_SANITIZE_STRING)) . "'");
			$reports = db_fetch_array($reports_query);
				
			$entity_id = $reports['entities_id'];
		}
		
		foreach(filter_var_array($_POST['markers']) as $marker)
		{
			$sql_data = array(
					'x' => $marker['x'],
					'y' => $marker['y']
		  ...
		  $marker_info_query = db_query("select * from app_image_map_markers where entities_id='" . db_input(filter_var($entity_id,FILTER_SANITIZE_STRING)) . "' and items_id = '" . db_input(filter_var($marker['id'],FILTER_SANITIZE_STRING)). "' and map_id='" . db_input(filter_var($map_id,FILTER_SANITIZE_STRING)) . "'");
		  if($marker_info = db_fetch_array($marker_info_query))
		  {
		  	db_perform('app_image_map_markers',$sql_data,'update',"id='" . db_input(filter_var($marker_info['id'],FILTER_SANITIZE_STRING)) . "'");
		  }
		  else
		  {
	...
	static function save_labels()
	{
		...
		foreach(filter_var_array($_POST['labels']) as $label)
		{
			$sql_data = array(
					'x' => $label['x'],
					'y' => $label['y']
			...
			$label_info_query = db_query("select * from app_image_map_labels where choices_id = '" . db_input($label['id']). "' and map_id='" . db_input($map_id) . "'");
			if($label_info = db_fetch_array($label_info_query))
			{
	...
	{
		db_query("delete from app_image_map_markers where entities_id='" . $entities_id . "' and items_id='" . $items_id . "'");
	}
	...
	static function render_markers_color($fields_id)
	{
		...
		if(strlen($cfg->get('background')))
		{
			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input(filter_var($cfg->get('background'),FILTER_SANITIZE_STRING)). "' and length(bg_color)>0 order by sort_order, name");
			while($choices = db_fetch_array($choices_query))
			{
				$css .= '
