...
while($fields = db_fetch_array($fields_query))
{
	...
	foreach(fields_choices::get_tree(filter_var($fields['id'],FILTER_SANITIZE_STRING),0,[],0,'','',true) as $choices)
	{
		$reports_info_query = db_query("select * from app_reports where entities_id='" . db_input(filter_vars($fields['entities_id'],FILTER_SANITIZE_STRING)). "' and reports_type='fields_choices" . filter_var($choices['id'],FILTER_SANITIZE_STRING) . "'");
		if($reports_info = db_fetch_array($reports_info_query))
		{
			$choices_to_reports_id[$choices['id']]=$reports_info['id'];
			
			$reports_filters_query = db_query("select f.id, f.type from app_reports_filters rf, app_fields f where reports_id='" . filter_var($reports_info['id'],FILTER_SANITIZE_STRING) . "' and rf.fields_id=f.id");
	...
	foreach($autostatus_fields as $autostatus_field)
	{
		...
		foreach($autostatus_field['choices'] as $choices)
		{
			...
			{	
				...
				$item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING), '',false,array('fields_in_listing'=>implode(',',filter_var_array($filters_fields))))  . fieldtype_related_records::prepare_query_select(filter_var($entities_id,FILTER_SANITIZE_STRING), ''). " from app_entity_" . filter_var($entities_id,FILTER_SANITIZE_STRING) . " e where e.id>0 " . $listing_sql_query . (count($exclude_items) ? " and e.id not in (" . implode(',',filter_var_array($exclude_items)). ")":''),false);
